...
| 4 | 音量つまみ（ADC＋連動スイッチでソフトOFF） | ⚠ ソフトは実機確認済み。配線取り違えを修理、熱で傷んだ抵抗カーブはソフトで補正（`kKnobCurve`）。**非接触（ホール式・AS5600）へ移行中**（[docs/KNOB-ENCODER.md](docs/KNOB-ENCODER.md)）。**回すとコーデックが死ぬ問題は未解決**（半田をやり直しても再発。[docs/KNOB-TROUBLE.md](docs/KNOB-TROUBLE.md)） |
