...
| 4 | 音量つまみ（ADC＋連動スイッチでソフトOFF） | ✅ 実機確認済み。配線取り違えを修理、熱で傷んだ抵抗カーブはソフトで補正（`kKnobCurve`）。**非接触（ホール式・AS5600）へ移行中**（[docs/KNOB-ENCODER.md](docs/KNOB-ENCODER.md)）。コーデックが稀に落ちる件だけ未解決で残る（[docs/KNOB-TROUBLE.md](docs/KNOB-TROUBLE.md)） |
