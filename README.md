...
1. **時刻の注入** — LLMは時計を持たず、聞かれると嘘の日時を答える。仕様書4章の
   「お薬の時間だよ」のようなプッシュを実装するなら、見守り用途では危険。
   DO側で接続時に現在時刻を systemInstruction へ埋めるのが最小の対処。
2. **ウェイクワード** — 今はボタン起動。ReSpeaker の `ch0-asr/ch1-mww` ファームに
   載せ替えるのが確実（自前のTinyMLより楽）。`KATANORI_MIC_CHANNEL` で切り替える。
3. **つなぎ言葉のローカル再生** — 応答までの実測1.3秒を埋める。仕様書5章。
4. **QRコードでのペアリング** — 仕様書3章。DO側にアカウントと個体番号の管理が必要で、
   現状の DO は `robot-1` 固定。ここだけ規模が大きい。
5. **証明書の有効期限検証** — `CONFIG_MBEDTLS_HAVE_TIME_DATE` が無効のため未検証。
   有効にするにはフレームワークの再ビルドが必要。
