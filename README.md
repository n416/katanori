...
| 電源 | 電池駆動（電池 → PowerBoost 直結・USB なし） | ✅ 起動・完全停止・再起動まで確認（2026-09-02）。稼働時間は起きたまま・会話なしで約 2 時間 13 分（2026-09-25・[docs/POWER.md](docs/POWER.md) 6章）。ピーク電流は未実測 |
...
| | 状態 |
|---|---|
| **いまの実機** | v5 の筐体（自宅のレジン）＋ ユニバーサル基板のハブ ＋ ReSpeaker Lite ＋ XIAO。2026-09-02 に電池で起動・会話・ミュートリレーまで通った |
...
4. **届いたら組み替える** — E リングを実物で嵌める・リードスイッチの反応距離（MJF で 0.5 遠くなった）
...
- **勝手に再起動する**（2026-09-11・原因未特定）。起動の声が 5〜15 分おきに鳴り、電池を使い切った。2026-09-24 にユーザー判断で追わないことにした（クローズ）。2026-09-25 の稼働時間の実測で、電池が減ると同じ形でブラウンアウトの再起動を繰り返すのを見た（[docs/POWER.md](docs/POWER.md) 6章）。
