...
| `wifi` | Wi-Fi へ接続 |
| `wifioff` | Wi-Fi を切断 |
| `c` | Durable Object へ WebSocket 接続 |
| `d` | WebSocket を切断 |
| `n` | ネットワーク状態を表示 |
...
- **自動再接続はしません。** DO はクライアント接続と同時に Gemini へ繋ぎにいくため、
  再接続を繰り返すと Gemini のセッションを浪費します。切断されたら `c` で張り直す方針です
  （`wrapper.py` と同じ「会話するときだけ繋ぐ」設計）。
- ESP32 は **2.4GHz帯のみ**です。5GHz専用のSSIDには接続できません。

### 既知の未対応: TLS証明書の検証

現在 `beginSSL()` に fingerprint を渡していないため、ライブラリ内部で `setInsecure()` が
呼ばれ、**サーバ証明書を検証していません**。中間者攻撃を検出できない状態です。
接続性の確認を優先した Stage 2 の暫定実装で、接続時に警告をログへ出しています。
出荷前に `beginSslWithCA()` でルート証明書を渡すか証明書バンドルを積むこと。
