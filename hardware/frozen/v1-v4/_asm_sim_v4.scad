...
// 🆕 2026-08-28（16 度目の机上の通し）検査の「相手が空でないか」を工具の道の側でも見るために足した 2 つ。
//   15 度目は当たり検査の 0 だけを疑い、**円筒を撃つ側は疑っていなかった**。相手に居ない物は当たらないので、
//   居ないまま撃てば「外まで通る」が出る。
//
//   ST="nb_<ID>" ＝ 手順 2 まで ＋ **その口以外の 9 本を挿した姿**。
//   手順 3 の「口を摑む」は st2（＝口が 1 本も無い姿）へ撃っていたので、先に挿した隣が居なかった。
for (id = PLUGGED_9) if (ST == str("nb_", id)) {
    upto(2);
    translate([HUB_DX, HUB_DY, 0]) for (j = PLUGGED_9) if (j != id) housing(j);
}
//   ST="w9_<束>" ＝ 手順 9 の姿から**その口自身の束だけ**を抜く。
//   st9r は 10 束を丸ごと抜いていたので、**隣の束**も障害物から消えていた。
//   ⚠ 線は剛体の管で描いてあるので、隣の束に当たる ＝ 組めない、ではない（実物の線は逃げる）。
//     この段は「順番が効いているか」を見るためのもの。
for (b = ["oled", "ina_i2c", "chg", "pwr3", "bat_ext", "batout"]) if (ST == str("w9_", b)) {
    upto(8); s_oled(); oled_hous(); w_tgl(); w_reed();
    if (b != "oled")    w_oled();
    if (b != "ina_i2c") w_ina_i2c();
    if (b != "chg")     w_chg();
    if (b != "pwr3")    w_pwr3();
    if (b != "bat_ext") w_bat_ext();
    if (b != "batout")  w_batout();
}
if (ST == "topsub") s_top();
