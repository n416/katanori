...
G = 2.54
# ⭐ 2026-09-17: Basic 化で選び直した R6・R7（C22961 / C23254）は **0603**。
#   （同じときに選び直した R2 C23137 は、2026-09-20 に低電池の分圧ごと廃止した）
...
part("U1", "Regulator_Switching:TPS61090", "TPS61090RSAR", X0 + 30, Y0 + 8,
     {"1": "V5", "15": "V5", "16": "V5", "2": None, "3": "SW", "4": "SW",
      ...
      "8": "VSYS", "9": "GND", "10": "GND", "11": "EN", "12": None, "14": "FB"},
...
# ⭐ 2026-09-20: 低電池の分圧（R1 1.87M・R2 330K）を外し、LBI（9 番）を GND へ直結した。
#   LBO（12 番）はどこにも繋いでいないので、この分圧が作る閾値 3.33V は誰も読まない。
#   📄 TPS61090/91/92 SLVS484C（2014 年 12 月改訂）12 ページ 9.3.7:
#     "If the low-battery detection circuit is not used, the LBI pin should be connected to GND
#      (or to VBAT) and the LBO pin can be left unconnected."
#   ⇒ 浮かせなければよいのではなく、GND へ繋げと書いてある。分圧である必要は無い。
#   低電池は INA226（U3）が I2C で読むので、機能としても重複していた。
#   得: 部品 2 個・VSYS から GND へ流れていた 1.9µA・JLCPCB へ出す品番 1 つ（330kΩ C23137 は R2 だけが使っていた）。
#   ⚠ 2026-09-19 に「分圧は残す」と書かれていたが（docs/POWER.md）、それは AI 側が決めた物で
#     ユーザーに聞いていなかった。2026-09-20 にユーザーの判断で外した。
...
# 🔴🔴 **R41 はケルビンで測る。**回路図では U3 の 9/10 番と R41 が「同じネット」としか書けないが、
#   それでは足りない。**同じ点**から取らないと、あいだの銅がシャントに足し算されて読み違える。
#   2026-09-16 に実測: v6.1 の自動配線は IN+ を R41 のパッドの 11.9mΩ 手前（Q2 の足元）から、
#   IN− を 73.7mΩ 先（U2 の VBAT パッド）から出していて、INA226 は 10mΩ ではなく **96.7mΩ**
#   ＝ 9.7 倍を見ていた。v5 で正しく読めていたのは、INA226 モジュールの上でシャントと IC が
#   数 mm しか離れておらず、ケルビンがモジュールの中で取れていたからである。
#   ⇒ 板の側の担保は **gen_pcb.py の PRE_TRACKS**。R41 の 2 つのパッドの**内側の縁**から、
#     負荷電流を流さない 0.25mm の線を 2 本、電流の向きと直角に出して U3 へ直行させている。
#     この回路図を作り直しても PRE_TRACKS は gen_pcb.py 側に残るが、**R41 か U3 を動かしたら
#     PRE_TRACKS の座標も直すこと**（座標は板の実寸で書いてある）。
r("R41", "10m", XI - 12, YI + 8, "BATP", "VLIPO", fp="Resistor_SMD:R_1206_3216Metric",
  note="シャント 10mΩ（今のモジュールと同じ値）。🔴 ケルビン必須 — gen_pcb.py の PRE_TRACKS を見よ")
...
part("J6", "Connector_Generic:Conn_01x02", "BTN2", 0, 0,
     ...
     grp="ports")
# ⭐ 2026-09-19 🔒 ユーザー「板の後ろの縁に押しボタンを載せる方法にしようか」: ハッチのトグルをやめて板の押しボタン SW2 へ。
#   J7 はリードだけの 2 ピンになった（3 EN・4 GND のトグルの組は消えた）
part("J7", "Connector_Generic:Conn_01x02", "REED", 0, 0,
     {"1": "EN", "2": "GND"},
     "Connector_JST:JST_PH_B2B-PH-K_1x02_P2.00mm_Vertical",
     note="1 EN・2 GND → リード（縦の口・上から挿す）", grp="ports")
# 電源の押しボタン（横向き・自己保持・DPDT）。リードと並列に EN を GND へ落とす。
#   📄 データシートの CIRCUIT DIAGRAM: 1–3 番は RELEASE で閉・LOCK で開 ⇒ 出ている間は OFF、押し込んで固定すると電源はつまみ（リード）に従う。
#   残りの 2・4・5・6 番は使わない。1–3 が「出ている間に閉」の根拠: 📄 CIRCUIT DIAGRAM（RELEASE で 1–3・LOCK で 2–4。4–6・2–5 は常に結ばれた同じ端子）と、📄 PCB LAYOUT のピンの前後（1・3 は胴の前寄り、2・4 はその 2.0 後ろ）。動く接点は押し子と一緒に前後し、出ていると前の 1・3、押し込むと後ろの 2・4 をつなぐ。2 つの図が同じことを言う
part("SW2", "katanori:SW_XKB5858-Z-E", "XKB5858-Z-E", 0, 0,
     {"1": "GND", "3": "EN", "2": None, "4": None, "5": None, "6": None},   # ⭐ 2026-09-19: 1 EN・3 GND → 1 GND・3 EN（無極性の 1 組。EN の行き先は右なので右の 3 番へ。左の 1 番は J6 との 1.1 の隙間で EN が引けなかった）
     "katanori:SW_XKB5858-Z-E",
     note="電源の押しボタン（板の後ろの縁・押し子は後ろの壁から出る）。1 GND・3 EN", grp="ports")
...
LCSC = {
    ...
    "R3": "C482988",                                            # 拡: 1.87MΩ 1%（R1 は 2026-09-20 に廃止）
    ...
    "SW2": "C780038",                                           # XKB5858-Z-E（電源の押しボタン・押し子 5.0・スルーホール・手はんだ。2026-09-19 LCSC $0.30）。⭐ 同日 -75（C780035・押し子 2.5）から替えた: キャップが付かない
}
for P in PARTS:
    if P["ref"] in LCSC:
        P["lcsc"] = LCSC[P["ref"]]
