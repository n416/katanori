...
# ⭐ 2026-09-20: 低電池の分圧（元の R1 1.87M・R2 340K）も写さない。LBO をどこにも繋いでいないので
#    閾値を誰も読まず、低電池は INA226 が I2C で読む。📄 SLVS484C 12 ページ 9.3.7 が
#    「使わないなら LBI は GND へ繋げ」と書いているので、分圧ではなく直結にした（gen_sch.py の U1）
DROPPED |= {"R1", "R2"}
...
CHANGED = {("J10", "+"): "電池の + と VLIPO のあいだに、逆接保護の P-ch（Q2）と INA226 のシャント（R41）を入れた",
           ("U1", "LBI"): "低電池の検出を使わないので、分圧を外して LBI を GND へ直結した（📄 SLVS484C 9.3.7）"}
