...
POWER_V61 = {
    ...
    "R4": (21.0, 10.65, 180), "R3": (24.1, 10.65, 180),
...
def bat_trunk(placed):
    ...
    y = 9.5                          # 前の帯。U4 の胴の下端 11.35 から 1.85 下・J4 の足 8.675〜10.425 は X 73.7 から
    k1 = q[1] - y                    # 45° で前へ下りる量
    x2 = j[0] - (j[1] - y)           # 45° で後ろへ上がりはじめる X（J10 の足の真下で合流する）
    tracks = [("BATRAW", "B.Cu", BAT_W, [bx(j[0], j[1]), bx(j[0], j[1] - 0.5),
                                         bx(x2, y), bx(vx + k1, y), bx(vx, q[1])]),
...
def build():
    ...
    _PRE_VIAS = []
    for tr, vi in (v5_trunk(placed), bat_trunk(placed)):
        _KELVIN += tr
        _PRE_VIAS += vi
