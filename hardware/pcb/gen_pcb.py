...
PRE_TRACKS = [
    # ---- シャント R41 のケルビン（2026-09-16・ステップ 2）----
    # R41 は 1206（パッド 1.125 × 1.75）。電流は **X 方向**に流れる（右の 1 番から左の 2 番へ）。
    # センスは「パッドの内側の縁から、電流と直角に出す」。ここを守らないと、途中の銅が
    # シャントに足し算されて INA226 が読み違える（旧配置では 10mΩ に 86.7mΩ が足されて 9.7 倍だった）。
    #   R41 1 番（BATP）… パッドは X 79.72〜80.845。内側の縁 79.72 のすぐ内で拾う
    #   R41 2 番（VLIPO）… パッドは X 76.79〜77.92。内側の縁 77.92 のすぐ内で拾う
    # 2 本は交差しない（出口も行き先も BATP のほうが右）。負荷電流は流れないので 0.25mm で足りる。
    ("BATP",  "F.Cu", 0.25, [(79.900, 63.100), (81.161, 61.900), (81.161, 61.500)]),
    ("VLIPO", "F.Cu", 0.25, [(77.750, 63.100), (80.161, 61.900), (80.161, 61.500)]),
]
_n = [0]
...
POWER_V61 = {
    ...
    "U2":  (33.0, 15.2, 0),     # 充電 MCP73871
    # 🔴 180°: 1 番（BATP・Q2 から来る）を **右**に、2 番（VLIPO・U2 へ行く）を左に。
    #    0° だと入口が左・出口が右になり、右の Q2 から来た電流が自分を跨いで左の U2 へ戻る
    "R41": (39.0, 13.5, 180),   # シャント 10mΩ
    ...
    # ---- Y 18.5 の帯: パスコンと電流計 ----
    # ⭐ R1/R2（LBI の分圧）を U1 の 9 番＝右の辺（板 X 23.96）の隣へ。9mm 離すと 1 本つながらない
    "R1": (24.5, 16.6, 0), "R2": (27.2, 16.6, 0),
    "C1":  (24.5, 19.6, 0), "C4": (27.5, 19.6, 0),   # VSYS
    "C7":  (30.5, 18.5, 0), "C8": (33.5, 18.5, 0),   # VLIPO・VBUS
    "C41": (36.2, 18.5, 0),
    # 270°: 6〜10 番の辺（8/9 = IN−・10 = IN+）を R41 の側（−Y）へ向ける
    "U3":  (39.0, 18.8, 270),   # INA226 — R41 の真上（ケルビン）
    "R42": (43.0, 18.5, 0), "R43": (46.0, 18.5, 0),  # I2C の引き上げ
    # ---- Y 22.5 の帯: 分圧と大きいコンデンサ ----
    "R3": (20.0, 22.5, 0), "R4": (23.0, 22.5, 0),
    "R13": (32.0, 22.5, 0),
    "R6": (35.0, 22.5, 0), "R7": (38.0, 22.5, 0),
    "C6": (46.0, 22.5, 0), "C9": (49.0, 22.5, 0),
    # ---- Y 27 の帯: 電池残量と 5V の印 ----
    "LED1": (32.0, 27.0, 0), "R20": (35.0, 27.0, 0), "Q1": (38.0, 27.0, 0),
    "LED2": (22.0, 27.0, 0), "R5":  (25.0, 27.0, 0),
}
FIXED_V61 = {
...
def pre_tracks(nets):
    ...
    out = []
    for net, lay, w, pts in PRE_TRACKS:
