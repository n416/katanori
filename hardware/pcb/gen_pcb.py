...
#   いまの電池の道（測値）:
#     J10 → Q2   B.Cu 28.6mm ＋ F.Cu 1.6mm ＋ 穴 1 個   **23.9mΩ**
#     Q2  → R41  F.Cu 6.5mm                          5.3mΩ 以下
#     R41 → U2   F.Cu（vlipo_link で先に引いてある）        2.8mΩ
#   合計 32mΩ 前後。充電の上限 1A で 32mV・実測の 560mA で 18mV。
...
POWER_V61 = {
    ...
    "U3":  (40.5, 19.3, 270),   # INA226 — R41 の真上（ケルビン。X を R41 と揃える）
    ...
    "C2": (22.0, 8.6, 0), "C6": (25.5, 8.6, 0), "C9": (29.0, 8.6, 0),
    ...
    "R4": (20.4, 10.65, 180), "R3": (24.7, 10.65, 180),
...
def bat_trunk(placed):
    ...
    #     lane_x 72.0 … 立入禁止の東の縁 71.2 の外・J4 の足（X 73.7 から）の 1.4 手前
    #     lane_y  6.5 … U4 の胴の下端から 4.85 下。取付穴 (79.354, 3.5) の逃げ 5.1 より上
    lane_x, lane_y = 72.0, 6.5
...
def vbus_trunk(placed):
    """USB-C から充電 IC の入口のコンデンサ（C8）までを **裏の面**で通す（2026-09-17 夕）。

    VBUS は幅を決める組に入っていない（U2 の QFN-20 の 6 本から 0.4mm が逃げられないため）。
    そのもで自動配線が幹も 0.2mm で内層へ逃がし、J13 → U2 が **102.5mΩ**、
    うち In2.Cu 0.2mm × 17.6mm が 87mΩ だった。
    MCP73871 はリニア充電なので VBUS には **充電電流 ＋ 機体の電流**が乗る。
    実測からの見積もりで 約 0.78A、R16 1.0kΩ の設計上限で 約 1.22A。
    このファイルの幅の表は 0.2mm で外層 1oz 許容 0.74Aで、**内層はその半分 約 0.37A**。
    実測ベースでも 2 倍超えていた。ーー 81 行の「5V 側なので効かない」は電圧降下だけを見た判断で、
    発熱と内層 0.5oz を見ていなかった。
    ⚠ 幹（J13 → C8）だけを太くする。C8 → U2 は 3mm の枝で、U2 の 0.25mm のパッドに
      0.6mm を押し込むと周りのピンが逃げなくなる。SEL・PROG2・TE・CE への枝は無電流。
    """
    ax, ay, _, ah = pad_box(placed, "J13", "A9")     # USB-C の VBUS（左側の 1 組）
    cx, cy, _, ch = pad_box(placed, "C8", "1")       # 充電 IC の入口のコンデンサ
    a = xb(ax, ay); c = xb(cx, cy)
    y1 = a[1] - 1.7          # J13 のパッドの下。USB-C の殻の足（板 Y 30.87〜32.97）を避ける
    y2 = c[1] + 1.7          # C8 のパッドの上。ここで表へ戻る
    k = a[0] - c[0]          # 45° で左へ寄せる量（X の差）
    tracks = [("VBUS", "F.Cu", BAT_W, [bx(a[0], a[1]), bx(a[0], y1)]),
              ("VBUS", "B.Cu", BAT_W, [bx(a[0], y1), bx(c[0], y1 - k), bx(c[0], y2)]),
              ("VBUS", "F.Cu", BAT_W, [bx(c[0], y2), bx(c[0], c[1])])]
    return tracks, [("VBUS", bx(a[0], y1)), ("VBUS", bx(c[0], y2))]
...
def build():
    ...
    _PRE_VIAS = []
    for tr, vi in (v5_trunk(placed), bat_trunk(placed), vbus_trunk(placed)):
        _KELVIN += tr
        _PRE_VIAS += vi
    ...
    kx0, kx1, ky0, ky1 = 60.2, 71.2, 7.8, 19.8          # U4 の胴 X 63.75〜67.65 / Y 11.35〜16.25 のまわり
    KX, KY = bx(kx0, ky1)
    KX2, KY2 = bx(kx1, ky0)
