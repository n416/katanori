...
# 🔴 2026-09-17 夕に測り直した。**その日の朝に書いた「J10 → Q2 31.16mm・25.5mΩ（B.Cu）」は誤り**。
#   長さだけで計算していて、その区間がどの層にあるかを見ていなかった。実際は内層 In2.Cu に
#   34.27mm 乗っており、JLCPCB の 4 層は内層が 0.5oz なので抵抗は倍で **56.6mΩ** だった。
# ⭐ そこで bat_trunk() を入れ、J10 → Q2 を **裏の面**へ先に引いて protect で固定した。
#   いまの電池の道（測値）:
#     J10 → Q2   B.Cu 37.6mm ＋ F.Cu 1.6mm ＋ 穴 1 個   **32mΩ**
#       （⭐ 2026-09-17 夜に板から測り直した。28.6mm・23.9mΩ は AS5600 をよけて回す前の値。bat_trunk() を見よ）
#     Q2  → R41  F.Cu 6.5mm                          5.3mΩ 以下
#     R41 → U2   F.Cu（vlipo_link で先に引いてある）        2.8mΩ
#   合計 40mΩ 前後。充電の上限 1A で 40mV・実測の 560mA で 22mV。
#   ⚠ 幅の表（上）は外層 1oz で計算した値。**内層はその 2 倍で読むこと**。
...
BAT_W = 0.6
VBUS_PIN_W = 1.2     # C8 の 1 番 → U2 の 17〜19 番（3 本とも VBUS）へ降ろす線。vbus_trunk() を見よ
...
V5_NETS = {"V5"}
V5_W = 0.8
# ⭐ 2026-09-17 夕: スピーカーの 3 本。既定の 0.2mm のままだったので、内層 0.2mm で
#   SPKM 93.1mm＝458mΩ・SPKP 138mΩ・SPKO 125mΩ、合計 **720mΩ** あった。
#   アンプの先は 8Ω なので、出力の **8.3%** が板の銅で熱になる。
#   電流の上限は 5V の BTL に 8Ω で 5÷8÷√2 で 0.44A rms。信号線の桁ではない。
SPK_NETS = {"SPKP", "SPKM", "SPKO"}
SPK_W = 0.6
...
_KELVIN = []      # build() が入れる
_PRE_VIAS = []    # 先に引いた線が層を替える所の穴。build() が入れる（net, (図面 X, 図面 Y)）
_n = [0]
...
MOUNT = list(VB.HOLES) + list(VB.GUIDE_HOLES)   # M2 × 4（床から立つ柱 φ7 の上）＋ 軸のガイドの足 3（2026-09-19）。足形・銅の逃げ・枠からの押し出し・ナットの下の検査が全部掛かる
...
SHAFT_CX, SHAFT_CY = VB.SHAFT[0] + VB.SHAFT[2] / 2, VB.SHAFT[1] + VB.SHAFT[3] / 2   # つまみの軸の芯＝ AS5600（U4）の芯
...
def xb(X, Y):
    """図面の座標 → 板の座標（bx() の逆）。pad_box() の返り値を板の座標で扱うときに使う。"""
    return (X - ORG[0], BOARD_W - (Y - ORG[1]))
...
# ⭐ 2026-09-19: v6 の板の並び（PLACE_V6）と、それを v6.1 へ写す pwr() / v61() を消した。
#   電源部（POWER_V61）・リレーの駆動（RELAY_V61）・口（BOX_PLACE）がすべて v6.1 の板の上で並べ直され、
#   写しから来る部品が 1 つも残らなくなったため。中身は git の e096847 以前にある。
...
#   ⚠ 細いパッドは spoke が 1 本しか立たず KiCad が starved_thermal を出す。
#     規則を緩めるのは gen_sch.py が書く .kicad_pro の min_resolved_spokes（1 にしてある）。
#     🔴 カスタム規則のファイル（.kicad_dru）に書いても **kicad-cli は読まない**。
#       わざと壊した規則を入れても素通りした（2026-09-17 夕に確かめた）。
HAND_SOLDER = {"J1", "J2", "J4", "J5", "J6", "J7", "J10", "SW2"}   # SW2 = 電源の押しボタン（スルーホール・2026-09-19）
...
BOX_PLACE = {
    ...
    "J7": (PH_V2, 0) + _fb("リード"),      # 縦 2 ピン（上から挿す）。⭐ 2026-09-19 トグルをやめて 4 ピン → 2 ピン
    ...
    "J10": (PH_V2, 270) + _fb("電池"),     # 縦 2 ピン（板の裏・口は下 ＝ 電池の側）
    ...
    # ミュートリレー（背 9.33）
    "K31": ("Relay_SMD:Relay_DPDT_Omron_G6S-2F", 180) + VB.RELAY[:2]
           + (VB.RELAY[0] + VB.RELAY[2], VB.RELAY[1] + VB.RELAY[3]),
}
# ======== 電源部の配置（板の座標・2026-09-16）========
# ⭐ ここまでの電源部は **二重の写し**だった: 旧 PowerBoost 基板 → v6 の板 → v6.1 の板（写しの関数は 2026-09-19 に消した）。
...
POWER_V61 = {
    # 並べ方: 小さい部品は列に揃える。⭐ 2026-09-19 🔒 ユーザー「板単体で見たときにも美しいと思える配置に」:
    #   X の間隔は **枠が重ならない 3.5**（0805 の枠は幅 3.36〜3.40。前の 3.0 では relax() が押し離して列が崩れていた）。
    #   Y の列は 8.4（出力の C）/ 10.7（FB・PROG・THERM）/ 14.0（主の道）/ 19.0（パスコン）/ 23.0（分圧）。
    #   **relax() が 1 つも動かさない狙い**にしてある。動いたら build() が ⚠ で名前を出す
    # 主の道だけ Y 14.0 の一直線（U2 は下の辺から 5 本逃がすので 1.5 上げてある）。
    # ---- 主の道（右から左へ流れる）----
    "U1":  (22.0, 14.3, 180),   # 昇圧 TPS61090（180°: SW と V5 を L1 の側＝右へ）
    "L1":  (27.5, 14.0, 180),   # インダクタ（180°: 2 番 SW を U1 の側＝左へ）
    "U2":  (33.5, 15.3, 0),     # 充電 MCP73871（下の辺 6〜10 番の逃げ道を空けるため 1.3 上）
    # ⭐ 2026-09-17 夕: 39.5 → 40.5。U2 の右の辺と R41 のあいだの谷が 1.595mm しか無く、
    #   VLIPO 0.6 ＋ PROG1 0.2 ＋ PROG3 0.2 ＋ すきま 0.15 × 4 ＝ 1.6 に 0.005 足りなかった。
    #   そのせいで VLIPO か PROG1 のどちらかが必ず浮いた。Q2 も一緒に右へ 1.0 送るので
    #   電池の道の長さはちゃら（R41→U2 が ＋1.0・J10→Q2 が −1.0）。
    "R41": (40.5, 14.0, 180),   # シャント 10mΩ（180°: 1 番 BATP を Q2 の側＝右へ）
    "Q2":  (45.0, 14.0, 0),     # 逆接保護 P-ch（3 番 ドレインが +X ＝ J10 の側）
    "U3":  (40.5, 19.0, 270),   # INA226 — R41 の真上（ケルビン。X を R41 と揃える）
    # ---- 前の列 Y 5.0（U2 の下の辺 6〜10 番へ行く物と CC）----
    # ⭐ 2026-09-19: CC の 5.1k を **J13 の足元**へ（前は前の列 Y 5.0 で J13 から 31〜35mm）。
    #   🔴 同じ轍の 2 回目: v6 でも「CC の引き下げは USB-C の足元」と書いたのに（v6 の並びの R44 の注）、
    #   v6.1 の並べ直しで前の列へ戻っていた。縦で 1 番（CC）が上 ＝ J13 の側。
    #   J13 の真ん中の足（D+・D−）は使っていないので、その真下に 2 本並べる。
    #   X 39.9 より左へ寄せない: VBUS の幹（vbus_trunk）が A9（X 38.56）から真下へ降りて Y 8.1 で裏へ潜る
    # ⭐ 2026-09-20: 胴どうしが 0.85mm しか空いていなかったので R44 を右へ 0.75（1.60mm へ）。
    #   R45 は上の注のとおり X 39.9 より左へ寄せられないので、開けるのは右側だけ
    "R45": (39.9, 27.8, 270), "R44": (42.75, 27.8, 270),  # CC2（B5 の下）・CC1（A5 の下）
    # ⭐ 2026-09-19: THERM は U2 の左下の 5 番の下へ（前は Y 5.0 の列で 9.4mm）。180° で 2 番（THERM）が左
    "R15": (32.75, 10.7, 180),                          # THERM（5 番）
    "R46": (44.0, 8.1, 0),                              # Q2 のゲート（J2 のライザーの枠 Y 6.54 の後ろ）。⭐ 2026-09-20: 7.6 → 8.1（J2 の枠まで 0.94 → 1.44）
    # ---- Y 9.5 / 11.0 ----
    ...
    # ⭐ 2026-09-17 夕: FB の 2 本（R3・R4）を U1 の下へ入れるために 9.5 → 8.6 へ 0.9 下げた。
    #   U1 の下のパッドは Y 11.625 から。C6（1206）の上の端が 10.400 だったので
    #   あいだは 1.225 しか無く、0805 のパッド 1.45 ＋ すきま 0.15 × 2 ＝ 1.75 が入らない。
    # ⚠ 2026-09-17 夕: C2 を 180° にして V5 のパッドを U1 の 15・16 番の真下へ向けてみたが、
    #   自動配線の答えが悪くなった（U1→C6 2.3 → 14.0mΩ・U1→XIAO 20.7 → 29.8mΩ、
    #   穴どうしの間隔の警告も 1 件）ので戻した。向きは 0° のまま。
    "C2": (21.8, 8.4, 0), "C6": (25.8, 8.4, 0), "C9": (30.4, 8.4, 0),
    # ⭐ 2026-09-17 夕: FB の分圧。**U1 の 14 番の真下**に置く。
    #   前は Y 23.0 の列（U1 から 12.4 / 14.3mm）にあり、FB の線はのべ 17.42mm、
    #   うち 13.21mm が内層 In2.Cu で、その斜めの区間が L1 の角と SW の線の真下を
    #   くぐっていた（x 24.73 で FB が y 14.21・SW が y 14.33）。R3 1.87M・R4 200k と
    #   高いので、この節点は拾いやすい。PowerBoost はこの 2 本を IC の真横に置いている。
    #   180° なので R4 は 1 番（FB）が右・R3 は 2 番（FB）が左。FB の 2 つのパッドが
    #   となり合い、その上 1.4mm に U1 の 14 番が来る。遠い端は R4 が GND・R3 が V5（C6 の側）。
    # ⚠ 2026-09-17 夕: 21.0 / 24.1 だと 2 つの FB のパッドの間が **0.22mm** しか無く、
    #   U1 の V5（15・16 番・X 22.3〜23.0）が真下へ降りられずに浮いた。
    #   左右に 0.6 ずつ離して隙間を 1.42mm にする（0.8mm の V5 ＋ すきま 0.15×2 が入る）。
    "R4": (20.6, 10.7, 180), "R3": (24.7, 10.7, 180),
    # ⚠ X 35.0 だと U2 の 9 番の逃げ道（escape_stub）が R16 の GND パッドに 0.01mm まで寄る
    "R16": (36.25, 10.7, 0), "R17": (39.75, 10.7, 0),    # PROG1 / PROG3（U2 の右の辺 11〜15 番）
    # ---- Y 19.0（パスコン）----
    "C1": (21.8, 19.0, 0), "C4": (25.3, 19.0, 0),       # VSYS
    # ⭐ 2026-09-19: C7（VLIPO・10µF）は **U2 の 16 番の右上**（前の C41 の場所）。1 番（VLIPO）が左。
    #   前は (28.0, 19.0) で、U2 の VLIPO まで裏 7.5mm（引き直したら内層 11.4mm・21mΩ）だった。
    #   C6・C9 と同じ轍（電源のコンデンサが IC から遠い）。1 番から U2 の 15 番へは c7_link() で先に引く。
    #   ⚠ 縦にして 16 番の真上へ寄せる案は、枠（courtyard）が U2・C41 と重なって押し離しで流された
    "C7": (36.4, 19.1, 0),
    # ⭐ 2026-09-17 夜: C8（VBUS）は **縦（90°）で U2 の 17〜19 番の真上**。1 番が下（U2 の側）。
    #   前は (31.0, 19.0, 0) の横向きで、U2 の入力まで 0.2mm × 6.6mm の枝が残っていた（vbus_trunk()）。
    #   X 31.0 は空きのまま。C41 は C8 の枠に押されないよう 34.0 → 36.4 へ。
    "C8": (33.5, 19.7, 90),
    # ⭐ 2026-09-19: C41（U3 の VS のパスコン）は C7 に場所を譲り、**U3 の 1 番（V33）・2 番（GND）の真上**
    #   （前の R7 の場所）へ。180° で 1 番（V33）が右＝U3 の 1 番の側。U3 の V33 まで 2mm 前後（前は 3.6mm）
    "C41": (40.8, 23.5, 180),
    # I2C の引き上げ。⭐ 2026-09-19: 前は R41 の右（Y 19.0）。リレーの駆動に場所を譲り、XIAO（J1）の SDA・SCL の右上へ
    # ⭐ 2026-09-20: 胴どうし 0.85 → 1.45mm（間隔 2.1 → 2.7）
    "R42": (17.0, 15.2, 0), "R43": (17.0, 17.9, 0),
    # ---- Y 23.0（分圧と大きいコンデンサ）----
    # R1・R2（低電池の分圧）は 2026-09-20 に廃止（gen_sch.py の U1 の注）。この列は空いた
    # R3・R4（FB）は 2026-09-17 夕に U1 の下へ移した。X 28.0 と 31.0 は空きのまま
    # ⭐ 2026-09-17 夜: 23.0 → 23.5。真下に縦の C8 が来て、その 2 番の GND の穴（cap_gnd_vias()）が
    #   R13 の 1 番まで 0.054 しか無かった。上の Q1 の枠までは 1.35 あったので 0.5 もらう
    "R13": (34.0, 23.5, 0),                             # EN
    # EN の跳ね止め。⭐ 2026-09-19: v6 の写し（押し離しで板 43.3, 21.7 へ流れていた）から、EN の口（SW2 の 3 番・板 28.55, 32.4）の右脇へ。
    #   SW2 の前（下）は口の名前「SW2 POWER」の場所（SILK_PORT を S にそろえた）。270° で 1 番（EN）が上 ＝ SW2 の 3 番と同じ高さ
    "C32": (31.2, 31.4, 270),
    # ⭐ 2026-09-19: VPCC の分圧は U2 の 2 番（左の辺）の左上、L1 の上へ（前は 9〜13mm）。
    #   どちらも VPCC の側を右（U2 の側）へ向ける: R6 は 0°（2 番 VPCC が右）・R7 は 180°（1 番 VPCC が右）
    "R6": (29.1, 20.1, 0), "R7": (29.1, 17.8, 180),    # VPCC（X 29.1: 枠が U2 の枠（X 30.9 から）に掛かると U2 が押されて C8 の真下から外れる）

    # 🔒 2026-09-19 ユーザー「LED は本来不要」: Y 27.0 の列（LED1・LED2・R5・R20・Q1）と
    #    Y 5.0 の列の LED3・LED4・R14・R8 を外した（gen_sch.py の U1 の下）
}
FIXED_V61 = {
    # つまみ（磁石と軸がこの真上に降りる）。AS5600 は芯で置く
    # ⭐ 2026-09-19: C42（AS5600 のパスコン）を U4 の左の足（2 番 V33・4 番 GND）の横へ（前は 6.7mm）。
    #   270° で 1 番（V33）が上。U4 の芯の左 4.9: 磁石の軸の枠（芯の左 3.5 から）の外
    #   ⭐ 2026-09-19 夕: 数字の直書きをやめ、軸（VB.SHAFT）の芯から引く（つまみを左へ 2.0 動かしたとき一緒に付いてくるように）
    "U4": (SHAFT_CX, SHAFT_CY, 0), "C42": (SHAFT_CX - 4.9, SHAFT_CY - 0.3, 270),
    # 電源の押しボタン。位置は v61_board.py の PWR_BTN（押し子が壁を抜ける所から決まる）
    "SW2": VB.PWR_BTN,
}
# ⭐ 2026-09-19 🔒 ユーザー「板単体で見たときにも美しいと思える配置に」（機能ごとに島・電気が先）:
#   ミュートリレーの駆動を **K31 のコイルの足元**へ集める。前は v6 の写しのまま板の左（X 15〜18）にあり、
#   コイル（K31 の 1 番 V5・12 番 COL ＝ 板 X 55.83 / 47.83・Y 21.65）から 35mm 離れていた。
#   帯は K31 の枠の前（Y 20.3）から、軸のガイドの左の穴 (51.7, 13.8) の頭までの Y 16.6〜20.3。
#   D31 はコイルの 2 本の足のあいだに渡す（1 番 V5 が右）。Q31 は 3 番 COL を上（12 番の側）へ。
RELAY_V61 = {
    "Q31": (47.9, 18.31, 90),
    "D31": (52.8, 18.8, 180),
    "C31": (56.9, 18.8, 0),                             # リレーの V5 のパスコン（1 番 V5 が左）
    # ⭐ 2026-09-20: 胴どうし 0.75 → 1.55mm（間隔 2.0 → 2.8）
    "R31": (44.2, 17.1, 0), "R32": (44.2, 19.9, 180),   # ベースの直列と引き下げ（どちらも BASE の足が右 ＝ Q31 の 1 番の側）
}
FIXED_V61.update(POWER_V61)
FIXED_V61.update(RELAY_V61)
...
HEIGHT = {
    ...
    "J7":   (6.0,  "📄 ePH.pdf 3 ページ Top entry（B2B-PH-K の胴。嵌合は 8.0）⭐ 2026-09-19 に 4 ピン → 2 ピン"),
    "SW2":  (5.8,  "📄 XKB5858-Z-E 図面（前の面 5.80±0.10 角・押し子の芯は板の上 2.9）"),
...
HEIGHT_DEFAULT = (1.45, "⚠ 一般値・0805 の抵抗とコンデンサはこれ以下")
...
CHECK_PADS = {
    ...
    ("J1", "1"): (1.770, 12.105), ("J1", "7"): (17.010, 12.105),   # ⭐ 2026-09-19 +0.30（v61_board.py の RISER_XIAO）
    ...
    #    角度 0 で 3 番が +X 側（45.940）・1/2 番が −X 側（44.060）に並ぶ。J10（X 75.7）は Q2 の
    #    +X 側なので、電池は 3 番（ドレイン）から入る。ネットの対応（3=BATRAW・2=BATP・1=BATG）は check_sch.py が見る
    #    ⭐ 2026-09-17 夜: X を 1.0 送った。同日の夕に Q2 を 44.0 → 45.0 へ動かした（POWER_V61）のに
    #      この表が古いままで、生成のたびに「パッドの位置が違う Q2」が 3 行出ていた（向きは合っていた）。
    ("Q2", "1"): (44.060, 14.950), ("Q2", "2"): (44.060, 13.050), ("Q2", "3"): (45.940, 14.000),
...
# ⭐ 2026-09-19 🔒 ユーザー「口の下（前の縁の側）」: 口の名前は **S にそろえる**。
#   例外は 2 つ: J5（真下が J1 の枠）と、板の裏の J10（S に極性の ＋ − が座る）
SILK_PORT = {
    "J1":  ("XIAO",     "S"),
    "J2":  ("OLED",     "S"),
    "J4":  ("SPK IN",   "S"),   # W は U5 の枠に当たる。S は口の前の帯を外した下側
    "J5":  ("SPK OUT",  "N"),   # S は J1 の枠（J5 は J1 の真上に隙間 0.075 で座る）
...
SILK_TITLE = ["Katanori ver6.1n", "2026-09-20"]   # 裏から見て右上。日付は板を直したら書き換える
...
def v5_trunk(placed):
    """V5 の幹（U1 の足元 → XIAO のライザー）を **裏の面**で通す（2026-09-17 夕）。

    J1 の 1 番（V5）は板の左の端 X 1.77 にあり、U1 から 21mm ある。表で西へ抜ける道は
    U1 の下の辺（パッドの下端 Y 11.625）と V5 の出力コンデンサの列のあいだ 1.4mm の
    廊下しか無く、そこへ FB の R3・R4 を入れた。自動配線は **内層 In2** へ逃げたが、
    JLCPCB の 4 層は内層が 0.5oz なので同じ幅でも抵抗が 2 倍になり、U1 → XIAO が
    16.9 → 64.9mΩ に増えた（0.3A で 5.1 → 19.5mV）。
    裏は J10 しか居らず空いている。ここを通せば 15mΩ 前後で、しかも毎回同じ道になる。
    ⚠ 座標を書き写さない。両端はパッドの中心から取る。
    """
    # ⚠ 起点は **C6**。C2 から出すと U1 → C2 が 12.3mΩあり（R4 が間に入っている）
    #   幹の手前で損する。U1 → C6 は 2.3mΩで、C2 は逆にこの幹から館うことになる。
    cx, cy, _, _ = pad_box(placed, "C6", "1")       # U1 の足元の V5（出力コンデンサの V5 側）
    jx, jy, _, _ = pad_box(placed, "J1", "1")       # XIAO へ渡す V5。貫通なので裏に出ている
    c, j = xb(cx, cy), xb(jx, jy)
    y = c[1] - 1.39                                  # 板の手前へ。C2 のパッドの下の縁の 0.665 下
    k = 1.73                                         # 45° で板の左の端へ寄せる量
    tracks = [("V5", "F.Cu", V5_W, [bx(c[0], c[1]), bx(c[0], y)]),
              ("V5", "B.Cu", V5_W, [bx(c[0], y), bx(j[0] + k, y), bx(j[0], y + k), bx(j[0], j[1])])]
    return tracks, [("V5", bx(c[0], y))]


def bat_trunk(placed):
    """電池の生の線（J10 → 逆接保護 Q2）を **裏の面**で通す（2026-09-17 夕）。

    自動配線はこれを **内層 In2** に 34mm 載せていた。0.5oz なので 56.6mΩ ＝ 充電 1A で
    57mV。⚠ 同じ日の朝に「31.16mm・25.5mΩ（B.Cu）」と書いたのは私の誤りで、層を見ずに
    長さだけで計算していた。裏は J10 しか居らず空いているので、そこを通す。
    J10 は裏の部品・Q2 は表なので、Q2 の手前で 1 つだけ穴を開ける。

    🔒 ユーザー 2026-09-17 夕「数ミリΩをきにするのに、実働に影響が出そうな場所に線を
       通そうとするのはどうなんだろうな」。最初は J10 から Q2 へ Y 14.95 の一直線で引き、
       AS5600（U4・板の X 63.75〜67.65・Y 11.35〜16.25）の**真裏**を通していた。
       磁気の見積もりでは 0.063°（分解能 0.088° の 1 段未満）だが、これは磁石の 50mT と
       戻りの打ち消しを**仮に置いた**計算で、測った値ではない。
       ⇒ 前（Y 9.5）の帯へ下ろして U4 から離す。代償は **+4.6mΩ**（23.9 → 28.5mΩ）で、
         これは見えない。見えない物を払って、測っていない仮定の上のリスクを消す。
    """
    jx, jy, _, _ = pad_box(placed, "J10", "2")
    qx, qy, qw, _ = pad_box(placed, "Q2", "3")
    j, q = xb(jx, jy), xb(qx, qy)
    vx = q[0] + qw / 2 + 0.82        # Q2 の 3 番のパッドの外。φ0.6 の穴とすきまが入る
    # ⭐ 2026-09-17 夕（2 度目の引き直し）: 前は Y 9.5 の帯を西へ走らせていたが、AS5600 の胴の
    #   下端 11.35 から **1.85mm** しかなく、ユーザーに「AS5600 付近に極太線通すのやめませんか」
    #   と言われた。U4 のまわりの立入禁止（build() の ko）の外を回る。
    #     lane_x … 立入禁止の東の縁（U4 の芯の右 5.5）の 0.8 外。つまみを左へ 2.0 動かす前は 72.0 で J4 の足（X 73.7 から）の 1.4 手前
    #     lane_y  6.5 … U4 の胴の下端から 4.85 下。取付穴 (79.354, 3.5) の逃げ 5.1 より上
    lane_x, lane_y = SHAFT_CX + 6.3, 6.5
    y0 = j[1] - 1.45                 # J10 の足から下りる所。ここから 45°
    ky = y0 - (j[0] - lane_x)        # 45° で lane_x に着く Y。J4 の足の上端 10.425 を越える高さ
    tracks = [("BATRAW", "B.Cu", BAT_W, [bx(j[0], j[1]), bx(j[0], y0),
                                         bx(lane_x, ky), bx(lane_x, lane_y),
                                         bx(vx + (q[1] - lane_y), lane_y), bx(vx, q[1])]),
              ("BATRAW", "F.Cu", BAT_W, [bx(vx, q[1]), bx(q[0], q[1])])]
    return tracks, [("BATRAW", bx(vx, q[1]))]


def vbus_trunk(placed):
    """USB-C から充電 IC の入口（C8 と U2 の 17〜19 番）までを太い道で通す。

    VBUS は幅を決める組に入っていない（U2 の QFN-20 の 6 本から 0.4mm が逃げられないため）。
    そのもとで自動配線が幹も 0.2mm で内層へ逃がし、J13 → U2 が **102.5mΩ**、
    うち In2.Cu 0.2mm × 17.6mm が 87mΩ だった（2026-09-17 夕）。
    MCP73871 はリニア充電なので VBUS には **充電電流 ＋ 機体の電流**が乗る。
    実測からの見積もりで 約 0.78A、R16 1.0kΩ の設計上限で 約 1.22A。
    このファイルの幅の表は 0.2mm で外層 1oz 許容 0.74A で、**内層はその半分 約 0.37A**。

    ⭐ 2026-09-17 夜: **最後の枝も太くした。**前は幹（J13 → C8）だけ 0.6mm で、C8 → U2 の
      18・19 番は「3mm の枝」のつもりで自動配線に任せていた。板を測ると実際は
      B.Cu 0.2mm × 3.3mm → 穴 → F.Cu 0.2mm × 2.7mm ＋ ピンの足 0.66 の **6.6mm・16mΩ**で、
      太くした幹（15.2mm・12.5mΩ）より大きかった。原因は線幅ではなく**配置**:
        ・C8 が横向きで U2 の左上に居て、2 番（GND）の下端と U2 のピン列のあいだが 0.66mm
        ・C8 の 2 番が乗る表の GND の島に穴が無く、GND の 0.2mm 線が C8 の 1 番の真下を横切る
      ⇒ C8 を**縦（90°）**にして、1 番を U2 の 17・18・19 番（3 本とも VBUS で隣どうし）の
        真上に置く。1 番からピンへは VBUS_PIN_W の線をまっすぐ降ろす。隣の 20 番（VSYS）と
        16 番（VLIPO）までは 0.275 ずつ残る（線 1.2 の縁 ±0.6・ピンの縁 ±0.875）。
        幹は 1 番の**左の横**で表へ上がる（真上は 2 番 ＝ GND が居る）。
    SEL・PROG2・TE への枝は無電流なので自動配線に任せたまま。
    """
    ax, ay, _, ah = pad_box(placed, "J13", "A9")     # USB-C の VBUS（左側の 1 組）
    cx, cy, cw, ch = pad_box(placed, "C8", "1")      # 充電 IC の入口のコンデンサ
    px, py, _, ph = pad_box(placed, "U2", "18")      # 充電 IC の入力（17・18・19 番の真ん中）
    a = xb(ax, ay); c = xb(cx, cy); u = xb(px, py)
    if abs(c[0] - u[0]) > 0.05 or c[1] < u[1]:
        raise SystemExit("C8 の 1 番が U2 の 18 番の真上に居ない: C8.1 (%.3f, %.3f)・U2.18 (%.3f, %.3f)。"
                         "押し離しで動いたなら POWER_V61 の C8 を直すこと" % (c + u))
    y1 = a[1] - 1.7          # J13 のパッドの下。USB-C の殻の足（板 Y 30.87〜32.97）を避ける
    vx = c[0] - cw / 2 - 0.15 - VIA4[0] / 2 - 0.1    # C8 の 1 番の左の横。穴 φ0.6 ＋ すきま 0.15 ＋ 余り 0.1
    k = a[0] - vx            # 45° で左へ寄せる量（X の差）
    # ピンへ降ろす線の下の端。丸い端（半径 VBUS_PIN_W / 2）が U2 の EP（GND）へ寄らないよう、
    # 線の芯はピンの外の端から 0.15 だけ中で止める（丸い端はピンの長さ 0.82 の中に収まる）
    y_pin = u[1] + ph / 2 - 0.15
    tracks = [("VBUS", "F.Cu", BAT_W, [bx(a[0], a[1]), bx(a[0], y1)]),
              ("VBUS", "B.Cu", BAT_W, [bx(a[0], y1), bx(vx, y1 - k), bx(vx, c[1])]),
              ("VBUS", "F.Cu", BAT_W, [bx(vx, c[1]), bx(c[0], c[1])]),
              ("VBUS", "F.Cu", VBUS_PIN_W, [bx(c[0], c[1]), bx(u[0], y_pin)])]
    return tracks, [("VBUS", bx(a[0], y1)), ("VBUS", bx(vx, c[1]))]


# 電源のコンデンサの GND を、パッドのすぐ横で GND の面（In1）へ落とす（2026-09-17 夜）。
#   (部品, パッド, 穴を置く向き ＝ 板の座標の (dx, dy)・Y は上向き)
#   塗ったベタで測ると、最寄りの GND の穴まで C6 3.7mm・C9 2.6mm・C1 3.6mm、
#   C8 は同じ島に穴が 1 つも無く、GND を 0.2mm の線で C7 まで引いていた。
#   📄 TPS61090 データシート 12.1: 主電流とパワー GND の道は短く太く・入力と出力の
#      コンデンサは IC のできるだけ近く。In1 は表の 0.1mm 下なので、穴 1 つで戻りの輪が閉じる。
#   向きは「相手のパッドの反対」ではなく**空いている側**で選んである（C6 の右は C9 の V5 で 0.55 しか無い）。
#   ⚠ C8 は上（R13 の側）。左は裏を降りてくる VBUS の幹（vbus_trunk()）の真上で、置くと短絡する
#     （一度そう置いた。check_pre() はそのとき足した）。右は C41 の V33 まで 0.1 しか無い。
#   ⚠ C2（2.2µF）は入れていない。下は裏の V5 の幹（v5_trunk()）の真上・上は R4 の FB まで 0.18・
#     右は C6 の V5 まで 0.15 で、置ける側が無い。隣の C6 の穴（C2 の 2 番から 4.9mm）が受け持つ。
CAP_GND_VIAS = [("C8", "2", (0, 1)), ("C6", "2", (0, -1)), ("C9", "2", (1, 0)),
                ("C1", "2", (0, 1)), ("C7", "2", (0, 1)), ("C41", "2", (0, 1)),
                ("R44", "2", (0, -1)), ("R45", "2", (0, -1))]
CAP_GND_W = 0.5      # パッドから穴までの線。パッドの短い辺 1.0 より細く


def cap_gnd_vias(placed):
    """CAP_GND_VIAS の穴と、パッドから穴までの短い線を作る。→ (線, 穴)"""
    others = []          # GND でないパッドの箱（図面の座標）。穴が寄りすぎていないかを見る
    for fp in placed:
        ref = next((str(e[2]) for e in find(fp, "property") if str(e[1]) == "Reference"), None)
        for pad in find(fp, "pad"):
            net = find1(pad, "net")
            if net and str(net[-1]).strip('"') == "GND":
                continue
            X, Y, w, h = pad_box(placed, ref, str(pad[1])) if ref else (0, 0, 0, 0)
            others.append((ref, str(pad[1]), X - w / 2, Y - h / 2, X + w / 2, Y + h / 2))
    tracks, vias = [], []
    for ref, num, (dx, dy) in CAP_GND_VIAS:
        X, Y, w, h = pad_box(placed, ref, num)
        p = xb(X, Y)
        reach = (w / 2 if dx else h / 2) + 0.2 + VIA4[0] / 2      # パッドの縁から穴の縁まで 0.2（C8 の上は R13 まで 1.07 しか無い）
        v = (p[0] + dx * reach, p[1] + dy * reach)
        VX, VY = bx(*v)
        for oref, onum, x0, y0, x1, y1 in others:
            gap = math.hypot(max(x0 - VX, 0, VX - x1), max(y0 - VY, 0, VY - y1)) - VIA4[0] / 2
            if gap < 0.2:
                raise SystemExit("GND の穴 %s.%s が %s.%s に %.3f まで寄る。CAP_GND_VIAS の向きを直すこと"
                                 % (ref, num, oref, onum, gap))
        tracks.append(("GND", "F.Cu", CAP_GND_W, [bx(*p), (VX, VY)]))
        vias.append(("GND", (VX, VY)))
    return tracks, vias


def check_pre(tracks, vias):
    """先に引いた穴が、別のネットの先に引いた線に乗っていないか（貫通穴なので層は問わない）。

    どちらも protect で渡すので自動配線は直さず、DRC まで行って初めて短絡と出る。ここで止める。
    """
    bad = []
    for vnet, (VX, VY) in vias:
        for net, lay, w, pts in tracks:
            ...
            for (x1, y1), (x2, y2) in zip(pts, pts[1:]):
                dx, dy = x2 - x1, y2 - y1
                L = dx * dx + dy * dy
                t = 0.0 if L == 0 else max(0.0, min(1.0, ((VX - x1) * dx + (VY - y1) * dy) / L))
                gap = math.hypot(VX - (x1 + t * dx), VY - (y1 + t * dy)) - VIA4[0] / 2 - w / 2
                if gap < 0.15:
                    bad.append("%s の穴 (%.3f, %.3f) ↔ %s の線（%s）: すきま %.3f"
                               % (vnet, *xb(VX, VY), net, lay, gap))
    if bad:
        raise SystemExit("先に引いた穴と線が当たる:" + NL + "  " + (NL + "  ").join(bad))


def pre_vias(nets):
    """_PRE_VIAS（先に引いた線が層を替える所）を板の要素にする。"""
    return [["via", ["locked", "yes"], ["at", f"{X:.4f}", f"{Y:.4f}"],
             ["size", f"{VIA4[0]}"], ["drill", f"{VIA4[1]}"],
             ["layers", Str("F.Cu"), Str("B.Cu")],
             ["net", str(nets[net])], ["uuid", Str(uid())]]
            for net, (X, Y) in _PRE_VIAS]


def vlipo_link(placed):
    """充電 IC U2 の 15 番とシャント R41 の 2 番を、表で直に結ぶ（2026-09-17 夕）。

    ここは U2 の右の辺（PROG1・PROG3）と R41・U3 に挙まれた幅 2.6mm の谷で、
    自動配線は引けたり引けなかったりする。R3・R4 を動かしたときに引けなくなり、
    PROG1 のビアが斜めの区間の 0.771mm 横に来ていた（要るのは 0.75）。
    電池の道は幅も道もこちらで決めておきたいので、先に引いて protect で守る。
    PROG1 は信号なので、よける側に回す。
    ⚠ 座標を書き写さない。両端はパッドの中心から取る。
    """
    ax, ay, _, _ = pad_box(placed, "R41", "2")
    bx_, by_, _, _ = pad_box(placed, "U2", "15")
    # 45° で寄せる量。両側から挑む。
    #   小さい→ 斜めの区間が U3 の下のパッドの列（6 番 V33・下端 Y 16.17）に寄る。
    #     0.9 だと 0.488（要るのは 0.3 ＋ 0.15 ＋ パッドの半分 0.15 ＝ 0.6）。
    #   大きい→ PROG1 の逃げ道の端（U2 の 13 番から 1.612）に寄る。1.5 で 1.054（要るのは 0.575）。
    k = 1.5
    sx = 1.0 if bx_ > ax else -1.0
    sy = 1.0 if by_ > ay else -1.0
    pts = [(ax, ay), (ax, by_ - sy * k), (ax + sx * k, by_), (bx_, by_)]
    return ("VLIPO", "F.Cu", BAT_W, [(round(x, 3), round(y, 3)) for x, y in pts])


def c7_link(placed):
    """C7 の 1 番（VLIPO）から、U2 の 15 番（VLIPO）へまっすぐ下ろす（2026-09-19）。

    線は C7 の 1 番の中心の X で縦に下り、15 番の高さで止まる（15 番・14 番は同じ VLIPO なので重なってよい）。
    横にある 16 番（VLIPO）・17 番（VBUS）とのすきまを見る。17 番まで 0.15 無ければ止める。
    ⚠ 座標を書き写さない。両端はパッドの中心から取る。
    """
    ...
    if gap < 0.15:
        raise SystemExit("c7_link: U2 の 17 番（VBUS）まで %.3f しか無い" % gap)
    return ("VLIPO", "F.Cu", BAT_W, [(round(cx, 3), round(cy, 3)), (round(cx, 3), round(uy, 3))])


def u2_16_link(placed):
    """U2 の 16 番（VLIPO・上の辺の右端）から、右の c7_link の線へまっすぐ出す（2026-09-19）。

    c7_link を先に引くと、16 番の右は 0.5mm の隙間になり、自動配線が入れずに 16 番だけ浮いた。
    幅はパッドの幅（0.25）に合わせる。下の 15 番（VLIPO）の上の縁まで 0.4 以上ある。
    """
    px, py, _, _ = pad_box(placed, "U2", "16")
    cx, _, _, _ = pad_box(placed, "C7", "1")
    return ("VLIPO", "F.Cu", 0.25, [(round(px, 3), round(py, 3)), (round(cx, 3), round(py, 3))])
...
FIXED = {"J13", "J1", "J14", "U4", "SW1", "J2", "J4", "J5", "J6", "J10", "SW2"}
...
def build():
    ...
    before = dict(boxes)
    relax(placed, boxes)
    # ⭐ 2026-09-19: 列に揃えた狙いが押し離しで崩れたら名前を出す（崩れたまま 18 個が気づかれずにいた）
    pushed = [f"{r}（{b[0] - before[r][0]:+.2f}, {-(b[1] - before[r][1]):+.2f}）" for r, b in boxes
              if abs(b[0] - before[r][0]) > 0.005 or abs(b[1] - before[r][1]) > 0.005]
    print("  ⚠ 押し離しで狙いから動いた: " + "・".join(pushed) if pushed else "  押し離しで動いた部品は無い（狙いの列のまま）")
    out_of_board = []
    lim = (ORG[0], ORG[1], ORG[0] + BOARD_L, ORG[1] + BOARD_W)
    for ref, b in boxes:
    ...
    _KELVIN = (kelvin_tracks(placed) + [escape_stub(placed, "U2", "13", "PROG1")]
               + [vlipo_link(placed), c7_link(placed), u2_16_link(placed)])
    # 長い幹は裏へ。自動配線に任せると内層（0.5oz）へ逃げて抵抗が倍になる
    _PRE_VIAS = []
    for tr, vi in (v5_trunk(placed), bat_trunk(placed), vbus_trunk(placed), cap_gnd_vias(placed)):
        _KELVIN += tr
        _PRE_VIAS += vi
    check_pre(PRE_TRACKS + _KELVIN, _PRE_VIAS)
    doc += (outline() + mounting_holes() + placed + gnd_zone() + vbus_zone() + silk(placed, boxes)
            + pre_tracks(nets) + ep_vias(placed, nets) + pre_vias(nets))
    ...
    for mx, my in MOUNT:                                    # 取付穴（φ2.2 ＋ 逃げ 0.5）
        ...
        r = MOUNT_D / 2 + MOUNT_KEEP
        ko.append((X - r, Y - r, X + r, Y + r))
    # 🔒 ユーザー 2026-09-17「AS5600 付近に極太線通すのやめませんかね」
    #   幅を決める組（電池 0.6・V5 0.8・スピーカー 0.6）を足すたびに、自動配線が
    #   ここへ**大きい電流の線**を呼び込んでいた。測って安心するのをやめ、**置けなくする**。
    #   ⚠ 危ないのは「太い」からではない。線が作る磁場は B = µ0·I / 2πr で、
    #     **電流と距離**で決まり幅にはほぼ依らない（0.44A・3mm なら幅 0.2 でも 0.6 でも 29µT）。
    #     幅の組を足したことが変えたのは配線の選び方で、悪くなったのは距離の方
    #     （レビュー側の指摘・2026-09-17）。だから距離で守るのが正しい。
    #   実測（防ぐ前）: SPKP 0.6mm が胴の **真下**・SPKM 1.39mm・BATRAW 1.85mm。
    #   スピーカーは 0.44A rms の**交流**なので、電池の直流より性質が悪い。
    #   ⚠ 表（F.Cu）は塞がない。U4 の SDA・SCL・V33 が出られなくなる。
    #     表に残るのは U4 自身の細い線だけで、太い線は内層と裏から逐う。
    kx0, kx1, ky0, ky1 = SHAFT_CX - 5.5, SHAFT_CX + 5.5, SHAFT_CY - 6.0, SHAFT_CY + 6.0   # U4 の胴（芯 ±1.95 × ±2.45）のまわり。芯は VB.SHAFT から
    KX, KY = bx(kx0, ky1)
    KX2, KY2 = bx(kx1, ky0)
    ko.append((KX, KY, KX2, KY2, ("In2.Cu", "B.Cu")))
    ...
    for v in ep_vias(placed, nets) + pre_vias(nets):
        at = find1(v, "at")
