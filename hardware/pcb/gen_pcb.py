...
HEIGHT = {
    ...
    "J1":   (2.5,  "🔒 2.54 ヘッダの規格の姿（樹脂 2.5・ピン 6.0・先は板の面から 8.5）"),
    "J2":   (2.5,  "🔒 同上（1x04）"),   # 🔴 出どころの頭が 📄 か 🔒 でないと、gen_case_parts が 3D モデル（ピン込みの 8.65）で上書きする
...
CHECK_PADS = {
    ...
    #    ⭐⭐ 2026-09-16 夕に **Y 並びへ戻した**（🔒 ユーザー「J10の極性も手前奥に戻して」）。
    #    J4 を縦にして、板の Y 14.30〜16.30 を塞いでいたプラグの帯が消えたので、6.9 の枠が入るようになった。
    #    パッドは板の座標 X 75.700（枠 X 73.50〜79.00 の芯 76.25 から −0.55。B2B の足形はパッドが
    #    courtyard の芯に無い）・Y は 2.0 間隔で 14.950 と 16.950。
    #    🔴 1 番 GND が**奥**（Y 16.950）・2 番 VBAT ＋ が**手前**（Y 14.950）。PowerBoost B1 と同じ言い方に戻った
