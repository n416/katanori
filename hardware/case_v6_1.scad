...
PCB_PARTS = [
    ["J4  SPK IN",  72.350, 78.250,  6.700, 14.300, PCB_PLUG_H,    1,  1, PH_MATE_OUT, PCB_TH_LEG, [73.700, 76.900,  7.175,  8.925]],
    ["J5  SPK OUT",  5.700, 13.300, 14.150, 20.050, PCB_PLUG_H,   -3,  1, PH_MATE_OUT, PCB_TH_LEG, [11.350, 12.550, 15.225, 18.975]],   // 🔒 基板担当 2026-09-14: 抜く向きは −X（左の縁）。ライザーの裏の L 字の胴に 26.21mm³ 食い込んでいたので後ろへ 1.9 逃がしてある
    ["J6  BTN2",    14.550, 20.450, 23.200, 30.800, PCB_PLUG_H,   -1,  1, PH_MATE_OUT, PCB_TH_LEG, [15.900, 19.100, 28.575, 30.325]],   // 🔒 ユーザー 2026-09-14「会話ボタンを基板に付ける必要はないと言ったんであって、線は必要でしょ」: 会話ボタンは天板のまま・板は線を受ける口だけ持つ
    ["J7  REED+TGL",58.140, 68.450, 30.850, 35.760, PCB_PLUG_V_H,  2,  1, PH_MATE_OUT, PCB_TH_LEG, [59.900, 67.100, 32.775, 34.525]],   // 🔒 縦の B4B-PH-K（横出しはハッチに当たった）。通り道は上へ
    // ⭐ 2026-09-16 夕 🔒 ユーザー「ケーシング作ったから垂直のがいい。結局一番穴は小さくなる」:
    //    サイド型 S2B-PH-K（背 4.8・横出し）→ **トップ型 B2B-PH-K を板の裏に下向き**。基板側も同日に差し替えた（docs/PCB-V61.md 2 章）。
    //    旧 x 1.70〜10.30 / y 22.10〜29.00 は板の左端で、電池が −X へ抜ける道の真上にぶら下がっていた。
    //    右の帯（板 X 66.05 より右）は電池が一度も通らない。天板の窓は 12.7 × 7.9 → 8.11 × 5.91（面積 100.3 → 47.9mm²）。
    //    🔴🔴 置き場所を決めたのは「J10 は裏でも**はんだ足 2 本は表へ突き抜ける**」こと。足は
    //       ① J4 の胴（板 y 〜14.30）と ② **J4 に挿さったプラグ**（y 14.30〜16.30）の外に出さないといけない。
    //       ②を見落として 1 度置き、この scad の hit_wires が 1.17mm³ で見つけた（基板側の検査は表と裏を
    //       切り分けた所で足を見ていなかった。同日に gen_pcb.py へ足の判定を入れた）。
    //       その縛りで Y 方向に 6.9 が入らなくなり、パッドを X へ並べ替えた（極性は左が ＋・右が GND）。
    //    隔たり: 足は J4 のプラグから 0.60・枠は USB-C のシェルの足（板 y 20.63）から 0.655・右の壁の内面 84.354 まで 2.544
    ["J10 BAT",     73.500, 79.810, 14.975, 19.885, PCB_PLUG_V_H, -2, -1, PH_MATE_OUT, PCB_TH_LEG, [74.850, 78.050, 16.900, 18.650]],
    // ⭐ 2026-09-16 夕: リレーを**胴と足**に分けた。旧 15.3 × 10.7 × 9.33 はデータシートの「リードを含む外形」で、
    //    背 9.33 の**胴は 14.8 × 7.4**（KiCad の F.Fab）。Y で片側 1.65 ずつ、蓋との隙間を実物より食っていた。リードは板の面すれすれを這う
    ["K31 RELAY",   56.550, 71.350, 19.410, 26.810, 9.33,          0,  1, 0, 0, []],
    ["K31 LEADS",   56.300, 71.600, 17.800, 28.500, 0.5,           0,  1, 0, 0, []],
    ["J1  RISER XIAO", 0.500, 18.280, 10.535, 13.075, RISER_SOCK_H, 0, 1, 0, RISER_LEG, [0.920, 17.860, 10.955, 12.655]],   // 1x07 のオス。2026-09-14: 縦のメス（8.5）→ 背の低いオス（2.5）＋ ライザー裏面の L 字。⚠ Y は 8.935 → 11.805 へ後ろへ 2.87（L 字の胴のぶん）
    ["J2  RISER OLED", 35.970, 46.130, 3.500, 6.040, RISER_SOCK_H, 0, 1, 0, RISER_LEG, [36.390, 45.710, 3.920, 5.620]],   // 1x04 のオス・同上。⚠ Y は 1.9 → 4.77 へ後ろへ 2.87
];
...
module pcb61() color("#2b6b3f") difference() {
    ...
    translate([LW_X - 1, IN_Y - RAIL_W_R, HUB_AT[2] - 1]) cube([RAIL_T + 1, PCB_W, 1.6 + 2]);   // 🔴 後ろのレールの逃げは **X 1.694〜2.894 の 1.2 だけ**。後ろの下のレール（X 〜8.8）は Z 12 で止まるので板に当たらない。当たるのは背の高い方（RAIL_T 1.2・Z_TOP まで）だけ。最初 7.8 幅で切って板の角を大きく欠いていた
...
HUB_HOLES_W = [[HUB_AT[0] + 4.0, PCB_Y0 + 3.5], [74.5, PCB_Y0 + 3.5], [4.834, 47.4942], [81.354, 48.4]];   // 🔒 ユーザー 2026-09-16 夕「いっそ切り欠きなくして柱と同じ位置にすれば？」「レジンでのハッチ止めと、ナイロンでの左フラップ止めの穴の位置は合わせる必要がある」:
...
//   前 2 点はそのまま（🔒 ユーザー「前は今のままでいい」。レジンの前の柱は Y 3.25 で板の前縁 13.5 より外）
...
function hub_nut_ang(i) = (i == 1) ? 135 : (i == 3) ? -135 : 0;   // 左の 2 本は +X（0°）のまま。右前は −X+Y・右後ろは −X−Y
module hub_screw_cuts() for (i = [0 : 3]) if (!(nylon() && i == 2)) let (h = HUB_HOLES_W[i], zc = HUB_AT[2] - HUB_NUT_SKIN - NUT_T) translate([h[0], h[1], 0]) rotate([0, 0, hub_nut_ang(i)]) {   // ⭐ 2026-09-16 夕: ナイロンで飛ばしていた左前（i == 0）を立てる。溝の口は −X（仕切りの左の面 X 2.5）へ。蓋の左の板の内面 1.694 が 0.8 先で塞ぐ
...
// 🔴 skin() は v5 からの残りで v6.1 に定義が無く、part="skin" / part="all" が黙って空になっていた（2026-09-16 夕に発見）
module skin() { shell(); if (nylon()) lid(); else p_top(); }   // 箱の皆（底パーツ＋蓋）
