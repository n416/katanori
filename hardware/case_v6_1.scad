...
PCB_Y0  = nylon() ? 13.1 : 13.5;    // ⭐ 2026-09-17 🔒 ユーザー「ナイロンだけ基板を 0.4 手前へ」。箱は変えない（下の PCB_HATCH_GAP と対で効く）   // 🔒 ユーザー 2026-09-14「PCB の奥行きを小さくして ReSpeaker とぶつからないように」: 前縁 10.235 → 13.5
PCB_Y1  = PCB_Y0 + 37.4;            // PCB の後縁。ナイロン 50.5 / レジン 50.9（板の奥行きは 37.4 で同じ）
...
PCB_HATCH_GAP = nylon() ? 0.9 : 0.5;   // 板の後縁とハッチの内面の逃げ。PCB_Y0 の −0.4 と対で、**ハッチは動かず板だけ 0.4 前へ**寄る（IN_Y はどちらの材料でも 51.4）
