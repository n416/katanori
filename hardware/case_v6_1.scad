...
RISER_SOCK_H = 2.5;      // 板の上に出るオスのヘッダの樹脂。⚠ 2026-09-14 基板担当と確認: 樹脂 2.5・ピン 6.0・先は板の面から 8.5 は**どちらも一般値で、データシートで取った数ではない**。型番を決めるときに取り直す
...
// 🔒 出どころ（基板担当 2026-09-14）: Sullins の .100" Female Headers カタログ
//    https://sullinscorp.com/catalogs/101_PAGE114-115_.100_FEMALE_HDR.pdf
//    1 列の右角（LGB）: 胴 .335"[8.50] × .100"[2.54]・足 .124"[3.15]。
//    KiCad の PinSocket_1x07_P2.54mm_Horizontal（足跡の外形 10.03 ＝ 胴 8.50 ＋ 足の曲がり 1.53）と同じ系列。
//    ⇒ 胴は板の面から 2.54 の厚みで寝ていて、接点の軸はその真ん中 1.27。**J1・J2 の +2.87 はこれで裏が取れた**
//    ⚠ まだ仮定が 2 つ: 「胴が板に密着して寝ている」「接点が厚みの真ん中」。断面図は読んでいない
...
function lsock_axis_y(back_y) = back_y + LSOCK_T / 2;   // 受けるオスのピンの芯の Y ＝ 裏面 ＋ 胴の半分
...
module ina_bar(hole = INA_BAR_HOLE_D, peg = INA_BAR_PEG_D, mark = false, vent = INA_BAR_VENT_D, side = 0)
    ...
    for (h = [a, b]) translate([h[0], h[1], INA_BAR_Z0 - 0.01]) {
        ...
        if (side > 0) let (dx = (h == a) ? 1 : -1)   // 横穴: 凹みの壁から足の側面へ。向きはもう片方の足の側（🔒 ユーザー 2026-09-10 の図）。高さ 0.8〜1.5（凹みの天井 1.6 の 0.1 下で止めて膜を作らない）
            translate([dx > 0 ? 0 : -(INA_BAR_FOOT_D / 2 + 1), -side / 2, 0.8]) cube([INA_BAR_FOOT_D / 2 + 1, side, INA_BAR_POCKET - 0.1 - 0.8]);
    }
...
module print_inabar_d() translate([0, 0, -INA_BAR_Z0]) ina_bar(hole = 3.35, peg = 2.9, side = 1.5);   // D: B に幅 1.5 の横穴（上面とダボは触らない）
...
if (part == "plugs") plugs();
if (part == "bridge") { bridge(); panel_ribs("bridge"); brg_front(); straps(); one("bat"); }
if (part == "skin")   skin();
