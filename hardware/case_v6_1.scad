...
// 蓋は**たわむ**ので 1 つの剛体では書けない。⇒ **2 つの剛体に分けて別々に当てる**（2026-09-17）。
//   ① lidmain = 天板＋天板に付く物（真下へ降りるだけ）
//   ② lidflap = 左の L 字（板＋座）。降りているあいだ外へ FLAP_FLEX 1.5 出ていて、
//      LID_REST まで降りた所で内へ戻り、口が XIAO の殻に嵌まる
//   2 つは同じ蓋の部品なので互いには当てない（たわんだ姿勢でも常に繋がっている）。
//   ⚠ 最後の LID_REST(0.3) → 0（ねじで締める分）はここでは見ない。静止の当たり sk_* が見る
PATH_LIDMAIN = [[0, 0, 14, 0, 0, 0], [0, 0, LID_REST, 0, 0, 0]];
PATH_LIDFLAP = [[-FLAP_FLEX, 0, 14, 0, 0, 0], [-FLAP_FLEX, 0, LID_REST, 0, 0, 0], [0, 0, LID_REST, 0, 0, 0]];
