...
RSP_Z   = 2.5 + 5.0;                // 🔒 ユーザー 2026-09-14「ReSpeaker の Z 位置を 5mm 上に」: 7.5（v5 は 2.5）
...
BAT_CASE = 6.0;   // 🔒 ユーザー 2026-09-14「間を取って 3 で」→「じゃあ +4 で」: 板を 4.0 上げる（2.0 → 6.0）。板の裏 Z 12.0・床まで 12.0 で、電池の口のトップ型（嵌合 8.0）に余裕 4.0
...
PCB_PARTS = [
    ...
    ["J10 BAT",      4.00, 10.90, 23.00, 28.50, 6.0,        2, -1],   // 🔒 ユーザー 2026-09-14「基板を上げればいい。線を回すよりはいい」: 板を 3.0 上げてトップ型（B2B-PH-K）へ戻す。板の裏 11.0・嵌合 8.0 で床まで 3.0 残る。⚠ 枠は基板担当の裏向きの版（後で取り直し）
...
module oriser61() {
    ...
    zb = HUB_AT[2] + 1.6; zt = oriser_z() + 1.27 + 1.0;
...
POST_T_H_F = 5.0;     // 前の上の柱の高さ。2026-09-14: ReSpeaker を 5 上げたので裾も 5 上げる（裾 Z 42.25 = 板の頭 41.51 の 0.74 上）。ナットの溝（頭から 3.4）は入る
