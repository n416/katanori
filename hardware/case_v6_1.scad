...
RSP_Z   = 2.5 + 5.0 + 2.6;          // 🔒 ユーザー 2026-09-14「ReSpeaker の Z 位置を 5mm 上に」: 7.5 →「垂直にする場合どれくらい上げるのか・引っかかる物を後ろに下げたら解決するのか」→「そうしてください」: 10.1（+2.6）。XIAO の下のメスの下端を、板に立てるソケットの頭 18.1 より上へ出すのに 2.49 要る
...
ROW_Y   = 17.3 + 5.3;               // 🔒 ユーザー 2026-09-14「ボタンもスピーカーも下げられると踏んでる」: スピーカーを 5.3 後ろへ（22.6）。ReSpeaker を 2.6 上げた板の頭 44.11 が、バスタブの底 42.75 に入ったため。バスタブの前縁が板の裏 10.035 を越える量
ROW_RX  = 64.7;                     // 右の列の X 中央（会話ボタン・つまみ・トグル）。🔒 ユーザー 2026-09-08 夜「つまみと、トグルスイッチは右ですよ」: 入れ替えで動くのはスピーカーと会話ボタンだけ。値はスピーカーの居た 63.2 + 1.5
SPK_AT  = [24.0, ROW_Y];   // 🔒 ユーザー 2026-09-14「スピーカーとボタンを X 24 で揃える」: 19 → 24。天板でボタンと 1 本の縦の列になる。ボタンの中心が置けるのは X 23.04〜32.85 だけ（左は箱の左後ろの柱 7.694・右はつまみの台座 48.2・ボタンの頭まわり 30.7）で、その帯に入る合わせ先はスピーカーしか無かった   // 以下は v5 の経緯:             // 🔒 ユーザー 2026-09-08 夜「会話ボタンとスピーカーの位置入れ替えたらどうでしょうか。元々プロダクトとしてはそっちが正解」: 会話ボタンの居た X 19.0 へ（OLED の線 4 本の出口が無い・横の空きが無い、の 2 つを一度に解く）。それまで 63.2 + 1.5（🔒 ユーザー 2026-09-05「スピーカーも右に 1.5mm」（つまみの台座と一緒）。§2「SPK4 X 63.204」。2026-09-08 「右へ 3」を試したが、右のバスタブ 3 の壁とねじの頭が右の壁に 1.15 / 2.0 入るので戻した（右側の積み上げ 7.6 に対して余裕 8.15）
...
PCB_PARTS = [
    ...
    ["J1  RISER XIAO", 0.505, 18.275, 7.665, 10.205, 8.5,     0,  1],   // 🔒 基板担当: 1x07 のメスソケット（胴 17.78 × 2.54・背 8.5）。ライザーはこの上に挿さる
    ["J2  RISER OLED", 36.160, 46.320, 0.630, 3.170, 8.5,     0,  1],   // 🔒 同: 1x04（胴 10.16 × 2.54・背 8.5）
];
...
RISER_SOCK_H = 8.5;      // 🔒 同: メスソケットの背。**ライザーの板の下端はこの頭に乗る**（2026-09-14 まで板の面に置いていて、挿さらない形を「当たり 0」と報告していた）
...
module riser61() {
    ...
    for (r = [0, 1]) let (q = xiao_row_pts(r), xa = min(q[0][0], q[1][0]) - 1.27, w = abs(q[1][0] - q[0][0]) + 2.54)
        color("#333") difference() {                                                                  // メス（前向き・中は空でピンが入る）
...
module oriser61() {
    ...
    color("#333") difference() {                                                                      // メス（前向き・中は空）
...
POST_D_F_T = 7.0;     // 前の上の柱の奥行き（Y 1.0〜8.0）。2026-09-14: ReSpeaker を 2.6 上げたら板の頭が柱の裾に入ったので、板の前面 8.185 の手前で止める（8.0 → 7.0）。ナット（二面幅 4.2 を Y に）の前後に肉 1.4
