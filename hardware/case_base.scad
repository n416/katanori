...
HOUS_H = dupont_h();   // DuPont ハウジングの高さ（parts.scad DUPONT_H = 14）。
                 // 🔴 2026-09-05 ここは 10.0 で「✅ ユーザー実測」と書いてあった。ユーザー「ピンヘッダは 10、ハウジングは 14」で、
                 //    10 はヘッダの値。実機では OLED の線のハウジングが会話ボタンの腕に当たって天板が組めなかった。
HOUS_R = 4.5;    // ✅ 2026-08-25 ユーザー実測: ハブ基板の**部品面から一番高いところまで 17mm**（＝挿した DuPont の
                 //    線が曲がっている所）。内訳は ヘッダの樹脂 2.5 ＋ ハウジング 10.0 ＋ 曲がり **4.5**。
                 //    🔴 2026-09-05 ハウジングが 14 になったので、この内訳は 2.5 ＋ 14 ＋ 4.5 ＝ 21 になり、実測の 17 と合わない。
                 //    17 が正なら曲がりは 0.5 しか残らない（線の太さより薄い）。**どちらが正かは未決**（CASE-V4-OPEN.md）。4.5 は据え置き。
...
BACK_CL  = 10.1;                                   // 🔒 2026-09-05 ユーザー「筐体の Y を広げてもよい」: 8.1 → 10.1。電池の蓋の床の板（別部品）とハッチの縁が電源線（Y 70.7 直書き・4 車線）と同じ Y に居て、のりしろが下の帯にしか置けなかった。2.0 広げるとハッチ側の肉（IN_Y − 2.2 から）が線の後ろ 0.3 を越える。旧: 8.1 // 🔴 2026-08-23 v2 の値に戻す（ユーザー「Y 5mm を稼ぐ」）。初版は HOUS_H + HOUS_R = 13.6（後列の口が立ち上がる分・⬜未実測）で奥行きが v2 より 5.5 増えていた
...
// 窓のベベル（外面）。🔄 2026-09-03 hull → 輪切りの積み上げ。
// 🔴 2026-09-04 32 枚では**画面で段が見える**（ユーザー指摘・フロントパネルの窓の縁）。
//    輪郭が凸でないので hull() は使えない（逃げの台形が埋まる）。⇒ 枚数で潰す。
WIN_BEV_N = 24;    // 斜面の分割数。段ではなく傾いた面になるので少なくてよい
module win_chamfer_cut() {
    win_bev_slab(WIN_CH, FY_OUT, 0.51);   // 外面より外へ 0.5（切り口を面で切るため）
    p = bev_pts(WIN_CH, WIN_R, WIN_BEV_N);
    // 🔴 2026-09-04 板を積むのをやめ、隣どうしを hull() で繋いで**斜面**にする。
    //    輪郭は win_rrect（角丸長方形）と win_bulge（台形の逃げ）の和で、どちらも単体では凸。
    //    太らせる操作は和に分配できる（offset(g)(A∪B) = offset(g)(A) ∪ offset(g)(B)）ので、
    //    部品ごとに分けてから hull すれば逃げは埋まらない。全体を hull すると埋まる（上の注意書き）。
    // 🔴 2026-09-04 板を積むのをやめ、隣どうしを hull() で繋いで**斜面**にする。
    //    輪郭は win_rrect（角丸長方形）と win_bulge（台形の逃げ）の和で、どちらも単体では凸。
    //    太らせる操作は和に分配できる（offset(g)(A∪B) = offset(g)(A) ∪ offset(g)(B)）ので、
    //    部品ごとに分けてから hull すれば逃げは埋まらない。全体を hull すると埋まる。
    //    ⚠ 区間どうしが面で接するので非多様体の辺が 16 本残る。試した 3 案の比較:
    //        元（32 枚の階段）  三角 17738 / 面積ゼロ 0   / 閉じ OK      / 体積差  0       / 段が見える
    //        この斜面            三角  9280 / 面積ゼロ 8   / 閉じ NG(16)  / 体積差 -0.0002  / 段なし
    //        斜面＋重ね          三角 16478 / 面積ゼロ 4   / 閉じ NG(4)   / 体積差 +1.3998  / 段なし
    //        minkowski          三角  5194 / 面積ゼロ 124 / 閉じ NG(294) / 体積差 -0.0013  / 段なし
    //    ⇒ 体積が正確で段の無いこれを採った。**NG(16) は未解決**
    for (i = [0 : len(p) - 2], k = [0, 1])
        hull() for (j = [i, i + 1])
            translate([0, FY_OUT + p[j][0] + 0.01, 0]) rotate([90, 0, 0])
                linear_extrude(0.005) win_piece(k, p[j][1]);
}
module win_piece(k, g) offset(r = g, $fn = 24) { if (k == 0) win_rrect(0); else win_bulge(); }
