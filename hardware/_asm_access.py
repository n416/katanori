...
def run():
    ...
    for x in (10.002, 76.102):
        ...
        # 🔒 2026-08-26 T-2 の直し: ナットは**天面の小組（箱の外）**で入れる。
        #    箱に載せた後だと右は 1.3mm しか無い（すぐ後ろがつまみの島）。裏返せば左右とも開いている。
        #    ナットは 4.3 幅 × 1.6 厚なので、通るかどうかは厚みの側で決まる（φ2.0 で撃つ）。
        for stl, lbl, st_no in ((topsub, '天面の小組（箱の外）', '10'), (st10, '天面を載せた後', '11')):
            d = probe(stl, (x, 4.6 + 2.0 + 0.01, 46.1), 1, +1, 2.0)
            row('L の後ろへナット (X %.1f)・%s' % (x, lbl), st_no, 'ナット 4.3×1.6', 2.0, 8.0, d,
                '🔴 右は箱から出しても 1.3mm。塞いでいるのは天面のつまみの島で、外しようが無い')
