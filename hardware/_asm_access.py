...
def run():
    ...
    for x in (10.002, 76.102):
        ...
        # 🔴 2026-08-26 訂正 2 つ。① L の厚みは OLED_L_T = 2.0（3.2 と読み違えて 1.2mm 奥から撃っていた）
        #    ② ナットは 4.3 幅 × **1.6 厚**。丸い φ4.3 で撃つと上下にも 4.3 要ることになり、
        #       天面の継ぎ目の斜面（Z 47.1）を叩いて「両側とも 0.2mm」という嘘の値が出ていた。
        #       厚みの側で決まるので φ2.0 で撃つ。
        d = probe(st10, (x, 4.6 + 2.0 + 0.01, 46.1), 1, +1, 2.0)
        row('同 L の後ろへナット (X %.1f)' % x, '11', 'ナット 4.3×1.6', 2.0, 8.0, d,
            '後ろから +Y へ。厚みの 1.6 が通れば入る')
