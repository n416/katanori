...
FINGER = 12.0     # 指の腹
HOUS_R = 4.5      # 📄 case_base.scad と同じ。挿した口の上に取ってある「線が曲がり切るまで」の逃げの径
# OLED の L の裏の面（Y）。OLED の基板の面 4.6 ＋ L の厚み OLED_L_T。ナットはこの面から後ろへ差す。
#   🔴 2026-08-29 ここは撃ち始めを `4.6 + 2.0` と、**L の古い厚みで**書いてあった。
#     2026-08-28 に case_base.scad の OLED_L_T を 2.0 → 3.6 にして STL を焼き直したので、
#     撃ち始めが**ナットのポケットの中**（6.4〜8.2）へ入り、ポケットの口までの 1.59mm だけを測って
#     4 行とも「🔴 止まる」に化けていた。正しい面から撃つと 左 4.561・右 2.853（要る 1.6）で通る。
#   ⚠ case_base.scad の OLED_L_T を動かしたら、この数字も一緒に動かすこと。
OLED_L_BACK_Y = 4.6 + 3.6
...
def run():
    ...
    for x in (10.002, 76.102):
        ...
        for stl, lbl, st_no in ((topsub, '天面の小組（箱の外）', '10'), (st10, '天面を載せた後', '11')):
            ...
            d = probe(stl, (x, OLED_L_BACK_Y + 0.01, 46.1), 1, +1, NUT_M2)
    ...
    # 🆕 2026-08-28（16 度目の机上の通し）**手順 9 の中の順番**の裏取り。
    #   st9r は線を 10 束とも障害物から外しているので、「先に通した束」が見えない。
    #   ST="w9_oled" ＝ OLED 自身の束だけを抜いた姿（ほかの束は通してある）で撃つと、
    #   電流計 → PowerBoost の 2 本（Z 40.8〜48.0・X 11.1〜38.8）が OLED の口の上に来る。
    #   ⚠ 線は剛体の管で描いてあるので「組めない」ではない。**OLED を先に挿す理由**の裏取り。
    d = probe(stage('w9_oled'), (43.05, 20.70 + 0.01, 46.50), 1, +1, 10.8)
    row('（反例）OLED の 4 連を 電源の 7 本の後から挿す', '9', '口そのもの', 10.8, PINL, d,
        '相手は手順 9 の姿から OLED 自身の束だけを抜いたもの。止まるのが正 ＝ '
        '**手順 9 の中では OLED が先**（電流計 → PowerBoost の 2 本が口の上を渡る）', cex=True)
    ...
    # 🔴 2026-08-28（16 度目の机上の通し）: ここは相手が **st2 ＝ 口が 1 本も挿さっていない姿**だった。
    #    手順 3 は 10 本を順に挿す手順なので、最後の 1 本を挿すときには**残り 9 本が既に立っている**。
    #    その 9 本を障害物に入れないまま撃っていたので、20 行とも「外まで通る」が出ていた。
    #    15 度目に E リングで見つけたのと同じ形の穴（相手が居なければ当たらない）。
    #    ＋ 向きも直した。真上へ撃つと出発点が口の頭に重なり、隣のハウジングの**下の面**を
    #    跨いでしまう（eps で捨てられる）。道具は上から降りて来るので、上から降ろす向きで測る。
    PORT_Z = 50.0
    ports = hub_ports()
    xs = {nm: (x, y) for nm, x, y, z, top, w0, w1 in ports}
    for nm, x, y, z, top, wl0, wl1 in ports:
        world = stage('nb_%s' % nm)          # その口以外の 9 本を挿した姿
        need = PORT_Z - top - 0.1   # 0.1 は口の頭ちょうどで止まる丸めの見込み（J2・充電の口と同じ書き方）
        near = min([abs(x - bx) for bn, (bx, by) in xs.items() if bn != nm and abs(y - by) < 2.0] or [999])
        d = probe(world, (x, y, PORT_Z), 2, -1, TWEEZ)
        row('ハブの口 %s へ真上から (%.1f, %.1f)' % (nm, x, y), '3', 'ピンセット', TWEEZ, need, d,
            '口の頭 Z %.1f まで %.1f 要る。🆕 2026-08-28: 相手は**その口以外の 9 本を挿した姿**'
            '（前は口が 1 本も無い姿へ撃っていた）' % (top, need))
        d = probe(world, (x, y, PORT_Z), 2, -1, FINGER)
        note = ('同じ相手へ φ12。同じ列の隣の口まで %.1fmm' % near) if near < 900 else '同じ相手へ φ12'
        if d < need:
            row('同 指 φ12 で %s' % nm, '3', '指 φ12', FINGER, need, d,
                note + '。**止まるのが正 ＝ この口はピンセット**。隣の口の線の逃げ（半径 %.2f）を'
                       'よけて φ12（半径 6.0）を降ろすには %.2fmm の間隔が要る' % (HOUS_R / 2, 6.0 + HOUS_R / 2),
                cex=True)
        else:
            row('同 指 φ12 で %s' % nm, '3', '指 φ12', FINGER, need, d, note)
    for nm, p, ax, sg, dia, note in CONN:
        q = list(p); q[ax] += sg * 0.01
    ...
    for sx, nm in ((-1, '左'), (1, '右')):
        ...
        # 🆕 2026-08-28: つまみ系の簡素化（柱 φ8/φ3.6 の 2 段 → 上から下まで φ3.5）で、
        #   柱のあいだが φ9.1 → φ13.2 に広がり、**指 φ12 も口まで届く**ようになった
        #   （13.6 から 13.2 に減ったのは、同日足した基板の上限の段 φ4.0 のぶん）。
        #   それまではここが反例の行（止まるのが正）で、本文を「押さえるのはピンセット」にしていた。
        row('同 指で押さえる %s' % nm, '10', '指 φ12', FINGER, need, d,
            '🆕 2026-08-28 に通るようになった（柱が φ3.5 の棒になり、空きが φ9.1 → φ%.1f）。'
            '⚠ ナットは二面幅 4.3 の小片なので、確実なのは<b>ピンセット</b>' % wid)
