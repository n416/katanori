...
def run():
    ...
    for x in (10.002, 76.102):
        ...
        for stl, lbl, st_no in ((topsub, '天面の小組（箱の外）', '10'), (st10, '天面を載せた後', '11')):
            ...
            # 要る mm は**ナットの厚み 1.6**。初版の 8.0 は根拠の無い仮値だった（2026-08-26 に直した）。
            row('L の後ろへナット (X %.1f)・%s' % (x, lbl), st_no, 'ナット 4.3×1.6', 2.0, 1.6, d,
                '🔒 2026-08-26 つまみとスピーカーを内側（−X）へ 4 動かして 1.3 → 15.7（T-2 の直し）')
