...
//   🔒 ユーザー「コピーして使って」: v5 の形を変えないために分けた。違うのは S_NUT_ANG だけ（57 → 20）。
//   v5 側を直したら、ここへ手で持って来ること。
...
S_NUT_ANG = 20;                              // 🔒 ユーザー 2026-09-14（v6.1）「小判がた同心円上でナット位置を移動・2 つのナット位置は点対称をキープ」: 57 → 20。57 だとナットを差し込む道が ReSpeaker の板の頭（v6.1 で 5.0 上げた）に 8.49mm³ 入って組めない。20 で 0。点対称は S_HANDS が持つので角度だけで保たれる   // 🔒 ユーザー 2026-09-09「手は小判の同心円みたいな形になっていい」→「やっておくか」: 手・ナット・ねじ・壁を、端の半円の芯（x ±S_SEAT_CX）から S_NUT_ANG 度倒した放射方向に置く。0 なら真横（前の形）。57 は OLED の板の後ろの縁（世界 y 4.40）にねじの頭が 0.40 で並ぶ角度（60 だと 0.12 しか空かない）
