...
//
// 🔒 ユーザー 2026-09-08 案: 左右（±X）の天井から薄い板を下ろす。板の中央の台が胴の裏に当たり、座との間に挟む（クランプ）。
//   ⭐ 2026-09-18 に**横ねじをやめて上向き M2 へ**（下の「吊り」を見ること）。
//   それまでは バスタブの ±X に壁を立て、壁の外から横に M2×4 を入れて板の中の**立った六角**のナットへ入れていた
//   （🔒 ユーザー 2026-09-08「六角ポケットを縦にして、横からねじ止め」）。
//   やめた理由: ナイロンの 2 分割では蓋に左の板が付いたままなので、左のドライバーが板を貫かないと届かない（φ6 で 47.59mm³・φ5.5 で 39.99）。
//   S_NUT_ANG を振っても板が空く角（±90）では会話ボタンか天板が塞ぐ。右も 2026-09-16 に 57 → 20 へ戻した時から 12.15mm³ 塞がっていた
...
//   ⚠ ②③ は**横ねじの頃の話**（2026-09-18 に上向き M2 へ変えた）。①と台の高さはそのまま生きている
...
S_HEX_AF = 2.2; S_HEX_WALL = nylon() ? 1.0 : 0.6;   // ハニカム: 六角の二面幅 2.2・穴どうしの肉 0.6（2026-09-05）。ナイロン 2026-09-15: 1.0（MJF の最薄 0.8・壁 1.2。0.6 は `_stl_preflight --mat nylon` で 🔴 48 か所）
...
// ---- 吊り ----
// ⭐ 2026-09-18 横ねじをやめて **上向きのねじ** にした（🔒 ユーザー「②でいいと思う」）。
//   ねじが縦になるとナットは**寝る**ので、板の厚みを食うのがナットの厚み 1.8 から
//   二面幅 4.5 に変わる。板 2.8 には入らないので、板の下端に **ナットを抱く羽根**を外へ張り出す。
//   口は **座の側（半径の内向き）**（🔒 ユーザー 2026-09-18「ナットの入口を内側に作る必要があるよね」）。
//   ナットはスピーカーより先に内側から差し、入れたスピーカーの胴（逃げ 0.3）が口を塞ぐ。
//   ⇒ 横ねじの頭を出すためだけに在った**バスタブの壁（半径 10.8〜12.3・高さ 5.25）は消える**。
//     ケーシングは厚くならず、手の所の肉はむしろ減る。
S_PLATE_T = 1.8 + 1.0; S_PLAY = 0.2;         // 板の厚み 2.8（据え置き。ナットは下の羽根が持つ）（🔒 ユーザー 2026-09-09「ポケットの内壁は縁と一体化して無くして」）（🔒 ユーザー 2026-09-08「天板からの手を内側に曲げるのをやめる」: 足は無し）
...
// 手を倒す角。🔒 ユーザー 2026-09-09「手は小判の同心円みたいな形になっていい」: 手・ナット・ねじを、
//   端の半円の芯（x ±S_SEAT_CX）から S_NUT_ANG 度倒した放射方向に置く。0 なら真横。点対称は S_HANDS が持つ。
// ⭐ 2026-09-18: **この角はもうドライバーの道に効かない**（ねじが縦になったため）。効くのは置き場所だけ。
//   20 / 57 を行き来していたのは横ねじの頭とドライバーの道の話で、経緯は git（2026-09-14 4590848・2026-09-16 夕）にある。
// 🔒 ユーザー 2026-09-16 夕「ちょっと２０にしてみてくれる？」: 20。
S_NUT_ANG = 20;
...
// 寝たナットの部屋: 内の面（＝口）は座の輪郭 7.8、外は二面幅 4.5 ぶん out、その外に MJF の肉 0.8
S_NUT_FLR  = 1.0;                            // ナットを載せる床。⚠ ここが M2×4 の長さを決める: 頭の座（板の裏から 0.8）→ バスタブの板の残り 1.2 → この床 1.0 → ナット 1.8 ＝ 4.0 で、先がナットの上面とツライチになる。0.8 にすると先が 0.2 出て、穴の無い板を突く（会話ボタンの B3_BLK_FLR は 0.8）
S_NUT_ROOF = 0.8;                            // ナットの上に残す肉
S_BOSS_R1 = S_R_I + S_NUT_AF + 0.8;          // 羽根の外の面 13.1（ナイロン）/ 12.9（レジン）
S_SCR_R   = S_R_I + S_NUT_AF / 2;            // ねじの芯の半径 10.05 / 9.95（＝ ナットの芯）
// ⚠ 幅は**吊りの板とバスタブの腕で別**にする。
//   吊りの板: 寝たナットの対角 5.196 が接線方向に来るので、両脇の肉 0.70（旧 6.60）では MJF の 0.8 を割る。
//     7.0 で 0.902（会話ボタンのブロック B3_BLK_W と同じ数）。
//   バスタブの腕: 通すのは頭のザグリ φ4.2 だけなので旧の幅のままでよい。
//     7.0 に広げると右の腕の角が ReSpeaker の押さえ（羊羹 X 32.5〜）に 0.165mm³ 乗る（旧 0.019）。
//     ケーシングを太らせない方を採る
S_ARM_W = 7.0;                               // 吊りの板・羽根の幅（接線方向）
S_TUB_ARM_W = S_NUT_AF / cos(30) + 2 * S_SKIN;   // バスタブの腕の幅 6.60（ナイロン）/ 6.37（レジン）＝ 旧 S_ARM_W
...
// ⚠ 板の高さは**旧の立った六角ポケットの寸法をそのまま残す**（5.9 ナイロン / 5.7 レジン）。
//   下げる理由が無いのに動かすと、バスタブの板・底・クランプの台の Z が全部ずれる
S_PLATE_H = 2 * S_SKIN + S_NUT_AF;
Z_FOOT_B = Z_IN - S_PLATE_H;                 // 板の下端 −8.4（ナイロン）/ −8.2（レジン）
Z_NUT_B  = Z_FOOT_B + S_NUT_FLR;             // 寝たナットの下面（床の上）
S_BOSS_H = S_NUT_FLR + S_NUT_T + S_NUT_ROOF; // 羽根の高さ 3.4（床 0.8 ＋ ナット 1.8 ＋ 天井 0.8）
S_TUB_T = 2.0; Z_TUB_T = Z_FOOT_B; Z_TUB_B = Z_TUB_T - S_TUB_T;   // バスタブ 3 の板 −8.4〜−10.4（底は世界 42.55）
S_R_ARM = S_R_O + S_PLAY + 1.5;              // バスタブの腕の外の半径 12.3（旧 S_R_W1 ＝ 壁の外の面と同じ値のまま据え置き）。⚠ 吊りの羽根（13.1）に合わせて 13.1 へ伸ばすと、左の腕が蓋の左の板に 0.125mm³・右の腕が ReSpeaker の押さえに 0.788mm³ 乗る。羽根は天板側なので top_inner_trim が内面で切るが、腕はバスタブ（別部品）なので切られない
...
// ⭐ 2026-09-18 上向き M2×4。頭は**バスタブの板の裏**のザグリの中（板 2.0 のうち 0.8 掘って 1.2 残す ＝ MJF の壁ちょうど。頭 1.3 なので 0.5 下へ出るが、板の裏 42.55 の下は ReSpeaker の 36.58 まで 5.97 空いている）。
//   軸は 板の残り 1.2 → 羽根の床 0.8 → ナット 1.8。先はナットの上面の 0.2 先
S_CB_D = 4.2; S_CB_H = 0.8;                  // 頭のザグリ（径は会話ボタンの B3_CB_D と同じ）
S_SCR_LEN = 4.0;
...
// 吊りの板 ＋ ナットを抱く羽根。板の内の面は座の輪郭に接する直線。
//   羽根は板の下端から S_BOSS_H だけ外へ張り出し、hull で板の上端へ向かって傾斜で戻す（庇を作らない）
module spk_hang_arms() for (h = S_HANDS) spk_at_hand(h) hull() {
    translate([S_R_I, -S_ARM_W / 2, Z_FOOT_B]) cube([S_PLATE_T, S_ARM_W, Z_IN - Z_FOOT_B + 0.01]);      // 板（7.8〜10.6・全高）
    translate([S_R_I, -S_ARM_W / 2, Z_FOOT_B]) cube([S_BOSS_R1 - S_R_I, S_ARM_W, S_BOSS_H]);            // 羽根（7.8〜13.1・下端から 3.4）
}
module spk_hang_cut() for (h = S_HANDS) spk_at_hand(h) {
    // 🔴 通しは**ナットの下面まで**。板の天井まで開けると、上ほど細くなる板（13.1 → 10.6 のテーパー）の
    //   外の面を穴が突き破り、厚さ 0.07 の刃が Z 3.0 にわたって残る（`--mat nylon` で 🔴）
    translate([S_SCR_R, 0, Z_FOOT_B - 1]) cylinder(d = S_SCR_D, h = S_NUT_FLR + 1.01, $fn = 24);          // 上向きのねじの通し
    // 寝たナットの部屋。口は **座の側（半径の内向き）**（🔒 ユーザー 2026-09-18「ナットの入口を内側に」）。
    //   スピーカーより先に内側から差し、入れたスピーカーの胴（逃げ 0.3）が口を塞ぐ。
    //   六角は**二面幅を半径方向**へ向ける（部屋の半径方向の幅を最小の 4.5 にするため。会話ボタンの溝と同じ流儀）
    hull() for (dr = [0, -(S_NUT_AF / 2 + 0.5)])
        translate([S_SCR_R + dr, 0, Z_NUT_B]) rotate([0, 0, 30]) cylinder(d = S_NUT_AF / cos(30), h = S_NUT_T, $fn = 6);
}
module spk_driver_probe(d = 6.0, len = 45) for (h = S_HANDS) spk_at_hand(h) translate([S_SCR_R, 0, Z_TUB_B - len]) cylinder(d = d, h = len + 0.01, $fn = 24);   // ドライバーの軸が要る空間（バスタブの板の裏から**下へ**。天板を伏せて組むので上が空いている）
module spk_nut_path(len) for (h = S_HANDS) spk_at_hand(h) hull() for (dr = [0, -len])   // ナットが入る道（座の側から半径の内 → 外へ）。case_v6_1 の nutpath が呼ぶ
    translate([S_SCR_R + dr, 0, Z_NUT_B + (S_NUT_T - S_NUT_T_REAL) / 2]) rotate([0, 0, 30]) cylinder(d = (S_NUT_AF_REAL + 0.3) / cos(30), h = S_NUT_T_REAL, $fn = 6);   // 実物のナット（4.0 x 1.6）を部屋の高さの真ん中に置いて掃く。⚠ 部屋の遊び（Z 0.2）を道に足すと床と天井に当たって鳴る
...
// ⭐ 2026-09-18 トリムの相手を「ナットのポケット」から **手の窓そのもの** に変えた。
//   ポケットとの交わりを hull で取ると、その凸包の端で手の内の面が削り残り、
//   厚さ 0.07mm・高さ 3.0mm の刃が立つ（`--mat nylon` で 🔴・左右 2 本）。
//   手の幅より 1.0 広い窓の中で「座を S_THIN 太らせた形」を丸ごと抜けば、
//   切り口は座のオフセット曲線と窓の縁だけになり、羽根状の削り残りが出ない
module spk_thin_trim() translate([0, 0, Z_FOOT_B - 0.01]) linear_extrude(Z_IN - Z_FOOT_B + 0.02)
    for (h = S_HANDS) intersection() {
        offset(r = S_THIN) spk_seat2d();
        spk_at_hand(h) translate([0, -(S_ARM_W / 2 + 1.0)]) square([S_BOSS_R1 + 1.0, S_ARM_W + 2.0]);
    }
...
module spk_tub_floor2d() union() {
    ...
    for (h = S_HANDS) translate([h[0] * S_SEAT_CX, 0]) rotate(h[0] > 0 ? h[1] * S_NUT_ANG : 180 - h[1] * S_NUT_ANG) translate([S_ARM_R0, -S_TUB_ARM_W / 2]) square([S_R_ARM - S_ARM_R0, S_TUB_ARM_W]);   // ねじを受ける腕（放射方向・点対称）。外の面は吊りの羽根と同じ半径
}
...
// ⭐ 2026-09-18 壁を廃止。あれは横ねじの頭（φ4・壁の外 16.3）を出すためだけの物で、
//   ねじが縦になった今は何も持っていない。残すと腕の上に高さ 5.25 の羊羹が立つだけ
module spk_tub() color("#e0a040") difference() {
    ...
    for (h = S_HANDS) spk_at_hand(h) translate([S_SCR_R, 0, Z_TUB_B - 0.01]) {
        cylinder(d = S_SCR_D, h = S_TUB_T + 0.02, $fn = 24);      // 上向きのねじの通し
        cylinder(d = S_CB_D, h = S_CB_H + 0.01, $fn = 32);        // 頭のザグリ（板の裏から 0.8・残り 1.2）
    }
    spk_hang_clear();
}
// 🔴 吊りの板・羽根の居場所をバスタブから抜く。前の止めの壁（spk_lip・半径 7.8〜9.3 の帯）が
//   板の角と重なっていた（旧の形でも 1.282mm³。手を 6.60 → 7.0 に広げて 2.097 になる）。
//   板は天板から降りてくる別部品なので、バスタブ側を欠くのが正しい（縁に対する spk_rim_relief と同じ流儀）
module spk_hang_clear() for (h = S_HANDS) spk_at_hand(h)
    translate([S_R_I - 0.5, -(S_ARM_W / 2 + S_PLAY), Z_FOOT_B - 0.01])
        cube([S_BOSS_R1 - S_R_I + 1.0, S_ARM_W + 2 * S_PLAY, Z_IN - Z_FOOT_B + 1.0]);
module spk_tub_screws() color("#b8b8b8") for (h = S_HANDS) spk_at_hand(h) translate([S_SCR_R, 0, Z_TUB_B + S_CB_H]) { mirror([0, 0, 1]) cylinder(d = S_HEAD_D, h = S_HEAD_T, $fn = 24); cylinder(d = 2.0, h = S_SCR_LEN, $fn = 16); }   // M2×4。頭はザグリの中（0.5 下へ出る）、軸は 板 1.2 → 羽根の床 0.8 → ナット
module spk_tub_nuts() color("#b8b8b8") for (h = S_HANDS) spk_at_hand(h) translate([S_SCR_R, 0, Z_NUT_B]) rotate([0, 0, 30]) cylinder(d = S_NUT_AF_REAL / cos(30), h = S_NUT_T_REAL, $fn = 6);   // 寝たナット（羽根の中。絵として描く）
