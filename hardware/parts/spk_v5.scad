...
S_NUT_ANG = 20;                              // 🔒 ユーザー 2026-09-14（v6.1）「小判がた同心円上でナット位置を移動・2 つのナット位置は点対称をキープ」: 57 → 20。57 のままだと、ナットを差し込む道が ReSpeaker の板の頭（v6.1 で 5.0 上げた）に 8.49mm³ 入って組めなかった。20 で 0。点対称は S_HANDS が持つので角度だけで保たれる。⚠ この値は v5（case_v5.scad）のスピーカーにも効く   // 🔒 ユーザー 2026-09-09「手は小判の同心円みたいな形になっていい」→「やっておくか」: 手・ナット・ねじ・壁を、端の半円の芯（x ±S_SEAT_CX）から S_NUT_ANG 度倒した放射方向に置く。0 なら真横（前の形）。57 は OLED の板の後ろの縁（世界 y 4.40）にねじの頭が 0.40 で並ぶ角度（60 だと 0.12 しか空かない）
