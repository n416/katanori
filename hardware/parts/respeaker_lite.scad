...
//   → **外形寸法を面の位置として使わない。**
//
// 残っている ⚠（2026-09-06 に棚卸し。それ以外の ⚠ は確かめ済みか注意書きなので印を外した）:
//   - 挿さる物の長さ（ジャックのプラグ 20・USB-C ケーブル 25）… ⚠ 推測。箱の外に出る物で、皮の形には効かない
...
// プラグのオーバーモールドの上限（⚠ 推測。🔴 v5 では未使用: 右の壁の穴は殻の寸法で開け、樹脂の胴は壁の外に残る） [厚み(面から), 幅]。規格は殻（8.25×2.40）しか決めていない。
...
// ⚠ 挿さる物の長さ（推測。箱の外に出る物で、皮の形には効かない。実測で置き換える）
JACK_PLUG_OUT  = 20.0;
USBC_CABLE_OUT = 25.0;
...
module respeaker_mating_space() {
    ...
    color("#f80", 0.3) {
        ...
        // 📄向き / ⚠ 長さは推測: ReSpeakerのUSB-C。口は上の長辺より1.78mm内側。ケーブルは上へ抜ける
