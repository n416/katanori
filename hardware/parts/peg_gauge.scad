// 🔴 欠陥（ユーザー 2026-09-10「酷いゲージだった」）: ダボも穴も板に付いているので、ゲージ同士ではダボを穴に入れられない（斜めにして当てるしかなかった）。
//    次に作るなら、ダボは持てる棒の先に 1 本ずつ、穴は板の端に。
// ✅ 結果 2026-09-10（補償 OFF）: 口がプレート側の止まり穴で φ3.0 のダボに合うのは 35 だけ。口が上の穴では 29 でも入る。
