# -*- coding: utf-8 -*-
"""反り対策のリブの線を生成する（v5）。python hardware/tools/ribs_gen.py

case_v5.scad の part="ribfree_<板>" が出す 2D（リブの空きを線の半幅だけ縮めた形）を SVG に書き出し、
格子の線（ピッチ RIB_P）との交わりから「幅が丸ごと空いている区間」を取り、RIB_LMIN 以上の区間だけを
hardware/parts/ribs_v5_gen.scad の RIB_SEGS に書く。case_v5.scad の rib_2d() がこれを読む。

  seg = [板, 向き(0 = 2D の x に走る / 1 = y に走る), 線の位置, 始点, 終点]   2D の座標は case_v5 の flat() と同じ（壁: (Z, Y) ／ ハッチ・フロント: (X, Z)）
...
OUT = os.path.join(HW, 'parts', 'ribs_v5_gen.scad')
TMP = os.path.join(HW, '_tmp_v5')
OPENSCAD = r'C:\Program Files\OpenSCAD (Nightly)\openscad.com'
PLATES = ['lwall', 'rwall', 'hatch', 'front', 'bridge']
RIB_P, RIB_LMIN = 8.0, 4.0     # case_v5.scad と同じ値（ピッチ・最短。🔒 ユーザー 2026-09-05「4mm 未満は出さない」）
LO, HI = -200, 200
...
def main():
    ...
    with open(OUT, 'w', encoding='utf-8', newline='\n') as f:
        f.write('// 反り対策のリブの線（自動生成: python hardware/tools/ribs_gen.py。手で編集しない）\n')
        f.write('// seg = [板, 向き(0 = 2D の x に走る / 1 = y), 線の位置, 始点, 終点]。2D は case_v5 の flat() の座標（壁: (Z, Y) ／ ハッチ・フロント: (X, Z)）\n')
        f.write('RIB_SEGS = [\n')
        for s in allsegs:
            f.write('  ["%s", %d, %g, %g, %g],\n' % s)
        f.write('];\n')
