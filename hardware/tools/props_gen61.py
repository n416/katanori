# -*- coding: utf-8 -*-
"""刷る向きの STL から支柱の位置を決めて hardware/parts/props_v61_gen.scad を書き出す（v6.1。tools/props_gen.py の写し・読む .scad と書く先と PARTS だけ違う。元は v5。frozen/v1-v4/_v4_props.py の移植・2026-09-05）。
   ...
   使い方: python hardware/tools/props_gen61.py   （素の形は -D PROPS_OFF=true で焼く）
"""
...
OUT = os.path.join(HW, 'parts', 'props_v61_gen.scad')
...
PARTS = ['top']   # v6.1: 支柱が要るのは天板だけ（🔒 ユーザー 2026-09-14「天板だけですよね支柱とラフト」。帯 A/B/C は v6.1 に無い＝電流計と PowerBoost が PCB に載ったため）。top はつまみのへこみ（keepout_top で軸・ねじ・リードの穴を避ける）。v5: まず帯 3 本（ユーザー 2026-09-05「帯の柱を立てないといけませんね」）
...
out = ['// 🔴 自動生成。手で直さない。作り直しは `python hardware/tools/props_gen61.py`',
