...
BUNDLES = [
 ...
 ('会話ボタン', 'btn2', 2, 'J6（縦・プラグは上から挿す）', '1 GND・2 D2',
  'マイクロスイッチの <b>C</b>（GND）と <b>NO</b>（D2）。<b>DuPont のメスの端子</b>（殻なし）を圧着して挿し、はんだで固めて<b>収縮チューブ</b>で包む。NC は空き', '1 黒（白でも可）・2 紫'),
...
def acts_topsub(mat):
    ...
    return [
     ...
     '<b>会話ボタン</b>: 手順 0 で付けた DuPont のメスの端子を、スイッチの <b>C（黒）と NO（紫）</b>へ根元まで挿す（端子はレバーの根元から C・NO・NC。NC は空き）。挿したまま端子の口とスイッチの端子をはんだで固め、<b>収縮チューブを端子の口から線の被覆まで被せて縮める</b>。隣どうしは 5.08 間隔で、むき出しの金属の間は 3 弱しかない。',
     '<b>直はんだ 2 か所</b>（手順 0 で作った線の、殻の無い側）: スピーカー OUT → スピーカーのリード、リード → リードスイッチの足。',
...
# ---------------------------------------------------------------- 手順（ナイロン）
STEPS_N = [
 dict(n='0', t='基板に手はんだし、線を 4 本作る（机の上で）', acts=[
   '<b>ハブ（PCB）に足付きの部品 8 個をはんだ付けする</b>: J1・J2（オスのピンヘッダ）、J4・J6・J7（縦の PH）、J5（横出しの PH）、SW2（電源の押しボタン）は表。<b>J10（電池の PH）だけは板の裏</b>。'
   'PH の口の 1 番の向きは板のシルクと PCB-V61.md 8 章で確かめる（足形は gen_pcb.py の <span class="d">HAND_SOLDER</span>）。',
   '<b>XIAO のライザーの表</b>に表面実装のメス 2 個（1x3 が電源の列・1x4 が信号の列）。パッドは互い違いで、<b>電源の列の真ん中の足だけ下向き</b>。',
   '<b>XIAO のライザーの裏</b>に L 字のメス 1x7。<b>胴と板の間にはがき 2 枚（計 0.4 前後）を挟んだまま</b>はんだ付けし、足が固まったら抜いても残してもよい。'
   'L 字の軸を PCB の J1 に合わせ、垂らした板の下の端とピンヘッダーの樹脂の間を 0.4 空けるため（case_v6_1.scad の <span class="d">XRISER_T</span>・<span class="d">LSOCK_SHIM</span>）。コピー用紙は 1 枚 0.09 前後なので 2 枚では足りない。',
   '<b>OLED のライザー</b>: 表に縦のメス 1x4、裏に L 字のメス 1x4。<b>はがきは挟まない</b>（板厚 1.6）。',
   '<b>v6.1 は 1 枚の PCB に口が集まった</b>ので、v5 の 12 束は無い。線は 4 本で、<b>板の側は全部 JST-PH 2.0 の殻</b>（⭐ 2026-09-19 にトグルの 2 本が消えた。電源の押しボタンは板に直に付く）。',
   '__CUTTABLE__',
   '<b>板の側だけ先に圧着して殻に入れる。</b>逆の端はスピーカー IN（PH のプラグ）と会話ボタン以外は裸のまま（手順 1 で直はんだ）。',
   '<b>会話ボタンの逆の端</b>: <b>先に収縮チューブを 1 本ずつ線に通しておき</b>（板の側の殻はチューブを通らない）、DuPont のメスの端子を圧着する。<b>殻は付けない</b>（手順 1 で端子ごとはんだで固めるため）。',
...
STEPS_R = [
 dict(n='0', t='基板に手はんだし、線を 5 本作る（机の上で）', acts=STEPS_N[0]['acts'], note=STEPS_N[0]['note']),
...
BUY = ('<p>ほかに: PCB（katanori61）・XIAO のライザーと OLED のライザー（どちらも別の小さな板）・ReSpeaker Lite（XIAO 付き）・OLED・'
       ...
       '<p><b>XIAO のライザーは板厚 1.2 で頼む</b>（OLED のライザーは 1.6）。1.2 は、裏の L 字のメスの胴の下にはがき 2 枚を挟んではんだする前提（手順 0）。'
       '注文の数と品番は docs/ORDER-V61N.md。</p>')
