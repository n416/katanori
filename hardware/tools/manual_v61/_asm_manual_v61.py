...
STEPS_N = [
 ...
 dict(n='2', t='底パーツにナットを入れる（空のうちに）', img='n2', acts=[
   ...
   '<b>上の柱 3 本</b>: 後ろの 2 本は溝の口が<b>前（−Y）</b>、右前は<b>左（−X）</b>。<b>左前の上の柱にナットは無い</b>（奥行き 5.5 に溝が入らないので、M2 のタッピングねじを下穴 1.7 に直に切る・⚠ 未実機。合わなければインサート）。',
 ...
 dict(n='8', t='蓋の線を挿して、真上から降ろす', img='nlid_in', img2='n8',
   ...
   '羊羹とマッチ棒が ReSpeaker の頭に当たって蓋が 0.3 浮く。<b>天板のねじ 4 本で引き下ろす</b>: 右前・右後ろ・左後ろは <span class="d">M2×6</span>（ナットは手順 2）、左前は <span class="d">M2</span> のタッピングねじ（⚠ 長さ 6 は私の見当・未実機）。',
...
GLOSS = {
 'nylon': [
  ...
  ('柱', 'PCB の柱（右 2 本は床から・左後ろは隅の台）と、天板を留める上の柱 4 本。ナットは全部横差し（左前の上の柱だけタッピング）。', 'g_nposts'),
...
SCREWS = {
 'nylon': [
  ...
  ('M2 × 6', '3', '天板 → 上の柱（右前・右後ろ・左後ろ。ナットは横差し）', '8'),
  ('M2 タッピング', '1', '天板 → 左前の上の柱（下穴 1.7 に直に切る。⚠ 未実機・長さは見当で 6）', '8'),
...
CHECKS = {
 'nylon': [
  ('3', 'n_hub_case', 'PCB を 0.8 左で真上から（case の path_hub と同じ）', '当たりが出る = この動きでは入らない。case の path_hub（電池を先に入れた世界）では同じ動きで 174'),
...
PCBMOVE = ('<b>板を 1.5 持ち上げて、左の窓から差し入れる。</b>右の壁の手前（充電の USB-C の出のぶんだけ手前）で止め、'
           ...
           '上は背面のトグルの座（φ14）まで 13.4 空くので、傾ける必要は無い（case の path_hub は 174.08 → 0.59）。')
