...
STEPS_R = [
 ...
 dict(n='2', t='PCB を前の管 2 本に載せ、手前 2 本を締め、電池を J10 に挿す', img='r2', acts=[
 ...
 dict(n='9', t='天板の線を挿し、天板を真上から降ろす', img='r9', acts=[
   '<b>天板を箱の上に持ち、線を PCB に挿す</b>: スピーカー OUT → <b>J5</b>（左から）、会話ボタン → <b>J6</b>（前から）、リードとトグルの 4 極 → <b>J7</b>（縦・上から）。トグルの 2 本は後ろへ垂らしておく（手順 11）。',
 ...
 dict(n='11', t='ハッチにトグルを付け、後ろから押し込んで閉じる', img='r11', acts=[
   ...
   '<b>ハッチを垂直のまま、真後ろから押し込む。</b>上の耳 2 つが後ろの上の柱の上（天板の下）へ、足が床の上を滑る。押し込むと足の口の 0.5 先に電池が来て、ナットが落ちなくなる。',
...
SCREWS = {
 'nylon': [
  ...
  ('M2 × 12', '1', '床の裏 → 右後ろの柱の頭のナット（板は締めない）', '8'),
  ('M2 × 6', '4', '天板 → 上の柱（4 本ともナットは横差し）', '8'),
  ('M2 × 18', '1', '床の裏 → 左後ろの柱 → PCB → 蓋の座のナット', '8'),
