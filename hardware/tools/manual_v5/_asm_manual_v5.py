...
GLOSS_IMGS = ['sara', 'obi', 'koobi', 'maeita', 'dote', 'yokan', 'za', 'L', 'tsume', 'uke', 'futa', 'hashira', 'mimi', 'tub', 'uke_tc']
...
def wire_lengths():
    ...
    os.makedirs(TMP, exist_ok=True)   # 無いと OpenSCAD が出力先を開けず echo が 0 本になり、切る長さが 0・7・14 に潰れる（2026-09-07）
    ...
    if not segs:   # 空は「線が無い」ではなく評価の失敗。数字にせず止める
        sys.exit('wire_lengths: wlen の echo が 0 本。OpenSCAD の出力:\n' + (r.stderr + r.stdout)[-2000:])
    return segs
...
STEPS = [
 ...
 dict(n='7', t='留め帯 3 本を横から差す', img='st7', acts=[
   ...
   '<b>3 本は形が違う。</b>真ん中の帯の天板には電流計を受けるダボ 2 本（φ2.8・高さ 4.4）、奥の帯の天板には電流計のピンの足の逃げ（1.5 掘り）が付いている。向きや順を間違えると手順 9 で電流計が載らない。',
 ...
 dict(n='9', t='電流計を帯に載せ、電源の線を挿す', img='st9', acts=[
   '電流計は<b>板を箱の軸に平行</b>にして、真ん中の帯のダボ 2 本（φ2.8）に板の 2 穴を通し、真ん中と奥の帯の天板に全面で当てる。上からまっすぐ落とすだけで、横から差し込む所は無い。<b>電源の L 字 4 本が前（OLED 側）、I2C の L 字が後ろ（ハッチ側）。</b>',
   '<b>小帯を掛ける。</b>小帯は丸い足 2 つ（φ6・高さ 3）を橋（厚み 1.5）で結んだ小さな部品で、足の穴（φ3）をダボに通して電流計の上に載せる。ダボの先は小帯の上面より 0.2 低い。<b>E リングもねじも無い。</b>'
   '小帯はこの段では載っているだけで、手順 12 で天板の裏の足（φ3.6）が小帯の右の足の上に降りて押さえる。天板を載せるまで箱を傾けない。',
 ...
 dict(n='10', t='天板の小組を机の上で作る', img='topsub',
      cap='天板を裏返して見た図。PowerBoost（4 本のダボ）・つまみ（手 4 本とバスタブ 2・AS5600 込み）・会話ボタンのバスタブ・スピーカー（小判の座と縁・斜めに倒れた手 2 枚）・OLED の L・羊羹とマッチ棒・トグルの受け', acts=[
   ...
   '<b>つまみの島と持ち手</b>: 台座は天板と一体。<span class="g">島</span>をへこみに落とし（タブの向きを合わせるだけ・押し込む所は無い）、<span class="d">M2×6</span> 2 本で締める。ナットは<b>台座の裏の六角ポケット</b>へ落とす（天板が裏返しなので口が上を向いていて落ちない）。'
   '持ち手を軸ごと通し、<b>軸の溝に E リング（呼び 6）を横から差す</b>。<b>差す向きは、島の 2 本のねじを結んだ線と同じ側（左右）から</b>——手前・奥からは、天板の裏の <span class="g">手</span> 4 本が同じ高さに立っているので入らない。ねじの先はリングの帯まで届かないので、ねじの側は空いている。',
   '<b>AS5600 とつまみの <span class="g">バスタブ 2</span></b>（板と壁 2 枚の別部品）: <b>先に机の上で</b>基板をバスタブ 2 へ留める。<b>板の裏から</b> <span class="d">M2×6</span> を 4 本（頭は板の裏のザグリに沈む）、<b>ナット 4 個は基板の上</b>に直に置く（座リングは無い）。'
   '板はヘッダの 2 列を切り欠いてあるので、基板の裏のピンはその切り欠きから下へ出る。',
   '<b>つまみを吊る</b>: 台座の裏から <span class="g">手</span> が 4 本下りている（向かい合う 2 面に 2 本ずつ）。まず<b>手の横の溝</b>（隣り合う 2 本の、外を向いた面に開いている）へ <span class="d">M2</span> のナットを 1 個ずつ差す——<b>バスタブ 2 を被せる前に</b>。'
   '基板を付けたバスタブ 2 を、壁が手を外から掴むように被せる。手の下端が<b>板の上面のくぼみ</b>に入って位置が決まる。<b>壁の外から横に</b> <span class="d">M2×6</span> を 4 本（壁 1 枚に 2 本。2 本あるので首を振らない）。'
   'AS5600 の基板の裏の 5 本のピンが下を向いて出る——ここへ手順 12 で <span class="w">つまみ</span> の 1 連 × 5 を挿す。',
   ...
   '<b>スピーカー</b>: 天板の裏の小判の座へ<b>真下から真っすぐ</b>落とす（グリル側が天板側）。奥に当たるまで押すと、後ろ半分の<span class="g">縁</span>が位置を出す。<b>両面テープは使わない</b>——この後のバスタブで座との間に挟む。',
   '<b>スピーカーのバスタブ</b>: 天板から下りている<span class="g">手</span>（小判の端に沿って斜めに倒れた板 2 枚・対角）の溝へ <span class="d">M2</span> のナットを 1 個ずつ落とす（天板を裏返しているので口は上を向いている）。リードを座の後ろの縁から手と止めの壁の間の隙間へ逃がしてから、バスタブを被せ、<b>壁の外から横に</b> <span class="d">M2×4</span> を 2 本締める。板の中央の台がスピーカーの裏（磁石の面）に当たり、ねじで引き上げて挟む。',
   'リードは<b>スピーカーの左の端の後ろの縁</b>から出る（端子の三日月の基板が左の端にある）。手順 12 でハブの <span class="w">スピーカー OUT</span> と直はんだ。',
   '<b>OLED の <span class="g">L</span> には何も付けない。</b>ねじもナットも無い。足の前面のダボ 2 本（φ2.8・長さ 2.0）が折れていないかだけ見る。',
   '<b>トグルはここでは付けない</b>（ハッチに付く・手順 13）。',
 ], img2='knob', cap2='つまみまわりの分解（hardware/parts/knob_v5.scad の explode）。上から 持ち手・島・天板の切れ端・軸の E リング・島のねじ・AS5600・バスタブ 2（ねじとナット込み）。'),
 ...
 dict(n='12', t='上の道の線を挿し、天板を載せ、フロントを差す', img='st12', img2='pb_pins',
   ...
   '<b>天板を降ろす。</b>羊羹とマッチ棒が ReSpeaker の頭を 0.3 押し、つまみの台座の前の欠きに電流計の電源の口の先が入る。OLED の L はまだ何にも当たらず、足の前面（Y 4.4）のダボ 2 本が前を向いて待つ。天板の裏の足（φ3.6）が電流計の小帯の右の足の上に降りて、電流計を帯に押さえる。<b>降ろす前に小帯がダボから外れていないか見る。</b>'
...
GLOSS = [
 ...
 ('帯', '電池を上から押さえる ⊓ が 3 本（太さ 6.9・等間隔）。足のツバが土手の溝に入る。電流計は真ん中と奥の 2 本に載り、真ん中の帯のダボ 2 本に小帯を掛けて留める。', 'obi'),
 ('小帯', '電流計の 2 穴から出るダボ 2 本に掛ける橋。丸い足 2 つを結ぶ。天板の足が右の足を押さえて電流計を留める。', 'koobi'),
...
SCREWS = [
 ...
 ('M2 × 6',  '2', 'つまみの島 → 天板（上から。ナットは台座の裏の六角ポケット）', '10'),
 ('M2 × 6',  '4', 'つまみのバスタブ 2 → 天板の手（壁の外から横に。壁 1 枚に 2 本。ナットは手の中の横向きの六角）', '10'),
 ('M2 × 6',  '4', 'AS5600 → つまみのバスタブ 2（板の裏から。頭は板の裏のザグリ・ナットは基板の上）', '10'),
 ('M2 × 4',  '2', 'スピーカーのバスタブ → 天板の手（壁の外から横に。対角の 2 本。ナットは手の中の縦の六角）', '10'),
...
NUTS = [
 ...
 ('M2', '2', 'つまみの島（台座の裏の六角ポケット。天板が裏返しなので口は上向き）', '手順 10'),
 ('M2', '4', 'つまみの手の中（手の横の溝から差す。溝は隣り合う 2 本の、外を向いた面に開いている）', 'バスタブ 2 を被せる前（手順 10）'),
 ('M2', '4', 'AS5600 の基板の上（バスタブ 2 に留めるねじの相手。座リングは無い）', '手順 10'),
 ('M2', '2', 'スピーカーの手の中（板の下端の溝から差す。天板が裏返しなので口は上向き・バスタブを載せると口が塞がる）', '手順 10'),
 ('M2', '1', '蓋の床の板の閉じたポケット（右の縁の溝から差す）', '手順 13'),
 ('E リング 呼び 1.5', '4', '天板のダボ 4（PowerBoost）。電流計には使わない（樹脂の軸に E リングは割れる）', '手順 10'),
 ('E リング 呼び 6', '1', 'つまみの軸の溝（天板の裏から横に差す。基板の側は E リングをやめてバスタブ 2 のねじ止めになった）', '手順 10'),
]
PARTS = [
 ...
 ('ブリッジ（皿と腕）', '1', '腕の上に格子'),
 ('前板', '1', ''),
 ('帯', '3', '3 本とも違う形（真ん中の帯にダボ 2 本・奥の帯に足の逃げ）。hardware/stl/v5/v5_strap_a.stl・v5_strap_b.stl・v5_strap_c.stl'),
 ('電流計の小帯', '1', 'ダボ 2 本に掛ける橋。上面を下にして刷る。hardware/stl/v5/v5_inabar.stl'),
 ('蓋（シャッター）', '1', '磁石 2 個を圧入'),
 ('ロック', '1', ''),
 ('蓋の床の板', '1', 'ハッチの裏へ接着。ロックのナットのポケット・磁石 2 個'),
 ('つまみ一式', '1 組', '持ち手と島。hardware/parts/knob_v5.scad（hardware/stl/v5/v5_knob.stl・v5_knobwall.stl）'),
 ('つまみのバスタブ 2', '1', 'AS5600 を吊る板＋壁 2 枚（天板の手 4 本を外から掴む）。板の裏に基板のねじの頭のザグリ。板を下にして刷る。hardware/stl/v5/v5_knobhang.stl'),
 ('会話ボタンの押し子とバスタブ', '1 組', 'hardware/parts/btn_v3.scad'),
 ('スピーカーのバスタブ', '1', '小判なりの板＋倒した壁 2 枚＋クランプの台。hardware/stl/v5/v5_spktub.stl'),
]
