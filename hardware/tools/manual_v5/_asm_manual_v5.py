...
STEPS = [
 ...
 dict(n='5', t='前板を皿に留めてから、ブリッジを置く', img='st5', acts=[
   ...
   '<b>ブリッジ（<span class="g">皿</span> と左右の <span class="g">腕</span>）をまっすぐ下へ降ろす。</b>前板の脚が床に立ち、皿はハブの口の頭の 1.8 上に浮く。<b>腕を受ける棚はまだ無い</b>（手順 6 の壁に付いている）ので、ブリッジは前板の脚で前だけ支えられ、後ろは放すと下がる。手順 6 で壁を寄せるときに持ち上げる。',
 ...
 dict(n='10', t='天板の小組を机の上で作る', img='topsub',
   ...
   '<b>OLED の <span class="g">L</span> の後ろの六角に <span class="d">M2</span> ナットを 2 個入れる。</b>六角は後ろへ開いていて深さはナットの厚みちょうど、天板を箱に載せるまで摩擦だけで持つ。'
   '天板を載せた後では届かない（後ろから見ると会話ボタンの台座・PowerBoost・つまみの台座が道を塞ぎ、前は OLED、上は天板）。天板を起こすときに落ちていないか見る。',
 ...
 dict(n='12', t='上の道の線を挿し、天板を載せ、フロントを差す', img='st12', acts=[
   ...
   '<b>OLED の上の 2 穴に前から <span class="d">M2×6</span>。</b>ナットは手順 10 で L の後ろの六角に入れてある。フロントより先。',
 ...
 dict(n='13', t='ハッチに蓋とトグルを付け、後ろから押し込んで閉じる', img='st13', img2='claw',
   ...
   '<b>ハッチは垂直のまま、真後ろから真っ直ぐ押し込む。</b>内面の下端の <span class="g">爪</span> 2 つの唇が床の後ろの帯のポケットへ後ろから入ってバーの下へ、'
   'トグルの胴が天板の裏の <span class="g">受け</span> へ後ろから、<b>上の耳 2 つが天板の裏を滑って後ろの上の柱の頭の上へ、右下の耳が床と後ろ右の下の柱の間（3.2）へ</b>入る。'
   ...
   'ハッチには充電の Type-C の口（左下）が開いている。Type-C 基板の殻の面は外面の 0.8 裏。',
    ...
    note='ハッチは 下＝爪 2・上＝耳 2 のねじ・右下＝耳のねじ、で留まる。爪は後ろへ抜ける動きを止めないので、ハッチを持つのはねじ 3 本。トグルの受けは胴の X と前を持つ。'),
]
...
GLOSS = [
 ...
 ('爪と帯', 'ハッチの内面の下端の脚と唇。床の後ろの帯のポケットへ後ろから入り、バーの下に居る。', 'tsume'),
...
SCREWS = [
 ...
 ('M2 × 6',  '2', 'OLED → 天板の L（前から。ナットは手順 10 で L の後ろの六角に）', '12'),
...
NUTS = [
 ...
 ('M2', '3', '下の柱の頭（上向き・置いてあるだけ）', '壁を立ててから・締めるまで伏せない（手順 6）'),
 ('M2', '1', '前板のフランジの下の箱', '手順 5'),
 ('M2', '4', '会話ボタン（バスタブの壁 2・天板の腕のブロック 2）', '手順 10'),
 ('M2', '2', 'つまみの島', '手順 10'),
 ('M2', '2', 'OLED の L の後ろの六角（後ろへ開放・摩擦だけ）', '手順 10（天板を裏返しているうち）'),
