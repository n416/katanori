...
module sw4_dilate(cl) for (d = [[0, 0, 0], [cl, 0, 0], [-cl, 0, 0], [0, cl, 0], [0, -cl, 0], [0, 0, cl], [0, 0, -cl]])
    translate(d) children();
// 🔴 2026-09-04 線は Y を**貫いて**引く。線は Y 70.7 を走っていて、増し肉（板 69.8〜71.0 と
//   縁 71.0〜72.0）の厚みの**途中**で止まる。管の横っ腹が板の縁をかすめる所では、切り残しが
//   皮になって残った ── 実測: 床の板の X 63.00〜63.25・Z 24.75〜32.50 に **厚み 0.150mm**、
...
module sw4_backing_carve(cl) {
    ...
    for (t = [-SW4_CARVE_Y : SW4_CARVE_S : SW4_CARVE_Y]) translate([0, t, 0]) sw4_dilate(cl) w_pwr3();
}
...
module sw4_floor_plate() difference() {
    union() {
        ...
        //    外周はぐるり座の壁に載っている ── 2026-08-29 に直した「浮いた 7.9mm2 の天井」ではもう無い。
        //    座を 0.02 前へ出して 0.42 にできるかを測った: **右は隙間 0.000mm**（INA の I2C の束が
        //    X 47.7〜49.9・Z 22.47〜30.33 で、座の前面 Y 68.600 にぴったり着いている）。左は 0.5 以上空き。
        //    ⇒ 右を動かすには束の経路が先。**未解決のまま 0.40 で刷る**
    }
