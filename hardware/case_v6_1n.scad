// ============================================================
// 筐体 v6.1n（ナイロン・一体シェル版）— hardware/case_v6_1.scad の写し
//   🔒 ユーザー 2026-09-14: MJF（ナイロン PA12）で刷る前提。板を 6 枚に分ける理由（寝かせて刷る・反る・
//      支柱の跡）が全部消えるので、**床＋4 壁を 1 部品（シェル）にして、天板だけ蓋**にする。
//      ・中身は上からしか入らない（ReSpeaker と PCB は幅 82.02 に対し内寸 82.65＝左右の余裕 0.63）
//      ・つまみ・スピーカー・会話ボタンの小組は天板の上で今までどおりやるので、天面は蓋のまま
//      ・ねじが無くなれば面ごとに厚みを変える理由も無い → 全面 1.2 に統一（局所だけ厚くする）
//   レジン版（case_v6_1.scad）は触らない。こちらの直しをあちらへ持って行くときは手で運ぶこと。
// ============================================================
// ============================================================
...
MAT = "nylon";   // v6.1n はナイロン専用（"resin" にしても意味は無い）
...
// 嵌め合いを MJF 用に置き換える（🔴 include の**後ろ**に置くこと。前だと fit.scad の既定に上書きされる）
FIT_SLIP = 0.8;   // 印刷どうしの狙いの隙間。JLC3DP の「組む隙間 0.76 以上」より
FIT_BURN = 0;     // 初層の太りの打ち消し。粉の中で焼く MJF には初層が無い
...
use <parts/spk_v61n.scad>   // スピーカー v6.1（parts/spk_v5.scad の写し。いまは中身も同じ・v6.1 の直しはこちらへ）。原点 = スピーカーの中心・z0 = 天板の外面
...
// 🔒 ユーザー 2026-09-14「内寸を広げてよいですか」→「OK!」: v6.1n は −1.2 → −0.4（右の壁を外へ 0.8）。
//   ReSpeaker を箱へ上から降ろすとき、XIAO の USB-C の殻は壁の内面より 1.22 外へ出るので、
//   いったん右へ 1.22 寄せて降ろし、左へ寄せて口に入れる。遊びが 0.63 → 1.43 になりその道ができる
RIGHT_CL = -0.4;   // 🔒 v4 2026-08-25 ユーザー「壁の移動が足りないだけ」: XIAO の USB-C の殻（先 85.554）を壁に貫通させ、口の面を外面の 0.8 裏に置く
...
TGL_BOSS_D = 14.0; TGL_BOSS_T = 1.8;   // 🔒 ユーザー 2026-09-14「背面のトグルの座」: 板 1.2 ＋ 1.8 ＝ 3.0 でブッシングを受ける。
//   M6 のナットは外側（ブッシングは 8.8 出るので 3.0 使っても 5.8 残る）。スイッチの胴はこの座の面に当たるので、箱の中へ 1.8 深く入る
//   🔴 TGL_AT より**前**に置くこと（後ろだと undef のまま読まれる。2026-09-14 に 5 回目）
TGL_AT  = [(LW_X + IN_X) / 2, IN_Y - TGL_BOSS_T, IN_Z - 7.1];   // v6.1n: 背面の座（1.8）の面に胴が当たるので、その分だけ箱の中へ   // 🔒 ユーザー 2026-09-14「トグルを X 中央にもってきて上にくればいい・v4 の尻尾位置」: X は箱の中央 43.024（v4 は case_base.scad:190 の TGL_AT = [40, …]）・Z は天井から 7.1 吊る（v5 と同じ式）   // 🔒 ユーザー 2026-09-14「トグルスイッチを OLED の Y 中央に合わせて移動」: 顔の縦の中央 ＝ 世界 Z 24.05（それまで天井から 7.1 の 43.35）   // 🔒 ユーザー 2026-09-14「トグルスイッチは Y 軸対称位置に移動」: 左右対称の位置（X 6.194 → 79.854）。動かしたのは置き場所だけで、部品は裏返していない   // 🔒 ユーザー 2026-09-14「トグルの板部分を PCB 基板の Y にそろえて」: 取付面を PCB の後縁 50.9 に（ハッチはまだ v5 の 77.25）   // 🔒 ユーザー 2026-09-14「トグルスイッチを左壁側へ移動」（回転はしない）: ハッチに付いたまま X だけ左の壁へ。胴（8）と壁の内面の隙間 0.5 は私の仮
        // ハッチに付くトグルの軸。天井から 7.1（胴の上端は天井の 0.6 下・胴の下端は蓋の縁の上端 37.5 の 0.45 上）。🔴 8.4 は AI の仮定で、+2 にしたとき蓋の縁に 2.0 入っていた

// 🔴 2026-09-14: 1.2 → 1.6。JLC3DP の MJF は**最小肉 1.0・公差 ±0.3**。1.2 だと薄い側へ振れて 0.9 で下限を割る。
//   1.6 なら振れても 1.3。スパナの彫り 0.35 を引いても 1.25 残る（1.2 のままだと 0.85 で下限割れ）
WALL = 1.6; FLOOR_T = 1.6; FRONT_T = 1.6; HATCH_T = 1.6;   // v6.1n: 全面 1.2 に統一（🔒 ユーザー「ねじ止めが無い壁は、ある壁と統一出来る」）   // 板の厚み。
...
module p_lwall() difference() {
    ...
    // v6.1n: xusb_shell_relief（壁を上から降ろすとき殻が通る道）は消した。シェルでは壁は降りて来ないので前提が無い。
    //   代わりに ReSpeaker を右へ寄せて降ろす（上の RIGHT_CL）。この道を残すと口の下の肉が 0.23 になる
...
module p_front() difference() {
    union() { slab_front(); }   // v6.1n: 耳は無い（フロントは壁と一体）   // 板は Y −1.8〜1（厚み 2.8・内面 Y 1.0）。耳は上 2・下 2
...
// v6.1n: 壁が 1.2 なので彫りは 0.35（残り肉 0.85。1.0 のままだと残り 0.2 で MJF の最小肉 0.8 を大きく割る）
// 🔴 この行は代入が 7 つ並ぶ。行末にコメントを足すと後ろの 6 つが飲まれて undef になる（2026-09-14 に再発）
ICON_D = 0.35; ICON_GAP = 1.5; ICON_K = 1.5; ICON_R = 0.20; ICON_BOLT_K = 1.3; ICON_HP_SW = 2.0; SVC_MIN_W = 0.5;
...
module tgl_boss() at_tgl() translate([0, 0, -0.01]) cylinder(d = TGL_BOSS_D, h = TGL_BOSS_T + 0.01, $fn = 48);   // 🔴 局所 +z が壁の外。座は原点から**外へ** 1.8（内へ伸ばすと壁から離れた円盤が宙に浮く）
module p_hatch() difference() {
    union() { slab_hatch(); tgl_boss(); }   // v6.1n: 耳も足も無い（背面は壁と一体）。トグルの座を足す   // v6.1: 蓋の縁・Type-C の羊羹は無い   // 縁（溝の床〜内面・つば）・上の耳 2 つ・下の足 2 つはハッチと一体。下の爪 2 つは 2026-09-07 に廃止（床の裏からのねじ 2 本に）。右下の耳も同日廃止（🔒 ユーザー「羽根はいらないね」）
    hatch_foot_cuts();   // 足のナットの横穴とねじの通し（羊羹に埋められないよう、ハッチ全体から引く）
    at_tgl() translate([0, 0, -1]) mts102_hole61(TGL_BOSS_T + HATCH_T + 2);   // v6.1n: 座（1.8）のぶん穴を長くする（🔴 短いと壁が抜けず、トグルが 173mm³ 壁に食い込む）   // 局所 +z がハッチの外（🔴 2026-09-05 まで −z 側に切っていて穴が内側に居た）
}
...
module p_floor() difference() { union() { slab_floor(); hub_posts(); rsp_seat(); oled_rib(); } hub_screw_cuts(); }   // v6.1n: 板を留めるねじ（床の 4 本とハッチの足 1 本）と、その座ぐり・床の台は無い。残るのは PCB の柱だけ   // v6.1: Type-C の受けと前板の溝は無い   // Type-C の受け・ハブの柱・ReSpeaker の座・OLED のリブ・ハッチの爪の帯は床と一体
...
// ============================================================
// v6.1n: 六角は 呼び 4.0 ＋ 0.5（レジンの実績は +0.3。MJF は穴の公差が ±0.3 なので 0.2 積む）。hex_pocket がさらに +0.1 する
NUT_AF = 4.50; NUT_T = 1.8; SCR_D = 2.5; SCR_CB = 4.4; SCR_CBT = 1.6;   // v4 case_base の値（ナット厚 1.8・通し φ2.5・座ぐり φ4.4 × 1.6）。🔴 NUT_AF は 4.10 → 4.20（2026-09-05 実機で 4.10＝ポケット 4.20 にナットが入らず、PRINT.md §2「六角は呼び + 0.3」に戻した。hex_pocket が +0.1 するので 4.30）
...
EAR_T = 0;   // v6.1n: 耳は無い。上の柱は天板の裏（Z_TOP）から直に下がる（幅と奥行きは耳付きの値のまま＝ POSTS_T の 3 番目は true のまま）
...
module fasten_lwall() { post_t(POSTS_T[0]); post_t(POSTS_T[2]); }   // v6.1n: 下の柱は無い（床と壁が一体なので留める物が無い）   // v6.1: ブリッジの棚は無い・左後ろの下の柱を追加
module fasten_rwall() { post_t(POSTS_T[1]); post_t(POSTS_T[3]); }   // v6.1n: 下の柱は無い   // v6.1: ブリッジの棚は無い
...
// v6.1n: ダボは φ2.8 → 2.4。OLED の穴は φ3.0 で、MJF の穴・寸法の公差は ±0.3。2.8 のままだと太い側で 3.1 になって入らない
OLED_PEG_D = 2.4; OLED_PEG_L = 2.0; OLED_PEG_CH = 0.6;   // ダボ: φ2.8・足の前面から 2.0（板 1.6 を抜けて 0.4 出る。フロントの内面 Y 1.0 まで 1.4 残る）・先の面取り 0.6（窓の横の遊び ±1.0 を寄せる）
...
if (part == "plugs") plugs();
module shell() { p_floor(); p_lwall(); p_rwall(); p_front(); p_hatch(); }   // v6.1n: 床＋4 壁を 1 部品に
if (part == "shell") color("#8fb8a0") shell();
if (part == "shellall") { color("#8fb8a0", 0.35) shell(); color("#c9d0d8", 0.35) p_top(); innards(); }
if (part == "seam_shell_top") intersection() { shell(); p_top(); }   // v6.1n の継ぎ目はここ 1 か所だけ（0 が正）
if (part == "print_shell") shell();   // 粉の中で焼くので向きは自由。支柱もラフトも無し
if (part == "skin")   skin();
