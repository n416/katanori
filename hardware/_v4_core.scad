...
module core() {
    ...
    translate([-2.5, 7, 0]) { knob_at_v4(); as_conn(); }                     // つまみ一式（天面から下りる島・柱込み）
...
module w_as5600() color("#8e44ad") {   // つまみ 5 本（GND 二股）: 口 → Z19 車線を右へ → X 59.5 で Z 13.3 へ下り（コネクタの逃げ 15.15 の下）→ 島の下を Y 36.8 へ。線は 2 列の真下で終わる（逃げの 0.35 下）
    // 🔒 2026-08-26 つまみ −4: 下りる X も横断の終点も −4（58.0 → 54.0・76.4 → 72.4）
    wire([[38.05, 29.2, 19], [54.0, 29.2, 19], [54.0, 29.2, 13.3], [54.0, 36.8, 13.3]], 3);   // 島 −1.5 に追従
    wire([[54.0, 36.8, 13.3], [72.4, 36.8, 13.3]], 2.2);   // 4 ピン側の列へ島の下を横断
}
