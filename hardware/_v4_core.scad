...
//     BLU 左の壁 Y 63.2〜69.2（帯の左端の**上**で留める。棚は帯の後ろに立つ。下の BLU_*）
//   L と R1 へは皿から腕（BRG_ARMS・厚み 2.0）を渡す。3 点は電池の重心（X 33・Y 37.9）を三角形の内側に囲む
//   ＝ 逆さにしても回らない。
//   ✅ 帯の左端は 2026-08-26 に留めた。**真下に棚を入れるのを諦め、継ぐ高さを上げた**（下の BLU_*）。
...
//    ブリッジ側を上へ太らせて**継ぐ高さを上げれば**棚の肉が取れる。棚は帯の後ろ（Y 63.2〜69.2）に立てる。
//    実測（2026-08-26）: X 1.694〜7・Y 62.9〜70 は Z 21〜26 が全面空き。下は Type-C の板が X 3.294 まで・頭 20.5。
//    帯の上の線（X 1.694〜5.2・Z 27.75〜30.75）には耳の天面 26.4 で 1.35 残る。
//    締める順は 頭 → 耳 2.0 → 棚の肉 1.6 → ナット で、他の 3 本と同じ M2×6。ナットは棚へ横（+Y の面）から差す。
BLU_W    = 5.0;                 // 棚と耳の X 幅（壁の内面から。他の棚と同じ）
BLU_Y0   = 63.2; BLU_Y1 = 69.2; // 棚と耳の Y（帯の後ろ端 62.9 から 0.3）
BLU_BOT  = 20.8;                // 棚の底（Type-C の板の頭 20.5 から 0.3 浮かせる）
BLU_TOP  = 24.4;                // 棚の天面 ＝ 耳の底
BLU_SKIN = 1.6;                 // 棚の天面 ↔ ナットの上面（締める力を受ける肉。4 層）
BLU_PTOP = 26.4;                // 耳と座の天面（耳の厚み 2.0）
BLU_PY0  = 57.0;                // 座（帯の上の増し肉）の前端
BLU_SCR  = [LW_X + BLU_W / 2, (BLU_Y0 + BLU_Y1) / 2];   // ビスとナットの中心 [4.194, 66.2]
...
module brg_up_pad() {
    translate([LW_X, BLU_PY0, BAT_Z]) cube([BLU_W, BRG_Y1 - BLU_PY0, BLU_PTOP - BAT_Z]);      // 座（帯と面で繋がる）
