...
module ina_frame() translate([INA_DX + BAT_X0 + (lipo_size()[1] - ina_size()[0]) / 2 + ina_size()[0], PAIR_Y0 + ina_size()[1], BAT_TOP + STRAP_T + BOARD_LIFT]) rotate([-THETA, 0, 0]) rotate([0, 0, 180]) children();
...
module pb_frame()  translate([BAT_X0 + (lipo_size()[1] - PB_L) / 2 + PB_L, PAIR_Y0 + ina_size()[1] + 0.5 + PB_W, BAT_TOP + STRAP_T + BOARD_LIFT]) rotate([-THETA, 0, 0]) rotate([0, 0, 180]) children();
...
module pbl_hous() color("#4a5568", 0.85) for (x = [32.2, 24.58, 22.04])
    translate([x - 1.27, 59.7, 35.8 + BOARD_LIFT - 1.27]) cube([2.54, 10, 2.54]);
...
function seat_d(hy) = hy * sin(THETA) + STRAP_T + BOARD_LIFT;   // 板の裏から天板の裏までの**鉛直**距離（板の持ち上げ込み）
...
function seat_nut_d(hy) = max(NUT_T, min(SEAT_NUT_DEEP, seat_d(hy) - SEAT_NUT_TOP));   // 天板の裏からのポケットの深さ
...
function slot_ceil(y) = NUT_SLOT_TOP + ((slot_out(y) > 0) ? (slot_len(y) + 1) : NUT_AF / cos(30) / 2) * tan(THETA);
function slot_ok(hy, y) = seat_d(hy) - slot_ceil(y) - NUT_SLOT_H >= NUT_SLOT_FLOOR;   // 肉が足りる穴だけ横穴にする
function seat_nut_ceil(hy, y) = slot_ok(hy, y) ? -slot_ceil(y) : -seat_d(hy) + seat_nut_d(hy);   // 締めたときのナットの上面
function seat_scr_l(hy, y) = 4;   // 🔒 6 本とも M2×4（天井を上げたので長さは 1 種類で足りる）
module seat_hole(hy, wy) seat_vert() {   // 通し穴＋ナットの居場所（どちらも鉛直）
    translate([0, 0, -seat_d(hy) - 3]) cylinder(d = SCR_D, h = seat_d(hy) + 6, $fn = 24);
    if (slot_ok(hy, wy)) {                                   // 横から差す捕捉溝
        z0 = -slot_ceil(wy) - NUT_SLOT_H;
        l  = slot_len(wy) + 1;
        ...
        translate([0, 0, -seat_d(hy) - 3]) hex_pocket(seat_nut_d(hy) + 3);
    }
}
...
function ina_hole_wy(hy) = ina_frame_y() - hy * cos(THETA);
function pb_hole_wy(hy)  = pb_frame_y()  - hy * cos(THETA);
module seat_screws() {
    for (h = ina_holes()) ina_frame() translate([h[0], h[1], 0]) seat_hole(h[1], ina_hole_wy(h[1]));
    for (h = pb_mount())  pb_frame()  translate([h[0], h[1], 0]) seat_hole(h[1], pb_hole_wy(h[1]));
}
module seat_bolt(hy, t, wy) seat_vert() {   // 🔴 現物の長さで描く（旧版は「入る上限の長さ」を描いていた＝実在しないネジ）
    l = seat_scr_l(hy, wy);
    ...
    color("#4a5568") translate([0, 0, seat_nut_ceil(hy, wy) - (NUT_T - 0.2)])
        rotate([0, 0, slot_ok(hy, wy) ? 30 : 0]) hex_pocket_af(4.0, NUT_T - 0.2);   // ナット（呼び 4.0 × 1.6・締めた位置＝天井）
}
module seat_hw() {   // ネジとナットの現物（検査と絵の用。straps_v4 には入れない）
    for (h = ina_holes()) ina_frame() translate([h[0], h[1], 0]) seat_bolt(h[1], ina_size()[2], ina_hole_wy(h[1]));
    for (h = pb_mount())  pb_frame()  translate([h[0], h[1], 0]) seat_bolt(h[1], pb_pcb_t(), pb_hole_wy(h[1]));
}
...
module w_pwr3() {
    ...
    color("#e53e3e") wire([[32.2, 70.5, 35.8 + BOARD_LIFT], [32.2, 70.7, 35.8 + BOARD_LIFT], [32.2, 70.7, 35.2], [64, 70.7, 35.2], [64, 70.7, 24], [64, 64.8, 24], [64, 64.8, 21.9]]);      // 5V
    color("#222222") wire([[24.58, 70.5, 35.8 + BOARD_LIFT], [24.58, 70.7, 35.8 + BOARD_LIFT], [24.58, 70.7, 33.7], [66.5, 70.7, 33.7], [66.5, 70.7, 26], [66.5, 64.8, 26], [66.5, 64.8, 21.9]]);   // GND
    color("#ecc94b") wire([[22.04, 70.5, 35.8 + BOARD_LIFT], [22.04, 70.7, 35.8 + BOARD_LIFT], [22.04, 70.7, 32.2], [69, 70.7, 32.2], [69, 70.7, 28], [69, 64.8, 28], [69, 64.8, 21.9]]);      // EN
}
// 電池のタブ（⚠ 後ろの端と仮定）→ 縦穴を上がり、左の壁ぎわを前へ → INA の INPUT の口。
//   ⚠ 2026-08-26 出発を Y 64.7 へ（電池を 1.0 後ろへ送った＝尻 63.9 の 0.05 後ろ。旧 63.7 は電池に 35mm³ 食い込んだ）。🔴 終端の止め代を +X（ハウジングの中）へ取る誤り → 出口の外 X 6.0（後端 6.8 から箱の半径 0.75 ＋ 0.05）
//   🔴 2026-08-25 壁 LW_X 1.694: 壁ぎわの車線を 2 → 2.75（壁＋0.3）・3.5 → 4.15 へ（基板 0.5〜20.5 の上を通る）
module w_batin() {
    color("#e53e3e") wire([[33.0, 64.7, 27], [33.0, 64.7, 28.5], [4.15, 64.7, 28.5], [4.15, 32.7, 28.5], [4.15, 32.7, 35.2 + BOARD_LIFT], [4.5, 32.7, 35.2 + BOARD_LIFT]]);
    color("#222222") wire([[35.0, 64.7, 27], [35.0, 64.7, 30], [2.75, 64.7, 30], [2.75, 28.4, 30], [2.75, 28.4, 36.1 + BOARD_LIFT], [4.5, 28.4, 36.1 + BOARD_LIFT]]);
}
// INA の OUT の口 → PB の JST（プラグは坂で持ち上がり Z 37〜43。前面 Y 32.1 から受ける）
module w_batout() {
    color("#e53e3e") wire([[4.5, 24.2, 36.9 + BOARD_LIFT], [2.75, 24.2, 36.9 + BOARD_LIFT], [2.75, 24.2, 42], [40, 24.2, 42], [40, 30, 42], [40, 30, 41 + BOARD_LIFT], [40, 32.1, 41 + BOARD_LIFT]]);
    color("#222222") wire([[4.5, 19.9, 37.8 + BOARD_LIFT], [2.75, 19.9, 37.8 + BOARD_LIFT], [2.75, 19.9, 43.5], [42, 19.9, 43.5], [42, 30, 43.5], [42, 30, 41 + BOARD_LIFT], [42, 32.1, 41 + BOARD_LIFT]]);
}
...
module w_xiao() color("#e67e22") {   // XIAO 7 本 = 上段 4 ＋ 下段 3（列の割り当ては respeaker_lite.scad XIAO_USED）
    ...
    wire([[39.3, 19.04, 19], [55.35, 19.04, 19], [55.35, 19.04, 27.13], [55.35, 29.9, 27.13], [76.01, 29.9, 27.13]], 3);
...
module w_oled() color("#2980b9")     // OLED 4 本: 口 → Z19 で左の溝（X 9）へ → 上がって前へ → Z 43.5 を右へ（INA の電源の口の頭 41.0 の上・傘はこの高さに居ない）→ 線の帯（X 37.9〜48.1）で上がる
    wire([[11.1, 38.09, 19], [9, 38.09, 19], [9, 38.09, 45.2], [9, 22.5, 45.2], [39.5, 22.5, 45.2], [39.5, 22.5, 46.5], [43, 22.4, 46.5]], 3);   // Z 45.2: 電池線（INA OUT→PB・X42 の縦走り Z≤44.25）の上。帯へは黒線の左 X 39.5 で上がる
module w_as5600() color("#8e44ad") {   // つまみ 5 本（GND 二股）: 口 → Z19 車線を右へ → X 59.5 で Z 13.3 へ下り（コネクタの逃げ 15.15 の下）→ 島の下を Y 36.8 へ。線は 2 列の真下で終わる（逃げの 0.35 下）
    wire([[38.05, 29.2, 19], [58.0, 29.2, 19], [58.0, 29.2, 13.3], [58.0, 36.8, 13.3]], 3);   // 島 −1.5 に追従
...
module w_btn2() color("#c0392b") {   // BTN2 2 本: 口 → Z19 で前へ（X 11.5・OLED の口の曲がり X≥13.26 を左に躱す）→ J2 ソケット（Y≲16）と INA の電源の口（Y 18.63〜）の間 Y 17.4 を上がる → タクトの足へ
    wire([[11.75, 54.85, 19], [11.75, 41.4, 19], [11.75, 41.4, 15.9], [11.75, 24.2, 15.9], [11.75, 24.2, 19], [11.75, 17.4, 19], [11.75, 17.4, 39.7], [11.75, 14.3, 39.7]], 2.2);   // Y 24.2〜41.4 は OLED の出口（Y 38.09・Z 19）と PHIN の 2 本（Y 26.4）の下 Z 15.9 をくぐる
...
module w_phin() color("#16a085") {   // スピーカー IN 2 本: J2 の確保空間の後ろの面から出て、Z19.6（下）/Z18（上）の 2 段で皿の下を右へ → 口の曲がりの手前 X 72.9 で上がって終わる
    wire([[6.6, 25.8, 19.5], [6.6, 26.0, 19.5], [8.5, 26.0, 19.6], [72.9, 26.0, 19.6], [72.9, 45.71, 19.6], [72.9, 45.71, 21.2]]);
    wire([[8.0, 25.8, 18.0], [8.0, 26.0, 18.0], [72.9, 26.0, 18.0], [72.9, 44.2, 18.0], [72.9, 44.2, 21.2]]);
}
...
module w_ina_i2c() color("#7f8c8d") {   // INA226 の I2C 4 本（ALER 裸）: 口 → 縦穴の後ろ寄り Y 68.1 を右へ → トグルとその線の東 X 51.6 で上がる →
    ...
    wire([[34.11, 64.76, 22.3], [34.11, 67.5, 22.3], [52.8, 67.5, 22.3], [52.8, 67.5, 40.6]], 2.2);
    wire([[52.8, 67.5, 40.6], [52.6, 67.5, 40.6], [52.6, 28.4, 40.6], [51.6, 28.4, 40.6]], 2.2);
    wire([[51.6, 23.9, 40.7 + BOARD_LIFT], [51.6, 30.1, 40.7 + BOARD_LIFT]]);   // 4 本ぶんの逃げの頭（〜39.54）に沿う受けの帯
}
...
module w_reed() color("#95a5a6") {   // リード 2 本: 🔒 直はんだ（2026-08-25 ユーザー）。座は knob_v5 デッキのポケット（REED_IN 14.5・接線向き・+Y 側）。
    ...
    wire([[56.97, 64.76, 22.3], [59.35, 64.76, 22.3], [59.35, 64.76, 42.6], [59.35, 61.5, 42.6], [59.35, 61.5, 41.0], [59.35, 57.4, 41.0]]);   // デッキの足 −1.5 に追従
    wire([[56.97, 64.76, 22.3], [56.97, 64.76, 44.6], [75.05, 64.76, 44.6], [75.05, 61.5, 44.6], [75.05, 61.5, 41.0], [75.05, 57.4, 41.0]]);
}
module w_chg() color("#dd6b20") {   // 充電 2 本: Type-C 基板の口（−Y 向き）→ 左の壁ぎわ X 4.1/4.9 を上がる → 天井の下 Z 47.3 を右へ → PB の USB ピン（8 番）の逃げの上で終わる
    wire([[5.8, 47.8, 6.5], [5.8, 43.0, 6.5], [5.8, 43.0, 47.3], [19.5, 43.0, 47.3], [19.5, 56.3, 47.3]]);   // VBUS（始点は基板 +1.2, +LW_X に追従）（⚠ どのピン対かは実物照合）。立ち上がりは電池線（X≤4.25）の右   // 🔒 2026-08-26 登りを Y 45.8 → 43.0 へ 2.8 前へ。3 点目の留め金（BLU_* の棚と耳・Y 45.0〜50.4）に 12.6mm³ 入っていた
    ...
    wire([[6.6, 47.8, 16.6], [6.6, 44.2, 16.6], [6.6, 44.2, 47.3], [28.9, 44.2, 47.3], [28.9, 58.9, 47.3], [29.66, 58.9, 46.4 + BOARD_LIFT]]);                    // GND ⚠ PB 側 8 ピン列 2 番（GND）と仮定・受けピン未実装
}
// PB の USB ピン（8 番・直ピン）に挿さる DuPont ＋ 逃げ。⚠ 逃げの頭 49.2 は v3 天井 48.454 を 0.75 超える（皮を描くときの宿題）
module pbu_hous() color("#4a5568", 0.85) {
    translate([19.15, 57.6, 35.62 + BOARD_LIFT]) cube([2.54, 2.85, 10.0]);
