include <btn_v1.scad>   // ⚠ v1（タクト）。2026-08-29 に v2、**2026-08-30 に v3** へ交代したが、まだ古い名前を参照する所が
...
// ---- 会話ボタン v3 の置き場所と、天板への置き道具（🔒 置き場所は 2026-08-29 に決定・2026-08-30 に v3 へ交代）----
...
BTN_ROW_DY = 3.0;   // 🔒 2026-08-28 前列（会話ボタン＋スピーカー）の逃がし量。SPK4 も同じ値を読む
                    ...
                    //   バスタブの −Y の壁が ReSpeaker に 0.18mm³ 食い込んでいた。掃引すると +0.2 で離れる。
                    //   ⚠ ボタンだけ動かすことはできない（下の ROW_Y はスピーカーと共有・🔒）。前列ごと動く。
...
//   （降りる側 top_knob() が置いていかれると、天面を降ろす検査だけが 5.4mm³ で光った）
//   KNOB4 だけ動かすと天板の座は動くのに基板とコネクタが置いていかれ、天板が core に
//   62.06mm³ 食い込んだ。**1 か所にまとめる。**
KNOB_DX = -2.5;   // 🔒 2026-08-26 ユーザー「両方を +4 移動」＝内側（−X）へ 4（+1.5 → −2.5）
KNOB_DY = 8;      // 🔒 2026-08-25 ユーザー「つまみを後ろに 5」→ 同日「つまみも 2mm 移動」で +7
                  // 🔒 2026-08-29 ユーザー「これならノブを後ろに下げた方がいいです」→ +8。
                  //   座の板の前端が 24.80 → 25.80 になり、スピーカーの縁の後端 25.70 から 0.10 離れる。
                  //   これで ① の欠きが要らなくなった。⚠ 動かしたら下の 2 本の線も同じだけ送ること
SPK4 = [KNOB_AT[0] + KNOB_DX, ROW_Y];                  // スピーカーの中心（X はつまみに追従・Y はボタンと同じ）
// ============================================================
...
// 🔄 2026-09-02 経緯（3 手かかった）:
//   ① 裏の彫り込み（WIN_SUNK 1.0）を廃止 ← 縁が 2.0−1.0−0.6 = 0.40mm で焼けなかった原因
//   ② 表に角ばった座を掘った ← ユーザー「裏掘りをやめ、表を掘りましょう」
//   ③ 🔴 **その座を廃止。** ユーザー「これじゃ 2 段ベゼルの二の舞だよ」。**その通り。**
//     btn_v3.scad に「🔒 2026-08-30 皿（口の手前の浅い凹み）は廃止。口が板を真っ直ぐ貫いて、
//     縁に面取り、だけ」と残っている。表の座は、そのとき捨てた皿と同じ形だった。
//   ⇒ **口（Type-C）の流儀に揃える: 貫通 ＋ 縁のベベルだけ。** 黒枠 61.0 は窓 62.0 を素通りし、
//     前へ 1.0mm 出る（ユーザー「多少出ても問題ない」）。縁に残る肉 = 2.0 − 1.0 = 1.0mm。
WIN_CH   = 1.0;   // 窓のベベル。🔒 口の PORT_BEV と同じ数字
WIN_SEAT = 0;     // 表の座は作らない（皿の二の舞）
WIN_R    = 0;     // ベベルの先端は丸めない（縁の肉を 1.0 のまま残す）
...
// 🔴 2026-09-02 実機（1525 のフロント）。縁が焼けなかったのは、ここが原因だった。
//   縁に残る肉 = BEZ_T 2.0 − 彫り込み 1.0 − ベベル 0.6 = **0.40mm**。PRINT.md の最薄肉 0.42 の下。
//   ⇒ ユーザー「窓の幅を 0.5mm 程度広げるだけで黒枠は出ますね。そうなると、周囲を掘る必要が
//     そもそもなくなる」。彫り込みをやめ、代わりに窓を広げて**黒枠を窓に通す**。
//   縁の肉 0.40 → **1.40mm**。⚠ WIN_SUNK_M（彫り込みが窓の外へ出る量 2.5）は掘らないので効かなくなる。
WIN_SUNK = 0.0;   // 🔄 1.0 → 0（掘らない）
// 🔒 ユーザー「窓の幅を 1mm 広げるだけで掘り込みがそもそも不要になる」＝**片側 +0.5**。
//   彫り込み（WIN_SUNK）は、黒枠が窓より大きくて通れないための逃げだった。窓を広げて通せば要らない。
//   ⚠ 模型の黒枠 OLED_GLASS = [61.0, 38.5] は窓（＝ガラス＋WIN_CL）より必ず小さく定義されているので、
//     **模型の当たり検査ではこの不具合は永遠に 0 になる**（2026-09-02 実際に 0 と出して外した）。
//     ⬜ 実物の黒枠の外形を測って OLED_GLASS を取り直すこと。ここの 0.8 はその代わりの逃げ。
// 🔒 2026-09-02 ユーザー「今の窓の高さで大丈夫です。横幅は 0.4mm 広げてください（1mm は広すぎました）」
//   ⇒ 縦は元のまま（片側 0.3 / 窓 39.1）、横だけ片側 +0.2（窓 61.6 → 62.0）。
WIN_CL_X = 1.0;   // 横・片側（窓 63.0）
                  // 🔒 2026-09-03 ユーザー「窓の横幅、1mm 増やしておいて。スっと入らないと困るから」
                  //   ⇒ 窓 62.0 → **63.0**（片側 +0.5）。黒枠の左右に 1.0 ずつ空く。
                  //   62.0 は 9/3 に刷って実物で合わせた版で、横は入ったが「スっと」ではなかった。
WIN_CL_Z = 0.15;  // 縦・片側（窓 38.8）
                  // 🔒 2026-09-03 ユーザー「縦幅そのものは 0.5mm くらい大きすぎている」⇒ 0.3 → 0.15（窓 39.1 → 38.8）。
                  //   0.5 まるごと詰めると隙間 0.05 になる。黒枠の 38.5 は 1 回の実測なので、
                  //   少しでも大きければ入らない＝また板が反る。半分だけ詰めた。
                  //   下辺中央の張り出しの逃げ（win_bulge）も、この隙間で全周に付く。
...
// 🔒 2026-08-28 ユーザー「落とそうか（天面に近づける）」。同日ベベル（EMB_H 0.6）を外して
//    ハニカムにしたとき、外面の彫りが無くなったぶん**グリルの長さが 0.9 → 1.5 に伸びていた**
//    （上の②が「グリルの長さ 0.9 も前と同じ」と書いているのはベベルがあった頃の話）。
//    0.6 だけ余分に持ち上げて 0.9 に戻す ── としたが、🔒 同日ユーザー「ビルドプレートから剥がす時に
//    割れない？」「潰れるのも怖い」で **0.3 戻した**。print_top は外面を下に刷る＝この穴あきの床が
//    プレートに直付きで、曲げに耐える力は板厚の 2 乗（0.9 では 1.5 の 1/2.8）。組む時もテープで
//    押し込む力をここが受ける。⇒ 持ち上げ 0.9・床 1.2mm（1.5 比 0.64・0.9 比 1.8 倍）で折り合う。
//    落とし込みは 座 0.9 ＋ 縁 0.8 ＝ 1.7 が枠（4.0）の横を押さえる。⚠ 残り肉は振動板の上 1.2・枠の上 1.6。
SPK_LIFT = 0.9;         // スピーカーを天面へ寄せる量（= 座の深さ）
SPK_REL  = 1.3;         // 振動板の逃げ ＝ 0.4 ＋ SPK_LIFT。⚠ include の途中では SPK_LIFT を式で参照できない
                        //    （OpenSCAD が「unknown variable」を出す）ので数字で置き、下の assert が食い違いを止める
assert(abs(SPK_REL - (0.4 + SPK_LIFT)) < 1e-9, "SPK_REL は 0.4 + SPK_LIFT（振動板の上の逃げを持ち上げ量に追従させる）");
...
module core() {
    ...
    translate([KNOB_DX, KNOB_DY, 0]) knob_at_v4();                            // つまみ一式（天面から下りる島・柱・基板・ヘッダ込み）
...
INA_DX = -4.7;
// 🔒 2026-08-29 ユーザー「PowerBoost の傾斜をあげられるだけあげる／INA226 を下に潜り込ませられるだけ
//   潜り込ませる」。会話ボタンのバスタブの下を空けるため。掃引の結果:
//     ・板の頭だけ見ると 21° まで行けるが、**PH プラグが抜き差しできなくなる**（ユーザー「PH コネクタ
//       刺せますかこれ」）。プラグは板と平行に抜けるので、傾斜のぶん通り道が持ち上がる。
//       通り道の頭: 10°→45.47 / 13°→47.42 / **14°→48.07** / 15°→48.71 / 21°→52.42。天井は 48.454。
//       ⇒ **14° が上限**。
//     ・その 14° で INA は **+3.0** まで送れる（+4 で PB に 1.83mm³）。前縁 Y 16.17 → 19.17。
//   ⚠ 帯（straps）はユーザー指示で今回の掃引から外してある。座とナットの溝は帯の再設計待ち。
THETA = 14;   // PowerBoost の傾斜（🔒 2026-08-25「傾斜を付けたら」→ 2026-08-29 に 10 から上げた）
INA_DY = 3.0;   // INA226 を +Y へ送る量（PowerBoost の持ち上がった前縁の下へ潜らせる）
...
module ina_frame() translate([INA_DX + BAT_X0 + (lipo_size()[1] - ina_size()[0]) / 2 + ina_size()[0], PAIR_Y0 + INA_DY + ina_size()[1], BAT_TOP + STRAP_T + INA_LIFT]) rotate([-INA_THETA, 0, 0]) rotate([0, 0, 180]) children();
...
// JP2 の i 番ピンの世界 X。板は pb_frame で 180° 回っているので **原点から局所 x を引く**。
// 🔴 2026-08-28 これが無かったせいで pbl_hous と w_pwr3 の X が世界の直書きになり、帯ごとの移動に
...
module tcb_v4() translate([-TCB_YB + LW_X, IN_Y - 0.50, 0]) rotate([0, 0, -90]) { tcb_at(); tcb_ra(); color("#63b3ed", 0.85) tcb_hous(); }   // 板の裏は左壁の内面（LW_X）にベタ付け。🔴 2026-09-05 まで −64.47 の直書き（TCB_YB は IN_Y 追従なので、奥行きを広げると板が 2 倍動いた）
// デュポンを挿す前の姿（板＋コネクタ＋L 型ピンだけ）。落とし込みの検査 close_tc はこれで見る:
//   手順では基板を立ててからハブの口を挿す（assembly_v4.html）ので、降りてくるときハウジングはまだ付いていない
module tcb_v4_bare() translate([-TCB_YB + LW_X, IN_Y - 0.50, 0]) rotate([0, 0, -90]) { tcb_at(); tcb_ra(); }
...
TC_PRESS = [IN_Y - 13.4, IN_Y - 9.8, TCB_ZTOP + 0.2];   // [Y0, Y1, Z0]。🔴 2026-09-05 まで 58.6/62.2 の直書き（基板はハッチ IN_Y に追従するので、押さえも IN_Y から測る）。🔴 2026-08-27 まで 20.7 の直書きだった。🔒「板が上へ動ける量 0.2」を式で持たせて、板の Z（TCB_ZTOP）を動かしても遊びが変わらないようにした。押さえの厚みは BAT_Z − BRG_T − ここ ＝ 21.4 − 20.95 ＝ 0.45（板を 0.25 上げたぶん 0.7 から薄くなった。📄 PRINT.md の最薄 0.42 は超える）
...
BLU_PTOP = 27.0;                // 耳と座の天面（耳の厚み 2.6）
// 🔒 2026-08-29 26.4 → 27.0（ユーザー了承・耳を 0.6 厚く）。ビスの座ぐり（深さ SCR_CBT）を
//   天面から掘ると、耳の底 BLU_TOP 24.4 との間に **0.40mm** しか残らず、刷るとそこが
//   「浮いた 0.4mm の板」になっていた（支柱を立てると破るので立てられない、と警告に出た）。
//   耳を厚くすると座ぐりの底が上がり、残りが 1.00mm になる。棚の中のナットは BLU_SKIN 1.6 で別に守られている。
...
// 🔴 2026-08-29 ここは長らく [[21.45, 33.2], ...] の**手打ち**で、STRAP_BANDS と同じ数字を二重に
//   持っていた。帯 A の幅を 6 → 9 にしたときレールの切れ目が付いてこず、⊓ の足が土手に
//   42.28mm³ 乗り上げた。⇒ **STRAP_BANDS の隙間から作る**。帯を動かせばレールが追従する。
//   左は最初の帯より前を作らない（元の RAIL_SEG_L と同じ流儀）。
function rail_segs(right) = [
    for (i = [(right ? 0 : 1) : len(STRAP_BANDS)])
        let (a = (i == 0) ? BAT_Y0 : STRAP_BANDS[i - 1][0] + STRAP_BANDS[i - 1][1],
             b = (i == len(STRAP_BANDS)) ? BAT_Y0 + lipo_size()[0] : STRAP_BANDS[i][0])
            if (b > a) [a, b]
];
...
// 🔒 2026-08-29 2 つに割った。**keepout（支柱を立ててはいけない体積）に入れてよいのは首の抜きだけ。**
//   🔴 それまで brgf_cuts() を丸ごと keepout_bridge() に渡していたので、ただのポケットである
//     掘り込みまで「立てるな」になり、その天井 141mm² の 96% が塞がれて**ブリッジに柱が 1 本も
//     立たなかった**（78mm² が無支持のまま・柱は 1108 本落ちていた）。
//     立ててはいけないのはネジ・ナット・軸・回る物で、フランジが沈むだけのポケットは違う。
module brgf_neck_cut()
    translate([LEG_X0 - BRGF_CL, BAT_Y0 - 1.0, BAT_Z - BRG_T - 1])
        cube([LEG4W + 2 * BRGF_CL, BRG_T + 1.0 + BRGF_CL, BRG_T + 2]);          // 首を通す抜き（Y 11.9〜15.1・皿を貫通）
module brgf_sink_cut()
...
module brgf_cuts() { brgf_neck_cut(); brgf_sink_cut(); }
...
// 🔒 2026-08-29 帯 A を**後ろへ広げる**（6 → 9・前端 15.45 は動かさない）。INA226 を +3 送ったので
//   前の取付穴が world Y 18.42 → 21.42 になり、旧 A（〜21.45）の後端まで 0.03mm しか残らず、
//   ナットを横から差す溝が作れなくなっていた。新 A（15.45〜24.45）で穴の後ろに 3.03mm 取れる。
...
// 🔒 2026-08-29 INA226 を PowerBoost の下へ潜らせたので、**PB の座の塊（board_under）が INA を
//   飲み込む**（113.28mm³）。座から INA の居場所を抜く。ビスのパッドは difference の外で union するので残る。
//   ⚠ 同時に INA の裏の足（L ヘッダ 1.2）も抜ける。SEAT_GAP 0.3 では足りておらず、潜らせる前から
//     0.73mm³ 刺さっていた。
INA_SEAT_CL = 0.4;   // INA のまわりに空ける逃げ
module ina_shadow() ina_frame()
    translate([-INA_SEAT_CL, -INA_SEAT_CL, -(1.2 + INA_SEAT_CL)])
        cube([ina_size()[0] + 2 * INA_SEAT_CL, ina_size()[1] + 2 * INA_SEAT_CL,
              1.2 + INA_SEAT_CL + ina_size()[2] + 20]);
module seats_v4() intersection() {
    union() {
        difference() { translate([0, 0, -SEAT_GAP]) board_under(); ina_shadow(); }
        seat_pads();
    }
    seat_cols();
}
...
NUT_SLOT_TOP   = 1.0;      // ナットの上に残す肉
// 🔒 2026-08-29 0.4 → 1.0（ユーザー了承）。旧コメントは「2 層。押されるだけなので曲げの肉は
//   要らない」で、**締める力に対しては正しい**。だが刷るときはこの面が天井になり、
//   剥離の力は締結の向きと関係なくかかる。0.4mm（8 層）の皮が浮いた状態で 2.5〜4.3mm 渡っていて、
//   支柱を立てても破るので立てられない、という警告が 3 か所（hatch / bridge / strap_a）出ていた。
//   座の深さ（seat_d = STRAP_T 2.0 + INA_LIFT 1.0 = 3.0）は変えていない。内訳の振り替えだけ:
//     天井 0.4 + ナット 1.7 + 床 0.9  →  天井 1.0 + ナット 1.7 + 床 0.3
//   床は「荷重を持たない縁」なので振り替えてよい。⚠ 1.6 は座に入らない（INA_LIFT を上げる必要）
...
//   🔴 2026-09-02 **下の抜け止めは成立していない**（ユーザー「下にEリング付ける溝ないじゃん」「ただ細くなってるだけ」
//     「電池は蓋にならないよね」）。帯の下に出る軸は 1.6 で、下の溝は軸の**先端**にあるため φ1.5 の段があるだけで
//     下側の壁が無い。リングは 0.1 の遊びの中で首の先から抜けられ、電池は柔らかいパウチで蓋にならない。
//     ⇒ 支柱を帯に留める物が無く、板と支柱 6 本がひとまとまりで帯から浮く。締め方は未決（docs/CASE-V4-OPEN.md）。
//
...
module w_pwr3() {
    ...
    //   ✅ 2026-08-28 ユーザーが実物で確認: **micro USB 側の端が `USB`、反対の端が `5Vo`**。
//   🔴 それまでこの行は「5Vo=1 番（x12.83）」と書いていて、列を端から端まで逆に持っていた（parts.scad の PB_PIN_* 参照）
    ...
    //       32.2/33.7/35.2 → 35.2/36.7/38.2。
    //    🔒 2026-08-28 **3 車線 → 4 車線**（充電の VBUS が同じ廊下に入るため）。トグルの真下の空きは
    //       **Z 33.55〜39.50 の 5.95mm**（_corridor.scad で実測）。両端に線の半径 0.75 を残した 34.30〜38.75 を
    //       4 等分 ＝ **34.30 / 35.78 / 37.27 / 38.75**（間隔 1.4833）。いちばん下が充電の VBUS。
    //       ⚠ 1.5 の剛体の管で描くと隣どうしが 0.017 ずつ重なるが、それは描き方の都合（CASE-V4-LOG §「線は剛体の管」）。
    //    ⚠ 線の出発はハウジングの出口の外 Y 70.5（後端 69.7 ＋ 0.75 ＋ 0.05）
    color("#e53e3e") wire([[pb_pin_x(pb_pin_5vo()), 70.5, 35.8 + BOARD_LIFT], [pb_pin_x(pb_pin_5vo()), 70.7, 35.8 + BOARD_LIFT], [pb_pin_x(pb_pin_5vo()), 70.7, 35.78], [64, 70.7, 35.78], [64, 70.7, 24], [64, 64.8, 24], [64, 64.8, 21.9]]);      // 5V
    color("#222222") wire([[pb_pin_x(pb_pin_gnd()), 70.5, 35.8 + BOARD_LIFT], [pb_pin_x(pb_pin_gnd()), 70.7, 35.8 + BOARD_LIFT], [pb_pin_x(pb_pin_gnd()), 70.7, 37.27], [66.5, 70.7, 37.27], [66.5, 70.7, 26], [66.5, 64.8, 26], [66.5, 64.8, 21.9]]);   // GND
    color("#ecc94b") wire([[pb_pin_x(pb_pin_en()), 70.5, 35.8 + BOARD_LIFT], [pb_pin_x(pb_pin_en()), 70.7, 35.8 + BOARD_LIFT], [pb_pin_x(pb_pin_en()), 70.7, 38.75], [69, 70.7, 38.75], [69, 70.7, 28], [69, 64.8, 28], [69, 64.8, 21.9]]);      // EN
}
...
// 🔴 2026-08-28（14 度目の机上の通し）**29.9 へ戻した。**上の 0.87mm は実物の当たりではなく、
//   コネクタの模型だけを Y へ −2 動かしていた ASC_DY の幻だった。ASC_DY は同日ユーザー指示で
//   削除され（コネクタは基板の上へ戻った）、通り道はいま **Y 27.535〜32.230 ＝ 4.695mm** ある
//   （断面の実測: XIAO 側のハウジングの尻 27.535 ／ AS5600 側の腹 32.230）。
//   束 3.0 を入れると片側 0.85mm ずつで、**ど真ん中が 29.88** なので 29.9 がその位置。
//   ⚠ 29.03 のままだと束の腹が XIAO のハウジングへ 0.005mm 入り、`chk_wire` が 0.168mm³ で
//     止まっていた（0 が正の検査）。戻して 0。上の 🔒 は「逃げ 0 でも通す」という**許し**で、
//     0 にしろという指示ではない。前提（幻の 0.87）が消えたので値を元へ戻した。
XIAO_UP_Y = 29.9;
...
module w_oled() color("#2980b9")     // OLED 4 本: 口 → Z19 で左の溝（X 9）へ → 上がって前へ → Z 43.5 を右へ（INA の電源の口の頭 41.0 の上・傘はこの高さに居ない）→ 線の帯（X 37.9〜48.1）で上がる
    ...
    // 🔒 2026-08-29 東西の走りを Y 35.6 → **39.5**。INA226 を +3 送ったので、直立ての電源の口の柱
    //   （X 10.53〜13.07・頭 Z 46.5）が旧の車線（35.6 ± 1.5 ＝ 34.1〜37.1）に入り 9.68mm³ 当たった。
    //   Z 46.5 のまま掃引すると空いているのは **Y 38.5〜41**（34→19.36 / 35.6→10.13 / 37→5.49 /
    //   38→1.68 / 38.5〜41→0 / 42→1.12）。その真ん中を取る。Z は上げられない（束の頭 48.0・天井 48.454）
    wire([[11.1, 38.09, 19], [6.5, 38.09, 19], [6.5, 38.09, 46.5], [6.5, 39.5, 46.5], [41.0, 39.5, 46.5], [41.0, 22.5, 46.5], [43, 22.4, 46.5]], 3);   // Z 45.2: 電池線（INA OUT→PB・X42 の縦走り Z≤44.25）の上。帯へは黒線の左 X 39.5 で上がる
...
function as_plug(s, r) =
    ...
    [KNOB_AT[0] + KNOB_DX + (-r * cos(R) - e * sin(R)),
     KNOB_AT[1] + KNOB_DY + (-r * sin(R) + e * cos(R))];
...
AS_TRUNK_Y = 36.8;   // 幹は 2 列の**あいだ**を通る（KNOB_DY 8 では 34.5 と 48.5 の間）
//   ⚠ 2026-08-29 つまみ +1 のとき、ここを 37.8 に動かせば直ると読んで一度動かしたが**関係なかった**。
//     close_top 5.4mm³ の真因は top_knob() が (-2.5, 7) を直書きしていて、降りる側のつまみ一式だけが
//     置いていかれていたこと。36.8 のままで当たりは 0。口の 2 列は as_plug() が KNOB_DY を読む。
...
module w_btn2() color("#c0392b") {   // BTN2 2 本: 口 → Z19 で前へ（X 11.5・OLED の口の曲がり X≥13.26 を左に躱す）→ J2 ソケット（Y≲16）と INA の電源の口（Y 18.63〜）の間 Y 17.4 を上がる → レバー式スイッチの足へ
    // 🔒 2026-08-26 帯ごと −4: 旧の立ち上がり（X 11.75・Y 17.4）は皿の下に入った。撃って探した空き車線は
    //    **X 3.5〜7.0 の Y 28**（J2 の確保空間 Y≤25.03 の後ろ・皿の縁 9.0 の左）。ここで上がって前へ回す
    // 🔴 2026-08-29 終点が **v1 のタクトの足のまま**だった（Z 39.7・X 16.6）。そこはレバー式 v2 では
    //   バスタブの横腹の中で、壁を 9.03mm³ 貫いていた。v2 の端子は胴の底から**下向き**に 3.5 出るので、
    //   行き先は Z 31.72〜35.22・X は芯 ±5.08（C 18.92 / NO 24.00 / NC 29.08・未使用）・Y はボタンの芯 17.3。
    //   横からは入れない（バスタブは底だけが開いている）ので、**バスタブの底の下**を通す。
    //   Z 31.9〜34.3 の帯は端子と自分の線以外に何も無いことを確かめてある。
//   ⬜ 2026-09-01 上の数字は **v2 の取り付け方**で出したもの。2026-08-30 の v3 交代（押し子を下から
//      通す・バスタブが別部品）でスイッチの保持が変わったので、終点を取り直したかは未確認。
    wire([[11.75, 54.85, 19], [11.75, 41.4, 19], [11.75, 41.4, 15.9], [11.75, 28.0, 15.9], [11.75, 28.0, 19], [5.5, 28.0, 19], [5.5, 28.0, 33.0], [5.5, 17.3, 33.0], [15.0, 17.3, 33.0]], 2.2);   // Y 24.2〜41.4 は OLED の出口（Y 38.09・Z 19）と PHIN の 2 本（Y 26.4）の下 Z 15.9 をくぐる
    // 端子は 1.2（X）× 0.8（Y）× 3.5（Z）の板。芯は C 18.92 / NO 24.00・Y 17.3・Z 31.72〜35.22。
    // 🔒 他の束と同じ流儀で、線は**端子の面で止める**（芯まで突っ込むと検査に重なりが出る）。
    //   束 1.5 の半分 0.75 ＋ 端子の半分 0.6 を引いた位置が面。
    wire([[15.0, 17.3, 33.0], [17.57, 17.3, 33.0]]);                                           // C（ヒンジ側・φ2.2 の穴の側）の −X 面 18.32 まで
    wire([[15.0, 17.3, 33.0], [15.0, 15.3, 33.0], [24.0, 15.3, 33.0], [24.0, 16.15, 33.0]]);   // NO（真ん中）の −Y 面 16.90 まで。C の足は Y 15.3 の車線で躱す
}
...
module w_tgl() color("#2c3e50") {   // トグル 2 本: 口 → 縦穴を上がる → 胴（Y 58.8〜）の手前 Y 55.4 を左へ → 端子の真上で終わる（ON-ON の中央＋片側 ⚠ どちら側かは配線時）
    ...
    wire([[49.35, 64.76, 21.9], [49.35, 64.76, 22.3], [55.2, 64.76, 22.3], [55.2, 64.76, 46.8], [55.2, 56.4, 46.8], [40, 56.4, 46.8], [40, 56.4, 44.75]]);
    wire([[49.35, 64.76, 21.9], [49.35, 64.76, 22.3], [55.2, 64.76, 22.3], [55.2, 64.76, 46.8], [55.2, 56.4, 46.8], [44.7, 56.4, 46.8], [44.7, 56.4, 44.75]]);
}
module w_reed() color("#95a5a6") {   // リード 2 本: 🔒 直はんだ（2026-08-25 ユーザー）。座は knob_v5 デッキのポケット（REED_IN 14.5・接線向き・+Y 側）。
    ...
    //   🔒 2026-08-29 つまみ +1（KNOB_DY 7 → 8）: 潜る点 61.5 → 62.5・終点 57.4 → 58.4
    wire([[56.97, 64.76, 22.3], [55.35, 64.76, 22.3], [55.35, 64.76, 42.6], [55.35, 62.5, 42.6], [55.35, 62.5, 41.0], [55.35, 58.4, 41.0]]);   // デッキの足 −1.5 に追従
    wire([[56.97, 64.76, 22.3], [56.97, 64.76, 44.6], [71.05, 64.76, 44.6], [71.05, 62.5, 44.6], [71.05, 62.5, 41.0], [71.05, 58.4, 41.0]]);
}
// ✅ 2026-08-28 決着。廊下の空きを実測し直すと **Z 33.55〜39.50 の 5.95mm**（古いコメントの 5.25 は誤り）。
//    3 車線 → **4 車線**に割り直し、いちばん下（34.30）を充電の VBUS にした。`USB` のパッドの真上は
//    マスタートグルの胴（Z 39.5〜47.5）なので、**上から降ろさず左の壁から低い車線で入る**。
//    部品との当たりは chk_wire / chk_wire_pwr / chk_all すべて 0。線どうしは 10mm³ 残るが、
//    それは 1.5 の剛体の管で描いている都合（CASE-V4-LOG §「線は剛体の管」）。
module w_chg() color("#dd6b20") {   // 充電 2 本: Type-C 基板の口（−Y 向き）→ 左の壁ぎわ X 4.1/4.9 を上がる → 天井の下 Z 47.3 を右へ → 板の上を越えて後ろへ回り、USB ピン（8 番）の DuPont へ後ろから入る
    wire([[5.8, 47.8, 6.5], [5.8, 43.0, 6.5], [5.8, 43.0, 34.30], [5.8, 70.7, 34.30], [pb_pin_x(pb_usb_pin()), 70.7, 34.30], [pb_pin_x(pb_usb_pin()), 70.7, 35.8 + BOARD_LIFT], [pb_pin_x(pb_usb_pin()), 70.5, 35.8 + BOARD_LIFT]]);   // 🔒 2026-08-28 上から降ろすのをやめた（`USB` のパッドの真上はマスタートグルの胴 Z 39.5〜47.5）。左の壁で Z 34.30 まで上がり、**廊下のいちばん下の車線**で右へ   // VBUS（始点は基板 +1.2, +LW_X に追従）（⚠ どのピン対かは実物照合）。立ち上がりは電池線（X≤4.25）の右   // 🔒 2026-08-26 登りを Y 45.8 → 43.0 へ 2.8 前へ。3 点目の留め金（BLU_* の棚と耳・Y 45.0〜50.4）に 12.6mm³ 入っていた
    // 🔴 2026-08-25 皮の検査: 旧終端（Z 50.05 まで跳ねて逃げ箱の上を越える）は天井 48.454 を 1.6 超えていた。
    // 🔒 2026-08-28 USB ピンを L 字にしたので、終端は他の 3 本（w_pwr3）と同じ **ハウジングの出口の外 Y 70.5・軸 Z 36.3**。
    //    板の上（Z 47.3）を後ろまで運んでから Y 70.7 の車線で降ろす。**板の中を横切らせない**（板は Y 37.1〜66.3・Z 30.9〜44.6）
    wire([[6.6, 47.8, 16.6], [6.6, 44.2, 16.6], [6.6, 44.2, 47.3], [pb_pin_x(pb_pin_gnd2()), 44.2, 47.3], [pb_pin_x(pb_pin_gnd2()), 70.7, 47.3], [pb_pin_x(pb_pin_gnd2()), 70.7, 35.8 + BOARD_LIFT], [pb_pin_x(pb_pin_gnd2()), 70.5, 35.8 + BOARD_LIFT]]);   // 🔒 2026-08-28 ユーザー決定「5Vo の隣の GND でいい」。受けピンも立てた（PB_RA_CHG）
}
// PB の USB ピン（8 番）に挿さる DuPont。🔒 2026-08-28 L 字に揃えたので、他の 3 本と同じ帯に並ぶ。
// 🔴 旧: 直ピン＋上向きの逃げ（縦 10 ＋ 肘 3.6・頭 Z 49.2）で、天井 48.454 を 0.75 超え、
//    天板を 1.77 彫って残り肉 0.73 にしていた。**その彫りは廃止**（case_v4.scad の天面）
module pbu_hous() pb_dup(pb_ra_chg());   // 充電の 2 本（`USB` と `5Vo` の隣の `GND`）
...
module top_knob()   translate([KNOB_DX, KNOB_DY, 0]) knob_at_v4();
module top_asconn() translate([KNOB_DX, KNOB_DY, 0]) as_conn();   // AS5600 のデュポン（分解図では出さない）
...
module top_btn() {   // 会話ボタン v3（レバー式マイクロスイッチ＋押し子）
    btn3_at_tub();
    btn3_at_switch();
    btn3_at_piston();
