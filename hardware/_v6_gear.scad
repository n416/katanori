...
//        -o x.png --camera=... -D "part=\"glook\"" hardware/_v6_gear.scad
//   part の値:
...
//     A1 **皿を表（顔）を下にして台に置く。**前の殻を顔を下にして皿へ被せ、スカート φ12.60 を
//        穴 φ12.85 へ入れる。止めピン（φ2.0・出しろ 0.90）が溝に入る向きで
//        ⚠ 皿は殻の**外**に付くので、殻を置いてから皿を載せることはできない。皿が先に台にある
//     A2 上（＝内側）からハブの柱を皿の穴へ挿す。**穴は二面幅**なので向きは 2 通りだけ
//     A3 **ここで回してみる。**皿を回してハブの歯車が連れて回ることを確かめる。
//        ⚠ この時点で回るのは 2 つ（皿とハブ）だけである。中継はまだ入れない・読み取りは板の側にある
//     A4 **殻を顔が上になるように返す。**皿は自重で壁の外面に座り、ハブは歯先 φ14.85 が
//        穴 φ12.85 を通れないので落ちない
//     A5 抜け止め: 皿の抜き φ11 から**未硬化レジンを呼び込みの円錐だけ**満たす。
//        🔴 **この工程は顔が上でなければできない。**注ぎ口は顔の側にあり、重力で柱の天面を
//           濡らす必要がある（下の「少し置く」）。A1 の姿勢のままでは注げない
//        ⚠ 三日月まで満たさない（UV の影に未硬化が残る）
//        ⚠ **注いだら少し置く。**荷重を持つのは栓と柱の天面（50.51mm²）の溶着で、
//           重力で天面が濡れてから硬化させる。⇒ 同じ穴から UV
//        ⚠ 走った分は穴の底の堀（φ9.70 × 0.40）が受ける。軸受けの 0.25 へ回り込ませない
//        🔴 **円錐と三日月が皿を止める。ただし止められるのは、栓が柱の天面に溶着している
//           間だけである。溶着が切れれば皿は栓ごと外れる。⇒ 荷重は溶着の引張 50.5mm² で読む。**
//           （測った: 皿と栓を 1 体で 3.0 持ち上げ → ハブとの交差は**空**／栓を止めて皿だけ
//            0.30 持ち上げ → **9.22mm³**。chk_weld_only / chk_ctrl21）
...
KN_MOAT_D = 9.70;                 // 堀の径。穴の底に彫って、走った未硬化レジンをここで止める
KN_MOAT_T = 0.40;                 // 堀の深さ（軸方向）
...
module knob_dish() color("#3c3f43") translate([KN_C[0], KN_C[1], 0]) {
    difference() {
        ...
        translate([0, 0, GTOP - 0.01]) cylinder(d = KN_MOAT_D, h = KN_MOAT_T);    // 堀
        translate([0, 0, GTOP + KN_ENG - 0.01])                                  // 呼び込みの円錐
            cylinder(d1 = KN_PEG + SLIP, d2 = KN_ID, h = KN_DISHZ - GTOP - KN_ENG + 0.02);
    }
}
// 栓（A4 で注ぐ未硬化レジンが硬化した物）。実体として持つ ── 磁石と同じで、模型に入る物である。
//   下端は柱の天面（GTOP + KN_ENG）で、そこから皿の呼び込みの円錐をそのまま満たす。
//   🔴 これは皿にもハブにも「掛かって」いない。柱の天面との**溶着**だけが繋いでいる。
PLUG_BOT = GTOP + KN_ENG;                       // 22.60 柱の天面＝栓の底＝溶着の面
PLUG_TOP = KN_DISHZ;                            // 23.05 皿の裏（円錐の上端）
//   🔴 円錐は皿の穴と**同じ式**で引く。別に引き直すと作図差が出て、面で乗るだけの所に
//      体積が立つ（最初 0.0833mm3 出た）
module knob_cone() translate([KN_C[0], KN_C[1], GTOP + KN_ENG - 0.01])
    cylinder(d1 = KN_PEG + SLIP, d2 = KN_ID, h = KN_DISHZ - GTOP - KN_ENG + 0.02);
module knob_plug() color("#cfa6d8", 0.85) difference() {
    knob_cone();
    translate([KN_C[0] - 20, KN_C[1] - 20, PLUG_BOT - 4]) cube([40, 40, 4]);   // 柱の天面より下を落とす
}
...
module sgear() color("#9aa5b1") translate([SENS[0], SENS[1], 0]) rotate([0, 0, ROT_S]) difference() {
    ...
    translate([0, 0, SGBOT - 1]) intersection() {                   // 止まり穴（下向き）。天井 0.40
...
EXP = 1;
module up(dz) translate([0, 0, EXP * dz]) children();
module out() translate([EXP * 7.0, 0, 0]) children();
module hold_wall() color("#c9c2a8") difference() {      // 前の壁（つまみと中継はここにぶら下がる）
    translate([0, 0, WALL_IN]) cube([BODY_X, BODY_Y, WALL]);
    knob_bore_front(); knob_stop_groove();
}
if (part == "ghold") {
    up(9.0) { knob_dish(); knob_reed_magnet(); }   // ① 外から落とす皿（＋リード用磁石 φ4×1）
    up(6.0) knob_plug();                 // ② 栓（円錐に注ぐ UV レジン）
    up(3.0) knob_hub();                  // ③ 内から挿すハブ＝つまみの歯車
            hold_wall();                 //    前の壁（①③ がこれを挟む）
    up(3.0) idler();                     // ④ 中継（載るだけ）
            idler_pin();                 //    壁から下ろした止まり軸 φ3.0
    // 読み取りの柱は壁の下に隠れるので、Z だけでなく +X へもずらして引き出す
    out() up(4.0) sgear();                     // ⑤ 読み取りの歯車（柱に被せる）
    out() up(2.0) { rotor(); rotor_magnet(); } // ⑥ ロータ＋磁石（筒へ上から落とす）
    out()         brk();                       // ⑦ ブラケット（板へ M2 で共締め）
    out()         chip_solid();
    color("#b9c6b9") translate([RS_X, RS_Y, RS_Z]) cube([34.007, 82.024, 1.85]);
    out() color("#d85a30") translate([HB_X, HB_Y, HB_Z]) cube([HB_W, HB_L, HB_T]);
}
if (part == "gspin") {
...
// ---- 栓の検査（2026-09-13・裁定 #4 を自分で確かめる）----
// ① 溶着が切れた場合: 皿と栓を 1 つの体として持ち上げ、ハブに当てる。
//    空なら「ハブ側に止める形は 1 つも無い」＝ 繋いでいるのは溶着だけ、が正しい
module dish_plug_up(dz) translate([0, 0, dz]) { knob_dish(); knob_plug(); }
if (part == "chk_weld_only")  intersection() { dish_plug_up(3.0); knob_hub(); }
// ② 栓が付いている場合: 栓を止めたまま皿だけ持ち上げる。ここが反応するなら栓は皿を止めている
if (part == "chk_ctrl21")     intersection() { translate([0, 0, 0.30]) knob_dish(); knob_plug(); }
// ③ 対照: 皿を動かさない（0.0000 ＝ 面で乗っているだけ）／下げると空（止まる向きは上だけ）
if (part == "chk_plug_seat")  intersection() { knob_dish(); knob_plug(); }
// ④ 栓と柱は形で掛かっていない（面で触れるだけ）
if (part == "chk_plug_peg")   intersection() { knob_plug(); knob_hub(); }
...
//    走らせ方: python hardware/tools/_v6_gear_sweep.py
