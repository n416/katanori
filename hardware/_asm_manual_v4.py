...
STEPS = [
 ...
 dict(n='10', t='つまみを組み、天面に小組を付けて配線する', img='st10', acts=[
   ...
   '押し棒（<span class="n">knob_v5.scad の part="rod"</span>）で<b>肩が裏の増し肉に当たるまで</b>押し込む。'
   ...
   '<b>ナットは天面の裏の下向きポケット</b>（裏面と面一）なので置いておけない。'
   ...
   '<b>［裏］AS5600 の基板</b>を、つまみの島の 4 本の柱へ <span class="d">M2×6</span> ×4'
   '（下から・ナットは柱のスロット）。<b>つまみまわりでは、これがいちばん最後。</b>'
...
'ナット 6 個を横穴にするため、板（電流計・PowerBoost）の蝶番を天板の面から <span class="d">0.5</span> 浮かせた。代金は 2 つ: <b>天面の裏の局所ポケット</b>（PB の USB の逃げ）が深さ 1.27 → 1.77 になり<b>板の残りが 0.73</b>、<b>INA の I2C の前走り</b>が 1.5×2 本では入らなくなり 2.2 の束 1 本に戻って <b>X の余裕が 0.35 → 0</b>。どちらも実物で効くかは刷ってから',
 ...
 ('⑧ 🆕 電池のコネクタ対の外形',
  ...
  '⬜ 現物待ち'),
 ('⑨ 🆕 つまみまわりの工具と手の道',
  '<b>工具と手の道の表（109 本）に、つまみまわりの道が 1 本も入っていない。</b>'
  '🔴 2026-08-27（12 度目の机上の通し）に見つけた: 手順 10 のつまみの小組そのものが手順に無かったので、'
  'その道も掃いていなかった。E リングの<b>片側 0.65mm</b> と島のねじの逃げは '
  '<span class="n">knob_v5.scad</span> の echo（🔒 ユーザーが実物で「難しいが工作できる」と判定済み）で、'
  '<span class="n">_asm_access.py</span> の掃引で出した数字ではない。'
  '⚠ 未測定なのは、島のナットを<b>下から押さえる指</b>の道と、'
  'AS5600 の基板を付けた後に E リングへ手が届くか（<b>この順では付けないので、いまは要らない</b>）',
  '⚠ 未測定'),
]
...
<header class="mast">
  ...
  <p class="sub">印刷部品 18 点・ビス 27 本・線 14 束 41 本（ほかにアンテナの同軸が 1 本）を、この順番で組む。
...
<div class="callout">
  ...
  <ul>
    ...
    <li><b>道具</b>: ドライバは先端 φ3.2 以下（座ぐりが φ3.4 なので、太いと頭に届かない）。
        ...
        トグル用の<b>六角のスパナ</b>、リードを押し込む<b>押し棒</b>
        （<span class="n">knob_v5.scad</span> の <span class="n">part="rod"</span> を刷る）。</li>
