...
STEPS = [
 ...
 dict(n='6', t='留め帯 A・B・C を横から差す', img='st6', acts=[
   '<span class="d">M2</span> ナット 6 個を、帯の天面に開いた<b>六角の落とし穴</b>へ<b>上から落とす</b>。'
   '穴は六角の二面幅で受けるので、落ちれば回らない。'
   '<b>そのすぐあとに、同じ穴の上へスペーサーを 1 個ずつかぶせて蓋にする。</b>'
   'スペーサーが蓋になるので、ナットは横へも上へも逃げない。',
   '<b>スペーサーは 6 個とも背が違う</b>（1.00 / 1.00 / 1.84 / 6.14 / 2.48 / 5.66mm）。'
   '低い 2 個が電流計、高い 4 個が PowerBoost。<b>順番を間違えると板が傾く。</b>',
   ...
   'A は電流計の前の穴、B は電流計の後ろの穴と PowerBoost の前 2 穴、C は PowerBoost の後ろ 2 穴を受ける。',
    ...
    note='🔴 2026-08-29 に<b>横穴をやめた</b>。6 か所のうち 3 か所は帯の幅が足りず横穴が作れておらず、'
         '残る肉も 0.30mm しかなかった。ナットを上から落としてスペーサーで蓋をする形に変えたので、'
         '横穴も、その外側の壁も要らなくなった。帯の天面はこれで平らな 1 枚になっている。'),
 ...
 dict(n='8', t='電流計と PowerBoost を座に締める', img='st8', acts=[
   '座は<b>手順 6 で置いたスペーサーの上面</b>。<b>傾きも浮きも 2 枚で別</b>で、PowerBoost は <b>14° 傾き</b>、電流計は <b>水平</b>。'
 ...
 dict(n='10', t='つまみを組み、天面に小組を付けて配線する', img='st10', img2='knob',
   ...
   '<b>🔴 板は裏返しでも留まってしまう。切り欠きが<u>スピーカー側</u>を向く向きで当てる。</b>'
   'ビス穴も足の窓も左右対称なので、向きを決めているのは<b>片側の角の切り欠き 1 つだけ</b>——'
   'これは ReSpeaker の前リブ（<b>羊羹</b>）の逃げで、反対を向けるとリブに乗って板が座らない。'
   '（🔴 2026-08-29 まで逃げが無く、<b>試し刷りでは入って本番の天板では入らない</b>状態だった。'
   '板は別に刷る部品なので、どの当たり検査にも出ていなかった）',
...
NUTS = [
 ...
 ('M2',  '6', '留め帯の座の<b>落とし穴</b>（帯の天面から落とし、スペーサーで蓋をする）', '蓋をすれば落ちない'),
...
PARTS = [
 ...
 ('スペーサー 6 個', 'print_spacer_0 〜 _5', '底（帯に当たる面）を下。φ5.8 の円盤・接地 22mm² なので<b>他の部品と一緒にプレートへ並べる</b>'),
...
<div class="callout">
  ...
  <ul>
    ...
    <li><b>ナットには 3 種類ある。</b>ブリッジの棚の 3 個は<b>横穴</b>なので、差せばどう向けても落ちない。
        留め帯の 6 個は<b>天面からの落とし穴</b>で、落としたあとスペーサーで蓋をして押さえる。
