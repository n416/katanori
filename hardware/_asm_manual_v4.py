...
STEPS = [
 dict(n='1', t='床にハブ基板を留める', img='st1', acts=[
   ...
   '🆕 <b>4 本ともナットドライバで締まる。</b>ナットの上の道は 前左 <span class="d">φ16.8</span>・前右 <span class="d">φ16.5</span>・'
   '後ろ左 <span class="d">φ10.1</span>・後ろ右 <span class="d">φ10.1</span> で、'
   '📄 5.5mm ナットドライバのボックス外径 <span class="d">9.0mm</span>（HOZAN D-840-5.5）が 4 本とも入る。'
   '🔴 <b>そのために、充電基板の受けはこの手順ではまだ置かない</b>（手順 4）。'
   '受けを先に置くと後ろ左が <span class="d">φ7.19</span> まで狭まって、ピンセットでナットを押さえる羽目になる。'
   ...
   '穴の位置は <span class="n">[9.0, 18.9] / [9.0, 64.9] / [77.0, 18.9] / [77.0, 64.9]</span>。ハブは箱の X 中央（<span class="n">X 6.0〜80.0</span>）に座る。',
         ...
         '🔒 2026-08-26 の「後ろ左だけピンセットで押さえる」は<b>不要になった</b>。'
         '塞いでいたのは充電基板の受けの補強の三角で、受けは 2026-08-27 に床から出て手順 4 で入る部品になった（<b>CASE-V4-OPEN.md</b> D-1）。'),

 dict(n='2', t='ReSpeaker を立てる', img='st2', acts=[
   'ReSpeaker を床の溝に上から差す。<b>ビスは無い。</b>頭は手順 11 で天面のリブ 2 本が <span class="d">0.3mm</span> 押さえる。',
   '向きは、イヤホンジャックと ReSpeaker 自身の USB-C が<b>左の壁</b>側、XIAO の USB-C が<b>右の壁</b>側。左の壁に開いているのはイヤホンジャックの丸い口（<span class="d">φ6.05</span>）だけで、ReSpeaker 自身の USB-C は 🔒 外に出さず、壁の内側の盲ポケットが受ける。',
    ...
    note='🔴 2026-08-25 <b>Type-C 基板はこの手順では入れない</b>。手順 4 で左の壁と一緒に降ろす。'
         '<b>その受け（別部品）も手順 4</b> で床の枠へ落とす（<b>CASE-V4-OPEN.md</b> D-1）。'),
 ...
 dict(n='3', t='ハブの口 10 本を全部挿して、低い車線に寝かせる', img='st3', acts=[
   ...
   '🆕 ⬜ <b>アンテナの同軸（XIAO の u.FL）も、いま挿して後ろへ出しておく。</b>この線は手順 2 で XIAO ごと箱に入っていて、'
   '行き先はハッチのスリット（トグルの下・幅 <span class="d">2.2</span>）と尻尾の中の溝＝<b>箱の後ろ</b>。'
   '🔴 <b>手順 11 で天面とフロントが閉じた後では挿せない</b>——残る開口は後ろのハッチ口だけで、'
   'そこから箱の一番前に居る XIAO までは手が届かない（手順 12 でトグルの 2 本を「箱の外で繋ぐ」のと同じ理由）。'
   '<b>箱の中のどの道を通し、どこで留めるかは ⬜ 未定</b>（<b>docs/TODO.md</b> の 🟡「アンテナの取り回し」・'
   '<b>docs/RESPEAKER-LITE.md</b>「存在は ✅・経路と固定は未定」）。'
   '🔒 2026-08-27 ユーザー: <b>長さは組みながら決める（現物合わせ）。</b>他の 13 束のように図面から折れ線長を出すことはしない。',
 ...
 dict(n='4', t='受けを落とし、左右の壁を降ろし、床の裏から 3 本で留める', img='st4', acts=[
   '🆕 🔴 <b>いちばん先に、充電基板の受け（<span class="n">v4_seat</span>）を床の枠へ真上から落とす。</b>'
   '枠は <span class="n">② 前の当て</span>の真ん前（<span class="n">X 1.694〜4.25・Y 54.50〜55.70・Z 0〜6.5</span>）に立っている 1 枚で、'
   '受けの前面がそこへ<b>面で当たる</b>。ケーブルを挿す力（<span class="n">−Y</span>・口の中心 <span class="d">Z 10.5</span>）を受けるのはこの面。'
   '<b>ビスは無い</b>——他の 3 方向は 左の壁の内面（<span class="n">−X</span>）・ハブの後ろ左の柱（<span class="n">+X</span>・<span class="d">0.3mm</span>）・'
   'ハッチの内面（<span class="n">+Y</span>）が止める。'
   '🔴 <b>手順 1〜3 では置かない</b>（後ろ左の M3 ナットの道が <span class="d">φ10.1 → φ7.19</span> に狭まる）。'
   '🔴 <b>板より後にも置けない</b>（底の座が板の下へ入るので、真上から落とすと <span class="d">255.5mm³</span> 当たる）。ここが唯一の場所。',
   ...
   '🆕 🔴 <b>Type-C 基板に充電の 2 本を挿すのは、板を手に持っているうち。</b>口は <span class="n">X 4.52〜7.07・Y 48.67〜58.67</span> の縦 2 段（<span class="n">Z 3.99〜9.07</span> と <span class="n">14.15〜19.23</span>）で、<b>下の口は上の口の真下</b>。壁を降ろした後は真上から届くのが上の口の頭までで、下の口には触れない。手順 5 でブリッジの帯（<span class="n">Y 50.5〜62.9・全幅</span>）が載ると、上の口も <span class="d">23.6mm</span>（頭の <span class="d">7.2mm</span> 上）で止まる。線は左の溝へ垂らしたまま降ろし、PowerBoost 側は手順 9 で挿す。✅ <b>2026-08-27（9 度目の机上の通し）</b>: この姿で降ろすと下の口のハウジングがハブ基板の左端に <span class="d">0.11mm</span> 乗っていた（1.169mm³・<span class="n">CHK="tcwall"</span>）。同日、口の中心を <b>Z 10.5 → 10.75</b> に上げて逃げ <span class="d">0.14mm</span> にした。板の下端は <span class="n">Z 0.75</span>、上端は <span class="n">20.75</span>。⚠ 板もハウジングも実測ではない（板は 📄 秋月の寸法図・ハウジングは 2.54 角の既定値）ので、<b>現物が来たら 3 つ（ハウジングの底・ハブ基板の上面・押さえの厚み 0.45）を測り直す。</b>',
   '🔴 <b>左の壁は Type-C 基板を抱いて降ろす。</b>基板の板の裏を左の壁の内面に当て、口の鼻先を<b>ハッチ側（後ろ）</b>へ向けたまま、壁と基板を一緒にまっすぐ下ろす。板の下端が受け（前の当て・後ろの控え・底の座）に入れば座る。<b>受けはこの手順の頭で床へ落としてある別部品</b>。<b>先に立てることも、壁の後から真上に落とすことも出来ない</b>——前者は板の裏を受ける面が左壁の内面そのものなので <span class="d">−X</span> へ倒れ、後者は天面の後ろ左のボスが板の真上を <span class="d">75.6mm³</span> 塞ぐ（<span class="n">close_tc</span>）。',
 ...
 dict(n='5', t='ブリッジを上から降ろす', img='st5', acts=[
   '🆕 🔴 <b>先に前板を床の溝へ差す。ブリッジは 2 部品。</b>前板（電池の返し＋前の脚・<span class="n">print_brgfront</span>・'
   '<span class="n">X 21.5〜44.5・Y 12.9〜14.9</span>）は 🔒 2026-08-26 にブリッジから切り離した別部品で、'
   '<b>ブリッジより先に</b>まっすぐ <span class="d">Z</span> で床の溝（深さ <span class="d">2.5</span>）へ落とす。ビスは無い。',
   '<b>そのあとブリッジ本体を、まっすぐ <span class="d">Z</span> で降ろす。</b>皿の裏の掘り込み（深さ <span class="d">1.0</span>）が前板のフランジに被さって、'
   'ここが前板の Z の留めと倒れ止めになる（前へ倒れるとフランジの背が掘り込みの天井、後ろへ倒れると首の背が抜きの奥に当たる）。',
   ...
   '帯の裏の左端に <b>Type-C 基板の押さえ</b>（<span class="n">X 1.694〜3.9・Y 58.6〜62.2</span>・下端 <span class="n">Z 20.7</span>）が付いている。ここが基板の Z の抜け止めなので、<b>ブリッジを載せるまで基板は上へ抜ける。</b>',
    ...
    note='✅ 2026-08-26 <b>3 本目は帯の前（OLED 側・<span class="n">X 4.194, Y 47.7</span>）へ移した。</b>'
         ...
         '🔴 2026-08-27（7 度目の机上の通し）に見つけた: この手順は<b>前板がブリッジの一部だった頃の文</b>のままで、'
         '「前の脚を床の受け溝へ入れながら降ろす」と 1 つの動きに書いてあった。前板は 🔒 2026-08-26 に別部品になっていて、'
         '<b>ブリッジを降ろした後では 1450mm³ 当たって入らない</b>（<span class="n">CHK="brgf_bad"</span>）。'
         '締めた 3 本を外してブリッジを抜くまで戻れないので、順を 2 つに分けて書き直した。'),
 ...
 dict(n='11', t='天面を降ろし、OLED を留め、フロントを差す', img='st11', acts=[
   ...
   '🆕 <b>ビスより先に</b> <span class="d">M2</span> ナット 2 個を、L の裏のポケットへ<b>後ろから差す</b>。L の後ろの空きは<b>左が <span class="d">3.4mm</span>・右が <span class="d">4.45mm</span></b>（ナットの厚み <span class="d">1.6</span> に対して）。'
 ...
 dict(n='12', t='トグル・ハッチ・電池の蓋・尻尾', img='st12', acts=[
   'トグルはハッチの穴にネジ部を通し、<b>外から六角ナット</b>で締める。⬜ <b>アンテナの同軸は、手順 3 で後ろへ出しておいた線</b>をトグルの下のスリットへ通す。ここで初めて挿すことは出来ない。',
   ...
   '尻尾をトグルのレバーのボアに挿す。アンテナの同軸は尻尾の中の溝（<span class="n">ANT_OFF 4.5</span>）へ。⬜ レバーは倒れる（尻尾も一緒に回る）ので、その分のたるみが要る。長さは未計測。',
 ]),
]
...
CHECKS = [
 ('🆕 ReSpeaker を床の溝へ差す', 'CHK="rsp"', '<b>0</b>', 'ok',
  '🆕 2026-08-27（9 度目の机上の通し）に初めて当てた。ここには反例（<span class="n">rsp_after</span> 444.4）だけがあって、'
  '<b>入れる動きそのものが掃かれていなかった</b>'),
 ...
 ('🆕 壁を降ろす（相手に<b>受け</b>を入れて）', 'CHK="wall_l_seat" / "wall_r_seat"', '<b>0 / 0.01mm³</b>', 'ok',
  '🆕 2026-08-27（9 度目）。受け（<span class="n">tc_seat4</span>）は 🔒 同日に床から出た別部品で、手順 4 の<b>頭</b>で先に落とす。'
  '壁はその後に降りるのに、<span class="n">wall_l</span> / <span class="n">wall_r</span> の相手（<span class="n">upto(3)</span>）に受けが居なかった。入れても変わらない'),
 ('🆕 左の壁が <b>Type-C 基板を抱いて</b>降りる', 'CHK="tcwall"', '<b>0</b>', 'ok',
  '🆕 2026-08-27（9 度目の机上の通し）に初めて当てた。手順 4 は 🔒「充電の 2 本を挿すのは板を手に持っているうち」なので、'
  '降ろすとき板には<b>デュポンのハウジング 4 個</b>が付いている。それまでの <span class="n">close_tc</span> は板を '
  '<span class="n">tcb_v4_bare</span>（デュポン<b>抜き</b>）で掃いていて、この姿を見ていなかった。'
  '🔴 当てたら <b>1.169mm³</b> 出た —— <b>下の口のハウジングの底が、ハブ基板の上面に 0.11mm 食い込んでいた</b>'
  '（<span class="n">X 6.002〜7.065・Y 48.669〜58.670・Z 3.99〜4.10</span>）。掃引ではなく<b>静止</b>の当たりで、'
  '受けが床の一部だった頃から在ったもの（分割の前の版でも同じ 1.169mm³）。'
  '✅ 同日、口の中心 <span class="n">CHG_C_LW</span> を <b>10.5 → 10.75</b>（板ごと 0.25 上げ）にして 0 にした。'
  '逃げ 0.14。押さえは <span class="n">TCB_ZTOP + 0.2</span> の式にして 🔒 遊び 0.2 を保ち、厚みが 0.7 → 0.45 になった'),
 ('🆕 前板を床の溝へ差す（ブリッジより先）', 'CHK="brgf"', '<b>0</b>', 'ok',
  '🆕 2026-08-27 に初めて当てた。前板は 🔒 2026-08-26 にブリッジから切り離された別部品なのに、'
  '<b>差す動きが 1 度も掃引されていなかった</b>（掃いていたのは本体の <span class="n">brg_v4</span> だけ）。線を入れても 0'),
 ('（反例）前板をブリッジの後から差した場合', 'CHK="brgf_bad"', '<b>1450mm³</b>', 'stop',
  '皿の裏の掘り込みが被さっているので入らない。<b>前板はブリッジより先</b>であることの裏取り'),
 ('ブリッジ本体を上から降ろす', 'CHK="brg"', '<b>46.4</b>（剛体は 0）', 'warn',
 ...
 ('🆕 留め帯を +2.0 の位置へ上から降ろす（押し込む前）', 'CHK="strap_down"', '<b>15.0</b>（剛体は 0）', 'warn',
  '🆕 2026-08-27 に初めて当てた。⑥ の掃引は <b>−X へ 2.0 押し込む所しか見ていなくて</b>、'
  'その前に必ず要る「+2.0 ずらした位置へ上から降ろす」動きが抜けていた。剛体は 0（足はレールの外へ落ちる）。'
  '残る 15.0mm³ は <span class="w">BTN2</span> の立ち上がり（<span class="n">X 11.75・Y 17.4</span>）で、線だけ。'
  '模型は線を<b>最後の道</b>で描いてあるが、この時点ではまだ低い車線に寝ているので、帯 A を降ろすときに指でよける'),
 ...
 ('OLED を上から降ろす', 'CHK="oled"', '<b>0</b>', 'ok',
  '🔴 2026-08-27、<b>この <span class="n">CHK</span> は検査スクリプトに実在していなかった</b>。'
  '打つと空のモデルが出るだけで、前からここに載っていた 0 は<b>測った 0 ではなかった</b>。'
  '書いて回したら本当に 0 だった（相手 = 手順⑧まで）'),
...
PARTS = [
 ...
 ('ブリッジ', 'print_bridge', '皿を伏せる（レール・パッド・ビスの座は全部上を向く）。<b>皿の裏が平らにプレートへ着く</b>（接地 2304mm²）。⚠ 2026-08-27 まではここに充電の Type-C の押さえが 0.7 下がっていて皿が浮いていたが、🔒 ユーザー指示で押さえを左の壁へ移した'),
 ('前板（電池の返し＋前の脚）', 'print_brgfront', '<b>前面を伏せて寝かせる</b>（🔒 2026-08-26 ユーザー）。前面（<span class="n">Y 12.9</span>）は 634mm² の 1 枚の平らな面'),
 ('留め帯 A / B / C', 'print_strap_a / _b / _c', '足とツバを下（ツバの上面が 45° なので支え無し。天板が足の間 36mm を渡る）'),
 ('会話ボタンのキャップ', 'print_btn', '天面を伏せる'),
 ('🆕 充電基板の受け', 'p_seat / print_seat', '<b>壁に当たる面（−X）を伏せて寝かせる。</b>部品の背は <span class="d">4.21mm</span>。立てると <span class="n">4.21 × 19.8</span> の足で 20.5 の塔になる。接地 100mm² なのでラフトを敷く'),
...
<header class="mast">
  ...
  <p class="sub">印刷部品 15 点・ビス 27 本・線 13 束 39 本（＋ ⬜ アンテナの同軸 1 本）を、この順番で組む。
...
<div class="callout">
  ...
  <ul>
    ...
    <li>🔴 <b>前板はブリッジより先。</b>ブリッジは <b>2 部品</b>（本体と前板）。前板は床の溝へ差し、皿の裏の掘り込みが上から被さって止まるので、ブリッジを降ろした後では入らない（<span class="n">CHK="brgf_bad"</span> で 1450mm³）。</li>
