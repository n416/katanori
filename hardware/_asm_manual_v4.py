...
STEPS = [
 ...
 ], warn='上限を超えると先が天板の裏を抜けて<b>電池の頭を突く</b>（天板の裏＝電池の上面で、隙間はゼロ）。🔴 2026-08-25 夜の板の 0.5 持ち上げ（<b>CASE-V4-LOG.md</b> §13.7）で、先が出る穴は無くなった（一番きついのは電流計の後穴で残り <span class="n">0.49mm</span>）。長い物に替えないこと。',
 ...
 dict(n='11', t='天面を降ろし、OLED を留め、フロントを差す', img='st11', acts=[
   ...
   '🔴 <b>ナットは右だけ入らない。</b>L の後ろの空きは左が全開、<b>右は <span class="d">1.3mm</span></b>（すぐ後ろが天面のつまみの島）。ナットの厚みは <span class="d">1.6</span> なので口まで持って行けない。<b>天面を外して裏返しても同じ 1.3mm</b>で、塞いでいるのが天面自身の島なので<b>順番では解けない</b>。形を変えるしかなく、直し方は未決（<b>CASE-V4-OPEN.md</b> T-2）。',
...
CHECKS = [
 ('壁を上から降ろす', 'CHK="wall_l" / "wall_r"', '<b>0 / 25mm³</b>', 'warn',
  '2026-08-25 に前の床ボスを L 形にして解消（旧 3.1 / 29.0）。2026-08-26 に右へ出た 25mm³ は、'
 ...
 ('ブリッジを上から降ろす', 'CHK="brg"', '<b>26 → 0</b>', 'ok',
  '内訳は BTN2 の<b>足へ渡る最後の枝</b> 10mm³（タクトは手順 10）と <span class="w">PHOUT</span> の天井下 16mm³（上と同じ）。どちらもこの時点では存在しない。'
 ...
 ('ブリッジを箱へ留める（棚・ビス・本体）', 'W="brgldg" / "brghw" / "brgchk"', '<b>0 / 0 / 0</b>', 'ok',
 ...
 ('留め帯 3 本を横から差す（−X へ 2.0）', 'CHK="strap" / "strap_dry"', '<b>20.6 / 0</b>', 'warn',
  '🔒 2026-08-26 にツバと溝を付けて、上から入れる形をやめた。剛体との当たりは <b>0</b>（<span class="n">strap_dry</span>）。'
  '残る 20.6mm³ は帯 A の右足が通る <span class="w">XIAO</span> の束（<span class="n">X 53.85〜55.0・Y 17.54〜21.45</span>）だけで、線なのでよけて差す。'
  '左から差す向きにすると <span class="w">BTN2</span> の束で 32.7mm³ になるため、右からに決めた'),
 ('（反例）帯を上からかぶせた場合 / 電池を先に入れた場合', 'CHK="strap_top" / "strap_bad"', '75 / 221mm³', 'stop',
 ...
 ('電流計と PowerBoost を座へ降ろす', 'CHK="boards"', '<b>3.4 → 0.4mm³</b>', 'warn',
  'BTN2 の枝を除いても <b>0.37mm³</b> 残る。電流計の板の縁 ↔ 帯 A の座（<span class="n">X 15.48〜16.12・Y 19.60〜20.38・Z 33.51〜34.51</span>）で、静止でも同じ値が出る'),
...
② <b>OLED の右のナット</b>は、L の後ろの空きが <span class="d">1.3mm</span>（すぐ後ろが天面のつまみの島）。
ナットは厚み <span class="d">1.6</span> なので口まで持って行けない。<b>天面を箱から外して裏返しても同じ 1.3mm</b>——
塞いでいるのが天面自身の島なので、<b>順番では解けない</b>。左は外まで開いている。
