...
**回し方と読み方は [SWEEP.md](SWEEP.md)**。ここでは考え方だけ:

- 箱は灰色の半透明、動く物は不透明
- 色は **2 色**（青＝問題なし／赤＝めり込み）。下の「色は 2 色」参照
- 当たった所では**交わりそのもの**を赤い実体で出す（OpenSCAD で厳密に取った形）
- 並びは ①入れる ②交わりを見せる ③**組み上がった姿勢で 2.5 秒止める**（ここで終わる）
...
**2026-09-17 にやって、2026-09-18 に捨てた形**: 蓋を 2 つの剛体（`lidmain` 天板・`lidflap` 左の板）に
割って別々に当てた。数字（最後の 0.44mm）は合っていたが、**左の板を剛体のまま 1.5 横へずらす**ので、
1 つの部品であるはずの**付け根が 1.5mm せん断された**形になっていた。
旧 `lid_flexed()` は 2 つを `union` していたので段差が塊の中に隠れていて、割って並べて描いて初めて見えた。

🔒 ユーザー「天板と左壁はつながっています」「お前はなにをしようとしている」「なんで折り目で曲げようとしている？
ナイロンだから柔軟にグニャグニャ曲がると思っているのか」。

**いまの形（2026-09-18）**: **剛体に割らない。形そのものを作り直す。**
`flap_bent(d)` が Z を 12 段に切り、段ごとに X のせん断（`multmatrix`）を掛けて片持ち梁の形を作る。

```
y(x) = d/2 ・( 3(x/L)^2 − (x/L)^3 )      x は付け根からの距離・L は付け根から口まで 26.50
```

口より下は荷重が無いので直線。実測 **付け根 0.016・口 1.602・下端 3.559・1 つの塊**。
d = 1.5 で 6.24N・表面のひずみ 0.53%（引継ぎの「約 8N・0.6%」と一致・PA12 の弾性内）。

道の点は 7 要素になった（`[dx,dy,dz,rx,ry,rz,d]`）。**7 つ目が変わると形が変わる**ので、
道具はたわみ量を 0.1 刻みに**小さい側へ丸めて**（判定として厳しい側）形を作り直し、姿勢ごとに使い分ける。

**結果**: 降ろす間は全部 0。当たるのは最後に板が戻る 2 コマだけ（0.220 → **0.440mm**）＝ スナップ。
台帳に `lid-snap-front` / `lid-snap-rear` として理由付きで載せた。

⚠ **蓋を平行移動させる道は作れない。** 蓋は 4 本の上の柱（`POSTS_T`）で位置決めされていて、
X にも Y にも **0.3mm** しか動けない（前左の柱の +X 面 6.69 ／ 天板の −X 面 7.0）。
口が要る逃げは **1.25mm**。だから逃げを出せる場所は左の板 1 枚だけで、これがたわみ部品である理由。
曲げない剛体として降ろすと口で **1.958mm** 当たる（2026-09-18 実測）。

### 色は「判断が要るか」だけを持つ（2026-09-18）

🔒 ユーザー「lidflap.mp4 を見る限り、入らないといっているように見えますね」。
皮で触れているだけの所を黄色にしていたため、`lidflap` は道の全域（s 0〜2）で触れていて
**最初から最後まで黄色**になっていた。自分で「皮はめり込みではない」と決めておきながら、
絵では警告を出しっぱなしだった（実測は s 0〜1.5 が全部 厚み 0.000 の 4 面の皮）。
⇒ まず **青＝めり込み無し（触れているのも含む）／赤＝めり込み** の 2 色にしたが、
今度は**了承済みのめり込みまで赤く**なり、蓋の最後の絵が真っ赤になった。台帳を作った意味が消える。
⇒ 最終形は 3 段: **青＝めり込み無し／橙＝了承済み／赤＝新しいめり込み**。
色が持つのは「**判断が要るか**」だけ。触れていることは焼き込みの文字（`touch`）に残す。
...
- 🔴 **ナイロン（MJF）しか見ていない。** `sweep_chk.py` は `-D MAT=` を渡しておらず、
  `case_v6_1.scad` の既定（`MAT = "nylon"`）のまま回る。6 本の道もナイロンの入れ方で、
  板 6 枚のレジンの組み立てを表していない。
  🔒 ユーザー 2026-09-15「レジン版とナイロン版はスイッチできるわけで、それに応じてアラートを分けて」
  に、**この道具だけが従っていない**。レジンの道を書くところから要る（2026-09-18 時点で未着手）
