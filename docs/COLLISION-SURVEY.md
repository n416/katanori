...
### 絵は「実物の部品」で見せる（2026-09-18）

🔒 ユーザー「天板と左壁はつながっています」。
検査は「たわむので 2 つの剛体」に分けるが、**実物は 1 つの部品**なので、
動画で左の板だけを単独で飛ばすのは**嘘の絵**だった。
⇒ 動画は場面（`lid`）として 2 つを同じ時間軸で動かす。区間の数は短い道に最後の姿勢を足して揃える。
交わりを見せるコマでは、**当たっている物だけ**を消して相手（天板）は残す。
...
⇒ **青＝問題なし（触れているのも含む）／赤＝めり込み**の 2 色だけにし、
